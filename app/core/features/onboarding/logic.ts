...
export const determineCompletedSteps = (values: OnboardingFormValues): OnboardingStepId[] => {
  ...
  const hasBasicDetails =
    ...
    values.usPassport ||
    values.openToTravel ||
    values.driversLicense !== (DRIVER_LICENSE_OPTIONS[0]?.value ?? 'none') ||
    values.veteran
  ...
  const hasRoleDetails = yearsExperience !== '' && jobTitle !== '' && values.primarySkills.length > 0
  ...
  const hasEducationDetails =
    ...
    hourlyRate !== '' ||
    sanitizeCertifications(values.certifications ?? '').length > 0
...
export const getNextOnboardingStep = (completedSteps: OnboardingStepId[]): OnboardingStepId => {
  const firstIncomplete = stepOrder.find((step) => step !== 'summary' && !completedSteps.includes(step))
