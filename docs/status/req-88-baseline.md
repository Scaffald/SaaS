...
| 16 | Implement pre-funding account credits system | ✅ COMPLETED | Migration `128_req_88_account_credits.sql` creates `account_credits` and `credit_ledger` tables with helper functions. `payments.router.ts` includes `getAccountCredits`, `depositCredits`, `getCreditLedger`, `applyCreditsToPayment`, `checkAndApplyCredits`. `OrganizationCreditsPanel` component added to Office organization edit page | Future: Integrate credit application into success fee and background check payment flows (check credits before creating PaymentIntents, use credits if sufficient, charge remainder), add credit usage preference settings |
