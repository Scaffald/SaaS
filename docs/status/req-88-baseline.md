...
| 14 | Implement organization payment method management | 🟡 PLANNED | Not started | Schema for saved payment methods (Stripe customer refs), tRPC router, SetupIntent workflow, Office UI |
| 15 | Build payment transaction history and receipt system | 🟡 PLANNED | `payment_transactions` table populated by routers/webhook | Need list/export endpoints, PDF/email receipts, Office-facing tables |
