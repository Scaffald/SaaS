 ...
 * Manual broker and contractor registration fixtures
 * E2E testing and error handling validation
 *
...
export async function createTestManualUserWithInvitation(
  ...
  if (manualUser.email) {
    const invitationData = {
      ...
      expires_at: invitationOverrides?.expires_at ??
        new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
      created_at: new Date().toISOString(),
    };

    const { data: invitation, error: invError } = await forsured(
    ...
    if (invError) {
      throw new Error(
        `Failed to create invitation for manual user: ${invError.message}`,
      );
    }

    // Track for cleanup
    createdManualUserData.push({
      table: "forsured.relationship_invitations",
      id: invitation.id,
    });
...
export async function createMergeScenario(input: MergeScenarioInput) {
  ...
  const { manualUser, invitation } = await createTestManualUserWithInvitation(
    manualUserData,
  );
