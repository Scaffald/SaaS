...
export interface SeededContractorData {
  ...
  relationships: Array<
    { id: string; manager_org_id: string; subcontractor_org_id: string }
  >;
...
async function createTestManagers(
  ...
  if (manager1.error) {
    throw new Error(
      `Failed to create test manager 1: ${manager1.error.message}`,
    );
  }
  if (manager2.error) {
    throw new Error(
      `Failed to create test manager 2: ${manager2.error.message}`,
    );
  }
...
async function createTestRelationships(
  ...
  Array<{ id: string; manager_org_id: string; subcontractor_org_id: string }>
> {
  ...
  if (rel1.error) {
    throw new Error(
      `Failed to create test relationship 1: ${rel1.error.message}`,
    );
  }
  if (rel2.error) {
    throw new Error(
      `Failed to create test relationship 2: ${rel2.error.message}`,
    );
  }
...
async function createTestProjects(
  ...
  if (project1.error || project2.error) {
    throw new Error(
      `Failed to create test projects: ${
        project1.error?.message || project2.error?.message
      }`,
    );
  }
...
async function createTestDocuments(
  ...
  if (!projectId) {
    ...
    if (tempProject.error) {
      throw new Error(
        `Failed to create temp project for documents: ${tempProject.error.message}`,
      );
    }
  ...
  if (subcontractor.error) {
    ...
    if (
      subcontractor.error.message.includes("multiple") ||
      subcontractor.error.message.includes("no rows")
    ) {
      ...
      if (allSubs && allSubs.length > 0) {
        ...
        console.warn(
          "[seed-contractor-data] No subcontractor found, skipping document creation",
        );
        return [];
      }
    } else {
      throw new Error(
        `Failed to lookup subcontractor: ${subcontractor.error.message}`,
      );
    }
    ...
    if (newSubcontractor.error) {
      throw new Error(
        `Failed to create subcontractor: ${newSubcontractor.error.message}`,
      );
    }
  ...
  if (doc1.error || doc2.error) {
    throw new Error(
      `Failed to create test documents: ${
        doc1.error?.message || doc2.error?.message
      }`,
    );
  }
  ...
  if (notif1.error || notif2.error) {
    throw new Error(
      `Failed to create test notifications: ${
        notif1.error?.message || notif2.error?.message
      }`,
    );
  }
  ...
  if (task1.error || task2.error) {
    throw new Error(
      `Failed to create test tasks: ${
        task1.error?.message || task2.error?.message
      }`,
    );
  }
 ...
 */
async function createTestHelpArticles(): Promise<
...
> {
  ...
  if (article1.error) {
    ...
    console.warn(
      "Help article creation failed (may already exist):",
      article1.error.message,
    );
...
export async function seedContractorTestData(
  ...
  const { contractorUserId, contractorOrgId, managerOrgId, managerUserId } =
    options;
  ...
  try {
    const { data: existingContractorOrg, error: checkError } = await core(
      "organizations",
    )
      .select("id")
    ...
    if (
      ...
      (checkError.message.includes("does not exist") ||
        ...
        checkError.message.includes("42P01"))
    ) {
      console.warn(
        "[seed-contractor-data] core.organizations table not found, skipping organization creation",
      );
    ...
    console.warn(
      "[seed-contractor-data] Error checking/creating contractor org, continuing:",
      error,
    );
  }
  ...
  try {
    if (managerOrgId) {
      ...
      const { data } = await forsured("organizations").select("id, name").eq(
        "id",
        managerOrgId,
      ).single();
      if (data) {
    ...
    console.warn(
      "[seed-contractor-data] Failed to create managers, continuing without them:",
      error,
    );
    managers = [];
  }
  ...
  if (managers.length > 0) {
    try {
      ...
      console.warn(
        "[seed-contractor-data] Failed to create relationships, continuing without them:",
        error,
      );
      relationships = [];
    }
  ...
  try {
    ...
    console.warn(
      "[seed-contractor-data] Failed to create projects, continuing without them:",
      error,
    );
    projects = [];
  }
  ...
  if (projects.length > 0) {
    try {
      const docResults = await createTestDocuments(
        contractorOrgId,
        projects[0]?.id,
      );
      ...
      console.warn(
        "[seed-contractor-data] Failed to create documents, continuing without them:",
        error,
      );
      documents = [];
    }
...
export async function cleanupContractorTestData() {
  ...
  for (const manager of seededData.managers) {
    ...
    const { data } = await core("organizations").select("id").eq(
      "id",
      manager.id,
    ).single();
    if (data) {
