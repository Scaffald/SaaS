...
test.describe("Subcontractor Comprehensive Page Audit", () => {
  ...
  test("Onboarding page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const heading = page.getByRole("heading", {
      name: /prequalification|onboarding/i,
    });
    ...
    const stepIndicators = page.locator(
      "text=/company info|licensing|insurance|safety/i",
    );
  ...
  test("Dashboard page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const uploadDocumentsButton = page.locator(
      'button:has-text("Upload Documents")',
    ).first();
    if (
      await uploadDocumentsButton.isVisible({ timeout: 5000 }).catch(() =>
        false
      )
    ) {
    ...
    const quickActionButtons = page.locator(
      'button:has-text("View"), button:has-text("See All"), a[href*="/"]',
    );
    ...
    const dashboardCards = page.locator(
      '[role="article"], .card, [data-testid*="card"]',
    );
  ...
  test("Dashboard - test widget interactions and quick actions", async ({ page, assertNoErrors }) => {
    ...
    const widgets = page.locator(
      '[role="article"], .card, [data-testid*="widget"], [data-testid*="card"]',
    );
    ...
    const quickActions = page.locator(
      'button:has-text("View"), button:has-text("Upload"), button:has-text("Schedule")',
    );
  ...
  test("Dashboard - test recent activity items", async ({ page, assertNoErrors }) => {
    ...
    const recentActivity = page.locator(
      "text=/recent activity|activity feed|recent/i",
    );
    if (await recentActivity.count() > 0) {
      // Find activity items
      const activityItems = page.locator(
        '[role="listitem"], .activity-item, [data-testid*="activity"]',
      );
  ...
  test("Dashboard - test refresh functionality", async ({ page, assertNoErrors }) => {
    ...
    const refreshButton = page.locator(
      'button:has-text("Refresh"), button[aria-label*="refresh" i], button[title*="refresh" i]',
    );
  ...
  test("GCs/Relationships page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    expect(currentUrl).toMatch(
      /\/subcontractor\/relationships|\/unauthorized|\/subcontractor\/dashboard/,
    );

    if (currentUrl.includes("/relationships")) {
      // Check for key elements
      const heading = page.getByRole("heading", {
        name: /(managers|gcs|relationships)/i,
      });
      ...
      if (buttonCount > 0) {
        ...
        for (let i = 0; i < Math.min(buttonCount, 5); i++) {
          ...
          if (
            text && !text.toLowerCase().includes("sign out") &&
    ...
    if (linkCount > 0) {
      ...
      if (tabCount > 0) {
        ...
        for (let i = 0; i < Math.min(tabCount, 5); i++) {
          ...
          if (
            tabText &&
            tabNames.some((name) => tabText.toLowerCase().includes(name))
          ) {
            try {
  ...
  test("Project Detail - test Comments section interactions", async ({ page, assertNoErrors }) => {
    ...
    if (await projectLinks.count() > 0) {
      ...
      const commentsTab = page.locator(
        '[role="tab"]:has-text("Comments"), button:has-text("Comments")',
      );
      if (await commentsTab.count() > 0) {
        ...
        const addCommentButton = page.locator(
          'button:has-text("Add"), button:has-text("Comment"), button:has-text("Post")',
        );
        ...
        const commentInput = page.locator(
          'textarea[placeholder*="comment" i], textarea[name*="comment"]',
        );
  ...
  test("Project Detail - test Compliance Issues section", async ({ page, assertNoErrors }) => {
    ...
    if (await projectLinks.count() > 0) {
      ...
      const complianceTab = page.locator(
        '[role="tab"]:has-text("Compliance"), button:has-text("Compliance")',
      );
      if (await complianceTab.count() > 0) {
        ...
        const issues = page.locator(
          '[role="listitem"], .issue-item, [data-testid*="issue"]',
        );
  ...
  test("Project Detail - test Participants section", async ({ page, assertNoErrors }) => {
    ...
    if (await projectLinks.count() > 0) {
      ...
      const participantsTab = page.locator(
        '[role="tab"]:has-text("Participants"), button:has-text("Participants")',
      );
      if (await participantsTab.count() > 0) {
        ...
        const participants = page.locator(
          '[role="listitem"], .participant-item, [data-testid*="participant"]',
        );
    ...
    if (await projectLinks.count() > 0) {
      ...
      if (await documentsTab.count() > 0) {
        ...
        const documents = page.locator(
          '[role="listitem"], .document-item, [data-testid*="document"]',
        );
  ...
  test("Documents page - test upload modal interactions", async ({ page, assertNoErrors }) => {
    ...
    if (await uploadButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await modal.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const formFields = page.locator(
          'input[type="text"], input[name*="name"], select',
        );
        const fieldCount = await formFields.count();
  ...
  test("Documents page - test document filtering and sorting", async ({ page, assertNoErrors }) => {
    ...
    const filterButton = page.locator(
      'button:has-text("Filter"), button:has-text("Sort"), select[name*="filter"], select[name*="sort"]',
    );
  ...
  test("Documents page - test document deletion (if available)", async ({ page, assertNoErrors }) => {
    ...
    const deleteButtons = page.locator(
      'button[aria-label*="delete" i], button:has-text("Delete"), button[title*="delete" i]',
    );
  ...
  test("Tasks page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const currentUrl = page.url();
    expect(currentUrl).toMatch(
      /\/subcontractor\/tasks|\/subcontractor\/dashboard|\/unauthorized/,
    );
    ...
    if (currentUrl.includes("/tasks")) {
      ...
      const taskItems = page.locator(
        '[role="article"], .task-item, [data-testid*="task"]',
      );
  ...
  test("Settings Company page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const taxIdInput = page.locator(
      'input[name*="tax"], input[name*="ein"], input[placeholder*="tax" i]',
    ).first();
  ...
  test("Settings Insurance page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const providerInput = page.locator(
      'input[name*="provider"], input[name*="insurance"], input[placeholder*="provider" i]',
    ).first();
    ...
    const policyNumberInput = page.locator(
    ...
    const coverageInput = page.locator(
      'input[name*="coverage"], input[type="number"], input[placeholder*="coverage" i]',
    ).first();
  ...
  test("Settings Notifications page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const labels = page.locator(
      'label, [role="label"], text=/email|push|sms|document|project/i',
    );
  ...
  test("Settings Documents page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    if (await uploadButton.isVisible({ timeout: 3000 })) {
      ...
      if (await modal.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        // Close modal
        const closeButton = page.locator(
          'button:has-text("Close"), button:has-text("Cancel"), button[aria-label*="close" i], button:has-text("×")',
        ).first();
        if (await closeButton.isVisible({ timeout: 2000 })) {
          await closeButton.click();
          await page.waitForTimeout(500);
    ...
    const documentList = page.locator(
      '[role="list"], .document-list, [data-testid*="document"]',
    );
  ...
  test("Help page - check for errors and validate functionality", async ({ page, assertNoErrors }) => {
    ...
    const currentUrl = page.url();
    expect(currentUrl).toMatch(
      /\/subcontractor\/help|\/subcontractor\/dashboard|\/unauthorized/,
    );
  ...
  test("Navigation via sidebar - test all links", async ({ page, assertNoErrors }) => {
    ...
    const sidebarLinks = [
      ...
      {
        text: /gcs|managers|relationships/i,
        url: /\/subcontractor\/relationships/,
      },
  ...
  test("Header actions - test notifications, settings, sign out buttons", async ({ page, assertNoErrors }) => {
    ...
    if (
      await notificationsButton.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
  ...
  test("Settings - test form submission and success messages", async ({ page, assertNoErrors }) => {
    ...
    if (await firstNameInput.isVisible({ timeout: 3000 })) {
      ...
      if (
        await saveButton.isVisible({ timeout: 3000 }) &&
        ...
        const successMessage = page.locator(
          "text=/saved|success|updated|successfully/i",
        );
  ...
  test("Projects - filter by status", async ({ page, assertNoErrors }) => {
    ...
    // Look for status filter
    const statusFilter = page.locator(
      'select[name="status"], button:has-text("Filter"), button:has-text("All")',
    ).first();
    if (await statusFilter.isVisible({ timeout: 5000 })) {
      try {
        await statusFilter.selectOption("active");
  ...
  test("Projects - navigate to project detail", async ({ page, assertNoErrors }) => {
    ...
    const projectLink = page.locator(
      'a[href*="/projects/"], [data-testid="project-card"], .project-card',
    ).first();
  ...
  test("Documents - filter by type", async ({ page, assertNoErrors }) => {
    ...
    const typeFilter = page.locator(
      'select[name="type"], button:has-text("Filter"), button:has-text("All Types")',
    ).first();
  ...
  test("Notifications - mark as read", async ({ page, assertNoErrors }) => {
    ...
    const notification = page.locator(
      '[data-testid="notification-item"], .notification-item, li',
    ).first();
  ...
  test("Notifications - filter by type", async ({ page, assertNoErrors }) => {
    ...
    // Look for type filter
    const typeFilter = page.locator(
      'select[name="type"], button:has-text("Filter"), button:has-text("All")',
    ).first();
    if (await typeFilter.isVisible({ timeout: 5000 })) {
      try {
