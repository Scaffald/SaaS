 ...
 * Admin compliance requirement management UI E2E.
 *
...
import { loginAs, TEST_USER_IDS, TEST_USERS } from "../utils/auth";
...
async function setupComplianceMocks(page: Page) {
  ...
  await page.route(
    "**/api/trpc/compliance.requirements.list*",
    async (route: Route) => {
  ...
  await page.route(
    "**/api/trpc/compliance.requirements.get*",
    async (route: Route) => {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    "**/api/trpc/compliance.requirements.create*",
    async (route: Route) => {
      const method = route.request().method();
      if (method === "POST") {
  ...
  await page.route(
    "**/api/trpc/compliance.dependencies.list*",
    async (route: Route) => {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    "**/api/trpc/compliance.requirements.getVersionHistory*",
    async (route: Route) => {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    "**/api/trpc/compliance.bulk.importPreview*",
    async (route: Route) => {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    "**/api/trpc/compliance.bulk.export*",
    async (route: Route) => {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
...
test.describe("Admin Compliance Management", () => {
  ...
  test.describe("Navigation", () => {
    ...
    test.skip("should display compliance requirements list", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: /compliance requirements/i }),
      ).toBeVisible();
  ...
  test.describe("Requirements Management", () => {
    ...
    test.skip("should edit an existing requirement", async ({ page }) => {
      ...
      await expect(page).toHaveURL(
        /\/admin\/compliance\/requirements\/.*\/edit/,
      );
  ...
  test.describe("Dependency Management", () => {
    test.skip("should display dependency visualizer", async ({ page }) => {
      ...
      await page.goto(
        "/admin/compliance/requirements/" + TEST_REQUIREMENT_IDS.umbrella,
      );
    ...
    test.skip("should add a new dependency", async ({ page }) => {
      ...
      await page.getByLabel(/requirement/i).selectOption(
  ...
  test.describe("Authorization", () => {
    ...
    test.skip("should hide admin features for non-admin users", async ({ page }) => {
      ...
      await expect(page.getByRole("button", { name: /create/i })).not
        .toBeVisible();
      await expect(page.getByRole("button", { name: /import/i })).not
        .toBeVisible();
    });
  });
});
