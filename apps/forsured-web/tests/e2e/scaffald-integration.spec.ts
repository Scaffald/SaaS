...
test.describe.skip("Scaffald Integration Flow", () => {
  ...
  test("User can initiate company connection flow", async ({ page }) => {
    ...
    await expect(scaffaldIntegrationRow.locator('text="disconnected"'))
      .toBeVisible();

    // Click Connect button
    const connectButton = scaffaldIntegrationRow.locator(
    ...
    await expect(scaffaldIntegrationRow.locator('text="connected"'))
      .toBeVisible();
    await expect(scaffaldIntegrationRow.locator('input[type="checkbox"]'))
      .toBeChecked();
  });
  ...
  test("User can trigger manual sync and view sync status", async ({ page }) => {
    ...
    await expect(page.getByText(/Company:.*Synced/).first()).toBeVisible({
      timeout: 5000,
    });
    await expect(page.getByText(/Projects:.*Synced/).first()).toBeVisible({
      timeout: 5000,
    });
  ...
  test("User can disconnect company and verify status", async ({ page }) => {
    ...
    const disconnectButton = scaffaldIntegrationRow.locator(
    ...
    await expect(scaffaldIntegrationRow.locator('text="disconnected"'))
      .toBeVisible();
    await expect(scaffaldIntegrationRow.locator('input[type="checkbox"]')).not
      .toBeChecked();
  });
...
test.describe.skip("Scaffald Document Upload Integration", () => {
  ...
  test("Document upload uses Scaffald when integration is enabled", async ({ page }) => {
    ...
    const pageContent = await page.content();
    const hasDocumentsUI = pageContent.toLowerCase().includes("document") ||
    ...
    const dropzone = page.locator(
      '[class*="dropzone"], [data-testid="dropzone"]',
    ).first();
  ...
  test("Batch upload displays progress for multiple files", async ({ page }) => {
    ...
    const batchUploadButton = page.locator(
      'button:has-text("Batch Upload"), button:has-text("Upload Multiple")',
    ).first();

    if (
      await batchUploadButton.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
  ...
  test("Document list shows storage backend information", async ({ page }) => {
    ...
    const documentList = page.locator(
      '[data-testid="document-list"], table, [class*="document-list"]',
    ).first();

    if (await documentList.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Look for storage backend indicator in document entries
      const storageIndicator = page.locator(
        '[data-testid="storage-backend"], [class*="storage"], span:has-text("Supabase"), span:has-text("Dropbox"), span:has-text("Google")',
      ).first();

      // Storage backend info may be shown in document details
      const hasStorageInfo = await storageIndicator.isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test("Performance metrics are tracked for uploads", async ({ page }) => {
    ...
    const performanceSection = page.locator(
      '[data-testid="performance-metrics"], h2:has-text("Performance"), [class*="metrics"]',
    ).first();

    if (
      await performanceSection.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
...
test.describe.skip("Contractor Scaffald Document Upload", () => {
  ...
  test("Contractor can upload COI documents via Scaffald", async ({ page }) => {
    ...
    const coiSection = page.locator(
      '[data-testid="coi-upload"], h2:has-text("Insurance"), h3:has-text("COI")',
    ).first();
    const uploadButton = page.locator(
      'button:has-text("Upload COI"), button:has-text("Upload Insurance")',
    ).first();
    ...
    const pageContent = await page.content();
    expect(hasUploadUI || pageContent.toLowerCase().includes("upload"))
      .toBeTruthy();
  });
  ...
  test("Contractor can view upload progress", async ({ page }) => {
    ...
    const progressBar = page.locator(
      '[role="progressbar"], [class*="progress"], [data-testid="upload-progress"]',
    ).first();
    const uploadingText = page.locator('text="Uploading", text="Processing"')
      .first();
