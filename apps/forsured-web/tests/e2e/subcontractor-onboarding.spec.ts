...
test.describe('Subcontractor Onboarding - Comprehensive', () => {
  ...
  test('should display onboarding page on first visit', async ({ page, assertNoErrors }) => {
    ...
    const hasOnboardingContent = pageContent.toLowerCase().includes('onboarding') ||
  ...
  test('should show first step (Company) with form fields', async ({ page, assertNoErrors }) => {
    ...
    const companyFields = page.locator('input[name*="company"], input[name*="name"], input[type="text"]');
    const fieldCount = await companyFields.count();
    
    // Should have at least one form field or step indicator
    const stepIndicator = page.locator('text=/step 1|step 1 of|company info|onboarding/i');
  ...
  test('should navigate to Insurance Step and show form fields', async ({ page, assertNoErrors }) => {
    ...
    const companyNameInput = page.locator('input[name*="company"], input[name*="name"]').first();
    if (await companyNameInput.isVisible({ timeout: 5000 })) {
  ...
  test('should navigate to COI Upload Step (optional)', async ({ page, assertNoErrors }) => {
    ...
    // Step 1: Company
    const companyInput = page.locator('input[name*="company"], input[name*="name"]').first();
    ...
    const pageContent = await page.content();
    if (pageContent.toLowerCase().includes('insurance') || pageContent.toLowerCase().includes('step 2')) {
      ...
      if (await next2.isVisible({ timeout: 5000 }) && await next2.isEnabled()) {
        ...
        const hasUploadContent = uploadContent.toLowerCase().includes('upload') ||
  ...
  test('should allow skipping COI Upload Step', async ({ page, assertNoErrors }) => {
    ...
    const skipButton = page.locator('button:has-text("Skip"), button:has-text("Skip this step")').first();
  ...
  test('should show file upload interface on COI Upload Step', async ({ page, assertNoErrors }) => {
    ...
    const hasUpload = await uploadInput.count() > 0 || await uploadButton.count() > 0;
  ...
  test('should complete onboarding and redirect to dashboard', async ({ page, assertNoErrors }) => {
    ...
    if (await completeButton.isVisible({ timeout: 5000 })) {
      ...
      const isComplete = currentUrl.includes('/dashboard') ||
        currentUrl.includes('/onboarding') && (await page.content()).toLowerCase().includes('success');
  ...
  test('should validate Insurance Step form fields', async ({ page, assertNoErrors }) => {
    ...
    const licenseInput = page.locator('input[name*="license"], input[placeholder*="license" i]').first();
    if (await licenseInput.isVisible({ timeout: 5000 })) {
      ...
      if (await nextButton2.isVisible({ timeout: 3000 }) && await nextButton2.isEnabled()) {
    ...
    const insuranceCarrierInput = page.locator('input[placeholder*="carrier" i], input[name*="carrier"]').first();
    if (await insuranceCarrierInput.isVisible({ timeout: 5000 })) {
      ...
      const policyInput = page.locator('input[placeholder*="policy" i], input[name*="policy"]').first();
  ...
  test('should test Success Step completion and redirect', async ({ page, assertNoErrors }) => {
    ...
    const successText = page.locator('text=/complete|success|all set|finished/i');
    const completeButton = page.locator('button:has-text("Go to Dashboard"), button:has-text("Complete")').first();
    
    if (await successText.count() > 0 || await completeButton.isVisible({ timeout: 5000 })) {
  ...
  test('should check console errors on each step during navigation', async ({ page, assertNoErrors }) => {
    ...
    const nextButton2 = page.locator('button:has-text("Next"), button:has-text("Continue")').first();
    if (await nextButton2.isVisible({ timeout: 5000 }) && await nextButton2.isEnabled()) {
      await nextButton2.click();
      await page.waitForLoadState('networkidle');
