...
// CCPA compliance - privacy dashboard E2E
...
test.describe("Privacy Dashboard - All User Types", () => {
  ...
  test.describe("Contractor User - Privacy Dashboard Access", () => {
    ...
    test("Contractor can see CCPA rights information", async ({ page, setupAuthAs }) => {
      ...
      const rightsSection = page.getByRole("heading", {
...
test.describe.skip("Privacy Dashboard - Data Categories", () => {
  ...
  test("should display at least one data category card", async ({ page }) => {
    ...
    const identifiersCategory = page.locator(
      'h3:has-text("Personal Identifiers")',
    );
    const financialCategory = page.locator(
      'h3:has-text("Financial Information")',
    );

    // At least one should be visible
    const identifiersVisible = await identifiersCategory.isVisible().catch(() =>
      false
    );
    const financialVisible = await financialCategory.isVisible().catch(() =>
      false
    );
...
test.describe("Privacy Dashboard - Privacy Rights", () => {
  ...
  test("should display Right to Opt-Out", async ({ page }) => {
    ...
    const right = page.getByRole("heading", { name: "Right to Opt-Out" })
      .first();
    await expect(right).toBeVisible();
  });
