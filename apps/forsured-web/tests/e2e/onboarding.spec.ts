...
test.describe("Onboarding Flows", () => {
  test("GC onboarding form fields E2E", async ({ page, loginAs }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Welcome, General Contractor" }),
    ).toBeVisible({ timeout: 10000 });

    // Verify all form fields are present
    await expect(page.getByPlaceholder("Enter your company name"))
      .toBeVisible();
    ...
    // Fill text fields
    await page.getByPlaceholder("Enter your company name").fill(
      "Test GC Company",
    );
  ...
  test("Contractor onboarding flow E2E - Step 1 Company Info", async ({ page, loginAs }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /subcontractor prequalification|complete your profile/i,
      }),
    ).toBeVisible({ timeout: 10000 });

    // Step 1 should show "Company Information" section heading
    await expect(page.getByRole("heading", { name: "Company Information" }))
      .toBeVisible({ timeout: 10000 });

    // Fill required fields using actual placeholders from SubcontractorPrequalificationWizard
    await page.getByPlaceholder("Legal company name").fill(
      "Test Contractor Co",
    );
    await page.getByPlaceholder("XX-XXXXXXX").fill("12-3456789");
    await page.getByPlaceholder("Street address, city, state, zip").first()
      .fill("789 Tool Rd, Dallas, TX 75201");
    ...
    await expect(page.getByRole("heading", { name: /licensing/i })).toBeVisible(
      { timeout: 10000 },
    );
  });
  ...
  test("Broker onboarding flow E2E", async ({ page, loginAs }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Welcome, Insurance Broker" }),
    ).toBeVisible({ timeout: 10000 });

    // Brokerage Details section
    await expect(page.getByRole("heading", { name: "Brokerage Details" }))
      .toBeVisible({ timeout: 10000 });

    // Fill brokerage fields
    await page.getByPlaceholder("Enter your brokerage name").fill(
      "Test Brokerage Inc",
    );
    ...
    await expect(page.getByRole("heading", { name: "Administrator Account" }))
      .toBeVisible();
  ...
  test("Broker onboarding team invitation", async ({ page, loginAs }) => {
    ...
    await expect(page.getByRole("heading", { name: "Invite Team Members" }))
      .toBeVisible({ timeout: 10000 });
