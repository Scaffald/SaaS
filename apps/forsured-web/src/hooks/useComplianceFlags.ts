...
export function useComplianceFlags(initialFilters?: ComplianceFlagsFilter) {
  ...
  const fetchFlags = useCallback(async (filters?: ComplianceFlagsFilter) => {
    try {
      ...
      if (useMockData) {
        ...
        if (filters?.entity_type) mockFilters.entity_type = filters.entity_type;
        if (filters?.entity_id) mockFilters.entity_id = filters.entity_id;
        if (filters?.status) mockFilters.status = filters.status;
        if (filters?.project_id) mockFilters.project_id = filters.project_id;
        if (filters?.subcontractor_id) mockFilters.subcontractor_id = filters.subcontractor_id;
        if (filters?.severity) mockFilters.severity = filters.severity;

        const data = await MockDatabase.query<ComplianceFlag>(
          'compliance_flags',
          mockFilters,
   ...
   */
  const getFlagsByEntity = useCallback(async (
  ...
  const createFlag = useCallback(async (
    request: CreateComplianceFlagRequest
  ): Promise<ComplianceFlag> => {
    ...
    try {
      ...
      if (useMockData) {
        const data = await MockDatabase.insert<ComplianceFlag>('compliance_flags', flagData);
  ...
  }, [useMockData, fetchFlags, initialFilters]);
  ...
  const updateFlag = useCallback(async (
    flagId: string,
    updates: UpdateComplianceFlagRequest
  ): Promise<ComplianceFlag> => {
    try {
      const updateData = {
        ...updates,
      ...
      if (useMockData) {
        const data = await MockDatabase.update<ComplianceFlag>(
          'compliance_flags',
          flagId,
          updateData
        );
  ...
  const deleteFlag = useCallback(async (flagId: string): Promise<void> => {
    try {
      if (useMockData) {
        await MockDatabase.delete('compliance_flags', flagId);
        await fetchFlags(initialFilters);
      } else {
        const client = supabaseServiceRole || null;
   ...
   */
  const getActiveFlagsCount = useCallback(async (
    entityType: FlaggableEntityType,
    entityId: string
  ): Promise<number> => {
    try {
