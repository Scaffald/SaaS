...
import { ComplianceIssue, SeverityLevel } from '../types';
...
export function useComplianceIssues(options: UseComplianceIssuesOptions = {}) {
  ...
  const fetchIssues = async () => {
    try {
      ...
      if (useMockData) {
        ...
        const data = await MockDatabase.query<ComplianceIssue>(
          'compliance_issues',
          filters,
  ...
  const createIssue = async (
    issue: Omit<ComplianceIssue, 'id' | 'created_at' | 'updated_at'>
  ) => {
    try {
      if (useMockData) {
        const data = await MockDatabase.insert<ComplianceIssue>(
          'compliance_issues',
          issue
        );
        await fetchIssues();
  ...
  const updateIssue = async (id: string, updates: Partial<ComplianceIssue>) => {
    try {
      if (useMockData) {
        const data = await MockDatabase.update<ComplianceIssue>(
          'compliance_issues',
          id,
          updates
        );
  ...
  const deleteIssue = async (id: string) => {
    try {
      if (useMockData) {
        await MockDatabase.delete('compliance_issues', id);
        await fetchIssues();
      } else {
        // Use real Supabase
