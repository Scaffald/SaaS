...
import type { Task, PolicyData, ComplianceRecord, Project, BrokerClient, TaskSeverity } from '../../../types';
import {
...
class DashboardService {
  ...
  async getOverview(): Promise<DashboardOverview> {
    try {
      ...
      const complianceRecords = await MockDatabase.query<ComplianceRecord>('compliance_records');
      const clients = await MockDatabase.query<BrokerClient>('broker_clients');
  ...
  async getSubcontractorScores(filters?: DashboardFilters): Promise<SubcontractorScore[]> {
    try {
      const clients = await MockDatabase.query<BrokerClient>('broker_clients');
      const complianceRecords = await MockDatabase.query<ComplianceRecord>('compliance_records');
      const tasks = await MockDatabase.query<Task>('tasks');
      const policies = await MockDatabase.query<PolicyData>('policies');
      const projects = await MockDatabase.query<Project>('projects');
      ...
      return {
        ...
        contributing_factors: [
          ...
          },
          {
            ...
            impact: changePercentage > 0 ? 'positive' : changePercentage < 0 ? 'negative' : 'neutral',
  ...
  async getSubcontractorDetail(subcontractorId: string): Promise<SubcontractorDetail> {
    try {
      ...
      const compliance = (await MockDatabase.query<ComplianceRecord>('compliance_records')).find(
        (c) => c.client_id === subcontractorId
      );
      const policies = (await MockDatabase.query<PolicyData>('policies')).filter(
        (p) => p.client_id === subcontractorId
      );
      const tasks = (await MockDatabase.query<Task>('tasks')).filter(
        (t) => t.client_id === subcontractorId && t.status !== 'completed' && t.status !== 'cancelled'
      );
  ...
  async getRiskDistribution(): Promise<RiskDistribution> {
    try {
      ...
      const complianceRate =
        totalClients > 0
          ? Math.round(((totalClients - clientsWithGaps) / totalClients) * 100)
          : 100;
  ...
  private generateCorrelationExplanation(
    ...
    if (severityCounts.high > 0) {
      explanations.push(
        `${severityCounts.high} high-severity tasks represent elevated risk.`
      );
    }
