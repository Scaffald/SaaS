...
// Broker invitation system and rate limiting
//
  ...
  if (error) {
    ...
    if (error.code === "PGRST116") {
      ...
      return {
        ...
        error: {
          type: "INVALID_CODE",
          message:
            "The invitation code you entered is not valid. Please check and try again.",
        },
      };
    }
    ...
    return {
      ...
      error: {
        type: "UNKNOWN",
        message:
          "An error occurred while validating the invitation code. Please try again.",
      },
    };
  }
  ...
  if (data.use_count >= data.max_uses) {
    ...
    return {
      ...
      error: {
        ...
        message:
          "This invitation code has already been used the maximum number of times.",
      },
    };
  }
  ...
  if (data.email && email && data.email.toLowerCase() !== email.toLowerCase()) {
    ...
    return {
      ...
      error: {
        type: "EMAIL_MISMATCH",
        message:
          "This invitation code is assigned to a different email address.",
      },
    };
  }
...
async function recordFailedAttempt(invitation: Invitation): Promise<void> {
  ...
  if (newFailedAttempts >= RATE_LIMIT_CONFIG.maxFailedAttempts) {
    ...
    lockUntil.setMinutes(
      lockUntil.getMinutes() + RATE_LIMIT_CONFIG.lockDurationMinutes,
    );
  ...
  const { error } = await forsured(TABLE_NAME).update(updates).eq(
    "id",
    invitation.id,
  );
...
export async function createInvitation(options: {
  ...
  const { email, maxUses = 1, expiresInDays = 30, createdBy, brokerageName } =
    options;
