 ...
 * Mock OCR and document parsing for ACORD 25 forms
 *
...
export class DocumentProcessor {
  ...
  async processDocument(input: OCRInput): Promise<ProcessingResult> {
    ...
    try {
      ...
      console.log(
        `[DocumentProcessor] Starting OCR for document ${input.documentId}`,
      );
      const ocrResult = await mockOCRService.extractText(input);

      // Validate ACORD 25 format
      const formatValidation = mockOCRService.validateACORD25Format(
        ocrResult.rawText,
      );
      ...
      console.log(
        `[DocumentProcessor] Parsing ACORD 25 for document ${input.documentId}`,
      );
   ...
   */
  private async createPolicyRecords(
    extraction: ACORD25Extraction,
  ): Promise<string[]> {
    ...
    for (const coverage of extraction.coverage_types) {
      ...
      try {
        const { data: policy, error } = await supabase.schema("forsured").from(
          "policies",
        )
          .insert(policyData)
          .select()
        ...
        console.log(
          `[DocumentProcessor] Created policy ${policy.id} for ${coverage.type}`,
        );
      } catch (error) {
        console.error(
          `[DocumentProcessor] Error creating policy for ${coverage.type}:`,
          error,
        );
      }
    }
  ...
  private async resolveClientId(_documentId: string): Promise<string | null> {
    ...
    const { data: clients = [] } = await supabase.schema("forsured").from(
      "clients",
    ).select("id").limit(1);
  ...
  private buildPolicyNotes(
    ...
    if (extraction.overall_confidence < 80) {
      notes.push(
        "⚠️ Manual review recommended due to low confidence extraction",
      );
    }
    ...
    if (extraction.errors.length > 0) {
      notes.push(
        `Extraction errors: ${
          extraction.errors.map((e) => e.message).join(", ")
        }`,
      );
    }
  ...
  async updateDocumentStatus(
    ...
    try {
      ...
      console.error(
        "[DocumentProcessor] Error updating document status:",
        error,
      );
    }
  }
