 ...
 *
 * Referral tracking for invitations
 *
...
export function captureReferral(): void {
  ...
  try {
    document.cookie =
      `${COOKIE_NAME}=${refCode.toUpperCase()}; max-age=${COOKIE_MAX_AGE}; path=/; secure; samesite=lax`;
  } catch (e) {
...
>>>>>>> 264530c73bf14a52195cd0553c9391f21eeccac1
    ...
    if (invError || !invitation) {
      console.warn(
        "[referral-tracking] Invitation not found for code:",
        referralData.code,
      );
      clearReferral();
      return { success: false };
...
>>>>>>> 264530c73bf14a52195cd0553c9391f21eeccac1
    ...
    if (updateError) {
      console.warn(
        "[referral-tracking] Failed to link invitation to user:",
        updateError,
      );
    }
