 ...
 */
async function getOrgRequirements(
  organizationId: string
): Promise<CoverageLimitRequirement[]> {
  return MockDatabase.query<CoverageLimitRequirement>('coverage_limit_requirements', {
...
export async function validateOrgLevelCompliance(
  ...
  const requiredOrgChecks = orgLevelChecks.filter(
    (check) => requiredOrgRequirements.some((req) => req.id === check.requirement_id)
  );
...
export async function validateProjectLevelCompliance(
  ...
  // Determine overall compliance (only required requirements affect this)
  const requiredProjectChecks = projectLevelChecks.filter(
    (check) => requiredProjectRequirements.some((req) => req.id === check.requirement_id)
  );
...
export function calculateRequiredComplianceScore(
  ...
  const requiredOrgIds = new Set(
    orgRequirements.filter((r) => r.required).map((r) => r.id)
  );
  const requiredProjectIds = new Set(
    projectRequirements.filter((r) => r.required).map((r) => r.id)
  );
