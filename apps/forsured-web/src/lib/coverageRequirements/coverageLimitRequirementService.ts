/**
 * Coverage limit requirements API service
 * API for coverage limit requirements management
 *
 ...
 */
export function canManageProjectLevel(
  user: UserContext,
  projectId: string,
): boolean {
 ...
 */
export function validateCoverageType(
  coverageType: string,
): ValidationError | undefined {
  if (!VALID_COVERAGE_TYPES.includes(coverageType as CoverageLimitType)) {
    return {
      field: "coverage_type",
      message: `Invalid coverage type. Must be one of: ${
        VALID_COVERAGE_TYPES.join(", ")
      }`,
    };
  }
  return undefined;
 ...
 */
export function validateMinimumLimit(
  limit: number,
): ValidationError | undefined {
...
export async function getCoverageLimitRequirements(
  ...
  try {
    ...
    if (projectId) {
      ...
      const { data: orgRequirements } = await supabase.schema("forsured").from(
      ...
      const { data: projectRequirements } = await supabase.schema("forsured")
        .from("coverage_limit_requirements")
        .select("*")
        .eq("organization_id", organizationId)
      ...
      const { data } = await supabase.schema("forsured").from(
        "coverage_limit_requirements",
      )
        .select("*")
        .eq("organization_id", organizationId)
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to fetch coverage requirements",
      statusCode: 500,
    };
  }
}
...
export async function getCoverageLimitRequirementById(
  ...
  try {
    const { data: requirement } = await supabase.schema("forsured").from(
      "coverage_limit_requirements",
    )
      .select("*")
      .eq("id", id)
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to fetch coverage requirement",
      statusCode: 500,
    };
  }
}
...
export async function createCoverageLimitRequirement(
  ...
  if (input.level === "org") {
    if (!canManageOrgLevel(user)) {
      return {
        success: false,
        error:
          "Unauthorized. Admin access required for org-level requirements.",
        statusCode: 403,
      };
    }
  ...
  try {
    const { data: requirement, error } = await supabase.schema("forsured").from(
      "coverage_limit_requirements",
    )
      .insert({
        name: input.name,
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to create coverage requirement",
      statusCode: 500,
    };
  }
}
...
export async function updateCoverageLimitRequirement(
  ...
  const { data: existing } = await supabase.schema("forsured").from(
    "coverage_limit_requirements",
  )
    .select("*")
    .eq("id", id)
  ...
  try {
    const { data: updated, error } = await supabase.schema("forsured").from(
      "coverage_limit_requirements",
    )
      .update(input)
      .eq("id", id)
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to update coverage requirement",
      statusCode: 500,
    };
  }
}
...
export async function deleteCoverageLimitRequirement(
  ...
  if (existing.level === "org") {
    if (!canManageOrgLevel(user)) {
      return {
        success: false,
        error:
          "Unauthorized. Admin access required to delete org-level requirements.",
        statusCode: 403,
      };
    }
  ...
  try {
    const { error } = await supabase.schema("forsured").from(
      "coverage_limit_requirements",
    ).delete().eq("id", id);
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to delete coverage requirement",
      statusCode: 500,
    };
  }
}
...
export async function getOrgLevelRequirements(
  ...
  try {
    const { data: requirements } = await supabase.schema("forsured").from(
      "coverage_limit_requirements",
    )
      .select("*")
      .eq("organization_id", organizationId)
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to fetch org-level requirements",
      statusCode: 500,
    };
  }
}
...
export async function getProjectLevelRequirements(
  ...
  try {
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : "Failed to fetch project-level requirements",
      statusCode: 500,
    };
  }
}
