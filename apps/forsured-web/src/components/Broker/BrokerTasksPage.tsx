...
} from '../../lib/tasks/taskAssignmentTypeService'
...
export default function BrokerTasksPage() {
    ...
    const getFilteredTasks = (excludeFilter: 'priority' | 'status' | 'client' | 'project') => {
      return tasks.filter((task) => {
        ...
        const matchesPriority =
          excludeFilter === 'priority' ||
          selectedPriority === 'all' ||
          task.priority === selectedPriority
        const matchesStatus =
          excludeFilter === 'status' || selectedStatus === 'all' || task.status === selectedStatus
        const matchesClient =
          excludeFilter === 'client' ||
          selectedClient === 'all' ||
          task.client_id === selectedClient
        const matchesProject =
          excludeFilter === 'project' ||
          selectedProject === 'all' ||
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === 'due_date') {
        ...
        comparison =
          (statusOrder[a.status as TaskStatus] ?? 1) - (statusOrder[b.status as TaskStatus] ?? 1)
      }
  ...
  const formatDueDate = (dueAt: string | undefined) => {
    ...
    if (diffDays === 1) return { text: 'Due tomorrow', color: 'var(--color-yellow-10)' }
    if (diffDays <= 3) return { text: `Due in ${diffDays}d`, color: 'var(--color-yellow-10)' }
    ...
    <Stack gap={24}>
      ...
      <Row alignItems="center" style={{ borderBottom: '1px solid var(--color-border)' }}>
        ...
        >
          <Inbox size={18} />
          <Text>Inbox</Text>
          <span style={tabBadgeStyle(selectedView === 'inbox')}>{viewCounts.inbox}</span>
        </button>
        <button
        ...
        >
          <List size={18} />
          <Text>All Tasks</Text>
          <span style={tabBadgeStyle(selectedView === 'all')}>{viewCounts.all}</span>
        </button>
      </Row>
                ...
                <Stack>
                  ...
                  >
                    ...
                    <option value="in_progress">
                      In Progress ({filterCounts.status.in_progress})
                    </option>
      ...
      <Stack gap={12}>
        {filteredAndSortedTasks.map((task) => {
          ...
          const projectName = projects.find((p) => p.id === task.project_id)?.name ?? 'Unknown'
                      ...
                      {task.sub_company_name && (
                        <>
                          ...
                          <Row
                            alignItems="center"
                            gap={4}
                            style={{ color: 'var(--color-purple-10)' }}
                          >
                            <HardHat size={14} />
                            <Text size="xs" style={{ color: 'var(--color-purple-10)' }}>
                              {task.sub_company_name}
                            </Text>
                          </Row>
                        </>
      ...
      {filteredAndSortedTasks.length === 0 && (
        ...
        >
          <Stack alignItems="center">
            ...
            <Text muted>Try adjusting your filters or search criteria</Text>
          </Stack>
        </Card>
      )}
