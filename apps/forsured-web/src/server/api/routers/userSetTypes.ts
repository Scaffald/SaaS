 ...
 * Admin CRUD and public API for user set types and lexicon.
 *
 * Provides:
...
const userSetTypeBaseSchema = z.object({
  ...
  managerLabelSingular: z.string().min(
    1,
    "Manager label (singular) is required",
  ),
  managerLabelPlural: z.string().min(1, "Manager label (plural) is required"),
  contractorLabelSingular: z.string().min(
    1,
    "Contractor label (singular) is required",
  ),
  contractorLabelPlural: z.string().min(
    1,
    "Contractor label (plural) is required",
  ),
...
export const userSetTypesRouter = createTRPCRouter({
  ...
  listActive: publicProcedure.query(async () => {
    const { data, error } = await forsured("user_set_types")
      .select(
        "id, name, slug, manager_label_singular, manager_label_plural, contractor_label_singular, contractor_label_plural, description",
      )
      .eq("is_active", true)
  ...
  getUserLexicon: protectedProcedure.query(async ({ ctx }) => {
    ...
    const { data: userProfile, error: profileError } = await forsured(
      "user_profiles",
    )
    ...
    const { data: userSetType, error: ustError } = await forsured(
      "user_set_types",
    )
      .select("*")
   ...
   * Broker organization-specific lexicon context switching
  ...
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { data: userCounts, error: countError } = await forsured(
        "user_profiles",
      )
   ...
   */
  create: protectedProcedure.input(createUserSetTypeSchema).mutation(
    async ({ ctx, input }) => {
      if (!ctx.session?.id) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
   ...
   */
  update: protectedProcedure.input(updateUserSetTypeSchema).mutation(
    async ({ ctx, input }) => {
      if (!ctx.session?.id) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (users && users.length > 0) {
        throw new TRPCError({
          ...
          message:
            "Cannot delete user set type with active users. Please reassign users first.",
        });
      }

      const { error } = await forsured("user_set_types").delete().eq(
        "id",
        input.id,
      );
   ...
   */
  updateLexicon: protectedProcedure.input(updateLexiconSchema).mutation(
    async ({ ctx, input }) => {
      if (!ctx.session?.id) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
      ...
      await forsured("user_set_type_lexicon").delete().eq(
        "user_set_type_id",
        input.userSetTypeId,
      );

      if (input.entries.length > 0) {
        const { error: insertError } = await forsured("user_set_type_lexicon")
          .insert(
  ...
  upsertLexiconEntry: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Get current lexicon
      const { data: current, error: getError } = await forsured(
        "user_set_types",
      )
        .select("lexicon")
        .eq("id", input.userSetTypeId)
