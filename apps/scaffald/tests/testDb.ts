...
const TEST_SUPABASE_URL = process.env.TEST_SUPABASE_URL || process.env.EXPO_PUBLIC_SUPABASE_URL || 'http://127.0.0.1:54321';
const TEST_SUPABASE_KEY = process.env.TEST_SUPABASE_SERVICE_KEY || process.env.EXPO_PUBLIC_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || '';
 ...
 */
export async function getTestTeamId(organizationId?: string): Promise<string | null> {
  let query = testSupabase
    .schema('core' as never)
    .from('teams')
    .select('id');
