import {
  fetchRows,
  renderUrlSet,
  SITE_ORIGIN,
  SITEMAP_HEADERS,
  type SitemapEntry,
} from '../utils/sitemap'

type UserRow = { slug: string | null; updated_at: string | null }

const LIMIT = 40000

export async function GET(): Promise<Response> {
  const rows = await fetchRows<UserRow>(
    'users',
    `select=slug,updated_at&slug=not.is.null&order=updated_at.desc&limit=${LIMIT}`
  )

  const entries: SitemapEntry[] = rows
    .filter((row): row is { slug: string; updated_at: string | null } => Boolean(row.slug))
    .map((row) => ({
      loc: `${SITE_ORIGIN}/users/${encodeURIComponent(row.slug)}`,
      lastmod: row.updated_at ? row.updated_at.slice(0, 10) : undefined,
      changefreq: 'weekly',
      priority: 0.6,
    }))

  return new Response(renderUrlSet(entries), { headers: SITEMAP_HEADERS })
}
