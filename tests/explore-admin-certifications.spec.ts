...
test.describe('EXPLORATION • Admin /dashboard/profile/certifications', () => {
  ...
  test('explore certifications management interface', async ({ page }: { page: Page }) => {
    ...
    await page.screenshot({ path: '.playwright-mcp/admin-certifications-01-initial.png', fullPage: true })
    ...
    for (const selector of addButtonSelectors) {
      ...
      if (await btn.isVisible().catch(() => false)) {
        ...
        await page.screenshot({ path: '.playwright-mcp/admin-certifications-02-before-add.png', fullPage: true })
        ...
        await page.screenshot({ path: '.playwright-mcp/admin-certifications-03-after-add-click.png', fullPage: true })
        console.log('📸 Screenshot saved: admin-certifications-03-after-add-click.png')
        
        // Check for modal/form
        const modalVisible = await page.locator('[role="dialog"], .modal, [data-testid="modal"]').isVisible().catch(() => false)
        ...
        if (modalVisible) {
          ...
          const modalInputs = await page.locator('[role="dialog"] input, [role="dialog"] textarea, [role="dialog"] select, .modal input, .modal textarea, .modal select').evaluateAll(inputs => 
            inputs.map(input => ({
              type: input.getAttribute('type'),
              name: input.getAttribute('name'),
              placeholder: input.getAttribute('placeholder'),
    ...
    const certificationItems = await page.locator('[data-testid*="certification"], .certification-item, li').count()
    ...
    const profileCompletionVisible = await page.locator('[data-testid*="completion"], .completion, .progress').isVisible().catch(() => false)
    findings.explorationResults.dataDisplay.profileCompletionVisible = profileCompletionVisible
    
    // Step 13: Take final screenshot
    await page.screenshot({ path: '.playwright-mcp/admin-certifications-04-final.png', fullPage: true })
