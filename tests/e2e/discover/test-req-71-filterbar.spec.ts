...
test.describe('REQ-71: FilterBar Visual Enhancement', () => {
  ...
  test('should display FilterBar visible over map with proper contrast', async ({ page }: { page: Page }) => {
    ...
    const filterBar = page.locator('button[aria-label*="search" i], button[aria-label*="filter" i], button:has([data-testid*="icon"])').first()
    
    // FilterBar should be visible
    await expect(filterBar).toBeVisible({ timeout: 10000 }).catch(() => {
      // If not found by aria-label, try finding by position (absolute bottom)
      const buttons = page.locator('button').filter({ hasText: /\d+/ }).or(page.locator('button[data-testid*="icon"]'))
