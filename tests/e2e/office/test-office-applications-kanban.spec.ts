...
test.describe("Office • /office/applications - Kanban Board", () => {
  ...
  test.describe("Kanban Board Structure", () => {
    test("displays all 6 status columns", async ({ page }: { page: Page }) => {
      ...
      const interviewColumn = await getKanbanColumn(
        page,
      ...
      const rejectedColumn = await getKanbanColumn(
        page,
    ...
    test("displays application cards with correct data-testid", async ({page,}: {page: Page;}) => {
      ...
      const cardCount = await page.locator('[data-testid^="kanban-card-"]')
        .count();
  ...
  test.describe("Basic Drag and Drop", () => {
    test("drags application from new to screen column", async ({ page }: { page: Page }) => {
      ...
      const newColumn = await getKanbanColumn(page, KANBAN_COLUMNS.NEW);
      const firstCard = newColumn.locator('[data-testid^="kanban-card-"]')
        .first();
      ...
      const isInScreen = await verifyApplicationInColumn(
        page,
        candidateName,
        KANBAN_COLUMNS.SCREEN,
      );
    ...
    test("drags application from screen to interview column", async ({ page }: { page: Page }) => {
      ...
      const screenColumn = await getKanbanColumn(page, KANBAN_COLUMNS.SCREEN);
      const firstCard = screenColumn.locator('[data-testid^="kanban-card-"]')
        .first();
      const candidateName =
        (await firstCard.locator("text").first().textContent()) || "";
    ...
    test("drags application from interview to offer column", async ({ page }: { page: Page }) => {
      ...
      const firstCard = interviewColumn.locator('[data-testid^="kanban-card-"]')
        .first();
      const candidateName =
        (await firstCard.locator("text").first().textContent()) || "";
      ...
      const isInOffer = await verifyApplicationInColumn(
        page,
        candidateName,
  ...
  test.describe("Critical Status Changes with Modal", () => {
    ...
    test("shows modal when dragging to hired column", async ({ page }: { page: Page }) => {
      ...
      let sourceColumn = KANBAN_COLUMNS.OFFER;
    ...
    test("confirms hiring with optional notes", async ({ page }: { page: Page }) => {
      ...
      const isInHired = await verifyApplicationInColumn(
        page,
        candidateName,
        KANBAN_COLUMNS.HIRED,
      );
  ...
  test.describe("Modal Cancel Flow", () => {
    test("cancels rejection and returns card to original column", async ({page,}: {page: Page;}) => {
      ...
      const isInOriginal = await verifyApplicationInColumn(
        page,
        candidateName,
        sourceColumn,
      );
  ...
  test.describe("Kanban Summary and Metrics", () => {
    ...
    test("calculates total applications across all columns", async ({ page }: { page: Page }) => {
      ...
      const totalApplications = Object.values(summary).reduce(
        (sum, count) => sum + count,
        0,
      );
      ...
      const allCards = await page.locator('[data-testid^="kanban-card-"]')
        .count();
  ...
  test.describe("Edge Cases", () => {
    test("handles empty columns gracefully", async ({ page }: { page: Page }) => {
      ...
      const emptyColumn = Object.entries(summary).find(([_, count]) =>
        count === 0
      );
    ...
    test("preserves card data after drag", async ({ page }: { page: Page }) => {
      ...
      if (initialSummary.new === 0) {
        test.skip(
          "No applications in new column to test card data preservation",
        );
      }
