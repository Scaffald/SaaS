 ...
 * - Create organization flow ✅ PASSING
 * - Edit organization flow ✅ PASSING
 *
 * Wait strategy: visibility checks, waitForResponse for API operations.
 */
...
test.describe("Office • Organizations Management", () => {
  ...
  test.describe("Search Functionality", () => {
    ...
    test("search filters organizations by name", async ({ page }: { page: Page }) => {
      ...
      await page
        .waitForSelector("text=/no organizations found|no results/i", {
          timeout: 5000,
        })
        .catch(() => {
  ...
  test.describe("Create Organization Flow", () => {
    ...
    test("displays all required form fields", async ({ page }: { page: Page }) => {
      ...
      const visibilitySelect = page.locator(
        '[data-testid="org-form-visibility"]',
      );
      ...
      const responsePromise = page
        .waitForResponse(
          (response) =>
            ...
            (response.url().includes("organization") ||
  ...
  test.describe("Edit Organization Flow", () => {
    test("navigates to edit page from list", async ({ page }: { page: Page }) => {
      ...
      const editButton = page.locator('[data-testid^="org-edit-button-"]')
        .first();
    ...
    test("updates organization name successfully", async ({ page }: { page: Page }) => {
      ...
      const originalSlug = originalName.toLowerCase().replace(
        /[^a-z0-9]+/g,
        "-",
      );
