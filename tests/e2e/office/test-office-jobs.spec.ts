 ...
 * - Create job flow (all required fields)
 * - Edit job flow (modify and save)
 *
 * Note: Some tests may be blocked if create/edit routes redirect to sign-in.
 */
...
test.describe("Office • Jobs Management", () => {
  ...
  test.describe("Jobs List Page", () => {
    ...
    test("should filter jobs using search", async ({ page }: { page: Page }) => {
      ...
      if (initialCount > 0) {
        // Get first job title
        const firstJobTitle = await rows.first().locator("td").first()
          .textContent();
        ...
        if (firstJobTitle) {
          ...
          console.log(
            `Search filtered from ${initialCount} to ${filteredCount} jobs`,
          );
        }
      } else {
  ...
  test.describe("Create Job Flow", () => {
    ...
    test("should display position level field", async ({ page }: { page: Page }) => {
      ...
      const positionLevelInput = page.locator(
    ...
    test("should validate required fields", async ({ page }: { page: Page }) => {
      ...
      console.log(
        "Form validation working - save button disabled without required fields",
      );
    });
  });
