...
test.describe('REQ-71: Profile Edit Cancellation', () => {
  ...
  test('should disable cancel button when form has no unsaved changes', async ({
    page,
  }: {
    page: Page
  }) => {
    ...
    if (buttonCount > 0) {
      const isDisabled = await cancelButton
        .first()
        .isDisabled()
        .catch(() => false)
  ...
  test('should show confirmation dialog when cancel button is clicked', async ({
    page,
  }: {
    page: Page
  }) => {
    ...
    if (inputVisible) {
      ...
      if (cancelVisible && !(await cancelButton.isDisabled().catch(() => true))) {
        ...
        const dialog = page.locator(
          '[role="dialog"], [data-testid*="dialog"], [data-testid*="confirmation"]'
        )
  ...
  test('should reset form when "Discard Changes" is confirmed', async ({
    page,
  }: {
    page: Page
  }) => {
  ...
  test('should keep form dirty when "Keep Editing" is clicked', async ({
    page,
  }: {
    page: Page
  }) => {
  ...
  test('should display cancel button on profile education form', async ({
    page,
  }: {
    page: Page
  }) => {
