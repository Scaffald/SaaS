...
test.describe('REQ-26 • Primary Industry Dropdown UX Enhancement', () => {
  ...
  test('industry selection persists after visual enhancements', async ({ page }: { page: Page }) => {
    await openPrimaryIndustryDropdown(page)
  ...
  test('skill search enables after industry selection', async ({ page }: { page: Page }) => {
    ...
    const hasInitialMessage = initialContent.includes('select an industry') || 
                              initialContent.includes('Select an industry')
