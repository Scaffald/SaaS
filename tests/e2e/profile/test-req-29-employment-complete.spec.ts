...
test.describe('REQ-29 • Employment Preferences Complete Form Flow', () => {
  ...
  test('completes full form submission with all fields filled', async ({
    page,
  }: {
    page: Page
  }) => {
  ...
  test('shows success toast after save', async ({ page }: { page: Page }) => {
    ...
    await page.waitForSelector('text=Employment Updated', { timeout: 5000 }).catch(() => {})
    await page
      .waitForSelector('text=Your employment preferences have been saved', { timeout: 5000 })
      .catch(() => {})
  })
  ...
  test('travel distance slider supports full range (10-250 miles)', async ({
    page,
  }: {
    page: Page
  }) => {
    ...
    if (await travelSlider.isVisible()) {
      ...
      await page.waitForTimeout(300)
      await expect(page.getByText('10 miles'))
        .toBeVisible()
        .catch(() => {})
      ...
      await page.waitForTimeout(300)
      await expect(page.getByText('250 miles'))
        .toBeVisible()
        .catch(() => {})
    }
  })
