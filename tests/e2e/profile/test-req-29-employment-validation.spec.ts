...
test.describe('REQ-29 • Employment Preferences Validation', () => {
  ...
  test('blocks submission when driver license toggle is ON but no classes selected (VR1)', async ({
    page,
  }: {
    page: Page
  }) => {
    ...
    await page
      .waitForSelector('text=Please select at least one license class', { timeout: 3000 })
      .catch(() => {})
  ...
  test('requires travel distance when travel is enabled (VR2)', async ({
    page,
  }: {
    page: Page
  }) => {
  ...
  test('blocks submission when no residency status is selected (VR3)', async ({
    page,
  }: {
    page: Page
  }) => {
    ...
    // Should show validation error
    await page
      .waitForSelector('text=Please indicate your work authorization status', { timeout: 3000 })
      .catch(() => {})
  })

  test('validates hourly rate maximum (200)', async ({ page }: { page: Page }) => {
    const hourlyRateInput = page
      .locator('input[placeholder*="hourly rate" i], input[type="number"]')
      .first()
