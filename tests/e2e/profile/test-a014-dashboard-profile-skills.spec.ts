...
test.describe('Admin • /dashboard/profile/skills', () => {
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasEmptyMessage = pageContent.includes('No skills added yet') ||
                            pageContent.includes('add your first skill')
    const hasSkillsList = pageContent.includes('Proficiency') ||
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasEmptySearchState = pageContent.includes('Start typing') ||
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const inputs = await page.locator('input[type="text"], input[placeholder*="Search"], input[placeholder*="search"]').all()
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasSkillDisplay = pageContent.includes('Code:') ||
                            pageContent.includes('No skills added yet')
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasProficiencyRange =
      ...
      (pageContent.includes('1') && pageContent.includes('5'))
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const inputs = await page.locator('input[placeholder*="Search"], input[placeholder*="search"]').all()
    ...
    if (inputs.length > 0) {
      try {
        ...
        const hasResults = resultsContent.includes('CSI') ||
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasLeftPanel = pageContent.includes('Skills & Expertise') ||
                         pageContent.includes('Primary Industry')
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasIndustryPrompt = pageContent.includes('select an industry') ||
