...
test.describe('Admin • /', () => {
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    // Verify widget title
    const profileWidgetVisible = await page.getByText('Complete Your Profile').isVisible().catch(() => false)
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const careerAssessmentVisible = await page.getByText('Career Assessment').isVisible().catch(() => false)
    ...
    if (careerAssessmentVisible) {
      ...
      for (const dimension of riasecDimensions) {
        const isVisible = await page.getByText(dimension).isVisible().catch(() => false)
      ...
      const completeButton = await page.getByRole('button', { name: /Complete Assessment/i }).isVisible().catch(() => false)
      if (completeButton) {
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const newsVisible = await page.getByText('News').isVisible().catch(() => false)

    if (newsVisible) {
      // Verify news source selector
      const enrVisible = await page.getByText('ENR National').isVisible().catch(() => false)
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const dashboardLinkVisible = await page.getByRole('link', { name: 'Dashboard' }).isVisible().catch(() => false)

    // Alternative: check if Discover or Profile navigation exists
    const discoverVisible = await page.getByText('Discover').first().isVisible().catch(() => false)
    const profileTextVisible = await page.getByText('Profile').first().isVisible().catch(() => false)
