...
test.describe('Admin • /dashboard/profile/employment', () => {
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const availabilityOptions = ['Part-time', 'Contract', 'Full-time', 'Weekend', 'Shift', 'Temporary']
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const hasSaveButton = buttonTexts.some(text =>
  ...
  // Authentication handled by storage state (tests/.auth/admin.json)
    ...
    const sections = [
      'location', 'travel', 'resident', 'passport',
      'license', 'military', 'available'
    ]
