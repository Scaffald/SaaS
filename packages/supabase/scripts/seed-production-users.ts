...
const OUTPUT_PATH = path.join(
  __dirname,
  "../seeds/03_seed-production-users.sql",
);
...
function generateHeadline(description: string, yearsExp: number): string {
  if (description && description.length > 10) {
    ...
    const shortDesc = firstSentence.length > 60
      ? `${firstSentence.substring(0, 57)}...`
      : firstSentence;

    if (shortDesc.length > 5) {
      return `${shortDesc} | ${yearsExp} ${
        yearsExp === 1 ? "Year" : "Years"
      } Experience`;
    }
  }

  const level = yearsExp >= 10
    ? "Senior"
    : yearsExp >= 5
    ? "Experienced"
    : "Construction";
...
function parseDriversLicense(licenseStr: string): string {
  ...
  // Map common values
  if (
    first.includes("CDL") || first === "A" || first === "B" || first === "C"
  ) {
    return first;
  }
...
async function geocodeAddress(
  ...
  try {
    ...
    const url =
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodedAddress}.json?access_token=${MAPBOX_TOKEN}&language=en&limit=1&types=address,place&country=us`;

    const response = await fetch(url);

    if (!response.ok) {
      console.warn(
        `⚠️  Mapbox API error for "${addressString}": ${response.status}`,
      );
      return null;
    }
    ...
    if (data.features && data.features.length > 0) {
      ...
      if (!city || !state) {
        const parts = feature.place_name.split(",").map((p: string) =>
          p.trim()
        );
        if (parts.length >= 3) {
...
async function processUser(
  ...
  const contactPrefs = parsePostgresArray(row.preferred_contact).map((p) =>
    p.toLowerCase()
  );
  ...
  if (row.location && row.location.trim() !== "") {
    ...
    if (wkbCoords) {
      ...
      const addressParts = row.address
        ? row.address.split(",").map((s) => s.trim())
        : [];

      geocodedAddress = {
        street: addressParts[0] || "",
        city: addressParts.length >= 2
          ? addressParts[addressParts.length - 2]
          : "",
        state: addressParts.length >= 2
          ? addressParts[addressParts.length - 1].split(" ")[0]
          : "",
        postal: "",
        country: "USA",
...
function parseWKB(
  ...
  try {
    ...
    const longitude = isLittleEndian
      ? buffer.readDoubleLE(xOffset)
      : buffer.readDoubleBE(xOffset);

    const latitude = isLittleEndian
      ? buffer.readDoubleLE(yOffset)
      : buffer.readDoubleBE(yOffset);
...
function generateSQL(users: ProcessedUser[]): string {
  ...
  sql.push(
    `-- Generated from production database CSV export on ${
  ...
  const privateValues = users.map((user) => {
    ...
    const values = [
      ...
      addressJsonb
        ? `'${JSON.stringify(addressJsonb).replace(/'/g, "''")}'::jsonb`
        : `NULL`,
      geoPoint,
...
async function main() {
  ...
  console.log(
    `Geocoded addresses: ${
      users.filter((u) => u.geocodedAddress !== null).length
    }`,
  );
  console.log(`With phone: ${users.filter((u) => u.phone).length}`);
  console.log(
    `With certifications: ${
      users.filter((u) => u.certifications.length > 0).length
    }`,
  );
  console.log(`Veterans: ${users.filter((u) => u.veteran).length}`);
  console.log(
    `US Passport holders: ${users.filter((u) => u.usPassport).length}`,
  );
  console.log(
    `Average experience: ${
      Math.round(
