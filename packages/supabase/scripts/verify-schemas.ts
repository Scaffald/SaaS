...
async function main() {
  ...
  try {
    ...
    const csiResult = await client.query('SELECT count(*) FROM data.masterformat')
    console.log(`✅ CSI MasterFormat: ${csiResult.rows[0].count} records`)

    // Check O*NET
    const onetResult = await client.query('SELECT count(*) FROM onet.occupation_data')
    console.log(`✅ O*NET Occupations: ${onetResult.rows[0].count} records`)

    // Check Universities
    const uniResult = await client.query('SELECT count(*) FROM data.universities')
    console.log(`✅ Universities: ${uniResult.rows[0].count} records`)

    // Check new polymorphic tables
    const userSkillsResult = await client.query('SELECT count(*) FROM public.user_skills')
    console.log(
      `✅ User Skills Table: ${userSkillsResult.rows[0].count} records (empty - new table)`
    )

    const jobSkillsResult = await client.query('SELECT count(*) FROM public.job_skills')
    console.log(`✅ Job Skills Table: ${jobSkillsResult.rows[0].count} records (empty - new table)`)

    const orgSkillsResult = await client.query('SELECT count(*) FROM public.organization_skills')
    console.log(`✅ Org Skills Table: ${orgSkillsResult.rows[0].count} records (empty - new table)`)
    ...
    `)
    console.log(`Found ${crossSearchResult.rows.length} results for 'engineer':`)
