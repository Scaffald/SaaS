 ...
 */
function processUniversities(records: UniversityRecord[]): ProcessedUniversity[] {
...
async function upsertUniversities(universities: ProcessedUniversity[]): Promise<void> {
  const batchSize = 500
  ...
  for (let i = 0; i < universities.length; i += batchSize) {
    ...
    const { error } = await supabase.schema('data').from('universities').upsert(batchData, {
      onConflict: 'slug',
    })
