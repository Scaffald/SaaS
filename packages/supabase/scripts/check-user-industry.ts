  ...
  try {
    ...
    for (const user of users) {
      ...
      if (matchesConstruction) {
        ...
        console.log(
          `   Industry: ${user.industries?.name || 'N/A'} (${user.industries?.slug || 'N/A'})`
        )
    ...
    console.log(
      `Users with other industries: ${users.length - usersWithConstruction - usersWithoutIndustry}`
    )
    ...
    if (articleCount && articleCount > 0 && usersWithConstruction > 0) {
      ...
      console.log(
        '   The widget should fallback to Construction industry, so users should still see news.'
      )
    } else {
      console.log(
