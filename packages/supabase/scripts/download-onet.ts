...
// When running from root, process.cwd() is root, so go to packages/supabase
...
async function downloadFile(url: string, filePath: string): Promise<boolean> {
  try {
    ...
    stats.errors.push(
      `Failed to download ${path.basename(filePath)}: ${errorMsg}`,
    );
    return false;
  }
}
...
async function downloadOnetFile(
  ...
  if (downloadedOnetFiles.has(mapping.onetName)) {
    ...
    if (fs.existsSync(sourcePath)) {
      ...
      console.log(
        `📋 Copied ${mapping.localName} from ${mapping.onetName}.sql`,
      );
      stats.downloaded++;
      return true;
    }
  ...
  console.log(
    `⬇️  Downloading ${mapping.localName} from ${mapping.onetName}.sql...`,
  );
  ...
  if (success) {
    ...
    if (tempPath !== localPath) {
      ...
      if (
        !FILE_MAPPING.some(
          (m) =>
            m.onetName === mapping.onetName &&
            m.localName !== mapping.localName,
        )
      ) {
...
function flattenExtractedTextFiles(): number {
  ...
  if (!hasTopLevelTxt) {
    for (const entry of entries) {
      ...
      for (
        const nested of fs.readdirSync(sourceDir, {
          withFileTypes: true,
        })
      ) {
...
async function main() {
  ...
  const fetchText = textOnly || args.includes("--with-text") ||
    !args.includes("--skip-text");
  ...
  if (skipSqlDownload) {
    ...
    const onetFiles = fs.readdirSync(ONET_DIR).filter((f) =>
      f.endsWith(".sql")
    );
