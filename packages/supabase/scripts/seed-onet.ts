...
const SOURCE_ROOT =
  process.env.ONET_SOURCE_DIR ??
  path.join(PROJECT_ROOT, "packages/supabase/seed-data/onet/raw");
const FORCE_RELOAD = process.env.ONET_FORCE_RELOAD === "1";
const DEFAULT_TABLE_PAUSE_MS = 1000;
const parsedPauseMs = Number.parseInt(
  process.env.ONET_TABLE_PAUSE_MS ?? "",
  10
);
const TABLE_PAUSE_MS = Number.isNaN(parsedPauseMs)
  ? DEFAULT_TABLE_PAUSE_MS
  : parsedPauseMs;
...
async function main() {
  ...
  const sentinelConfig = TABLE_LOAD_ORDER.find(
    (config) => config.table === "occupation_data"
  );
  ...
  try {
    const existingOccupationRows = await getTableRowCount(
      pool,
      sentinelConfig.table
    );
...
function copyStreamSafeDestroy(stream: Duplex, error: unknown): void {
  ...
  stream.destroy(
    new Error(
      typeof error === "string" ? error : "COPY stream failed with unknown error"
    )
  );
}
    ...
    for await (const record of parser) {
      const row = config.columns.map((column) => {
        ...
        return column.transform
          ? column.transform(rawValue)
          : defaultTransform(rawValue);
      });
