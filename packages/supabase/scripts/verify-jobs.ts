...
async function verifyJobs() {
  ...
  console.log(
    `   Jobs with Responsibilities: ${jobsWithResp} (${Math.round(
      (jobsWithResp! / totalJobs!) * 100
    )}%)`
  )
  console.log(
    `   Jobs with Requirements: ${jobsWithReq} (${Math.round((jobsWithReq! / totalJobs!) * 100)}%)`
  )
  console.log(
    `   Jobs with Benefits: ${jobsWithBen} (${Math.round((jobsWithBen! / totalJobs!) * 100)}%)`
  )
  console.log('')
}
