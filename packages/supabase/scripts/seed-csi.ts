...
function parentKey(AA: string, BB: string, CC: string, DD: string): string | null {
...
function loadXlsx(filePath: string): Row[] {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    const data = XLSX.utils.sheet_to_json(ws, { header: 1, blankrows: false }) as (
      | string
      | number
      | null
      | undefined
    ...
    for (const row of data) {
      ...
      if (row.length === 1 && typeof row[0] === 'string') {
        ...
        if (!isTwoDigits(AA) || !isTwoDigits(BB) || !isTwoDigits(CC) || !isTwoDigits(DD)) continue
  ...
  for (const r of rows) {
    ...
    for (const ak of ancestors) {
      ...
      add.push({
        ...
        id: toUUID(ak),
        parent_id: parentKey(AA, BB, CC, DD) ? toUUID(parentKey(AA, BB, CC, DD)!) : null,
      })
      existing.add(ak)
    }
  }
...
async function upsertSkills(client: Client, rows: Row[]) {
  const depth = (r: Row) => (r.DD !== '00' ? 4 : r.CC !== '00' ? 3 : r.BB !== '00' ? 2 : 1)
  rows.sort((a, b) => depth(a) - depth(b) || a.code_key.localeCompare(b.code_key))
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    slice.forEach((r, idx) => {
      const o = idx * 6
      tuples.push(
        `($${o + 1}, $${o + 2}, $${o + 3}, $${o + 4}, $${o + 5}, $${o + 6}, NOW(), NOW())`
      )
...
async function main() {
  ...
  try {
    ...
    console.log(`Seeded ${rows.length} CSI 2020 records (including synthesized parents).`)
  } catch (err) {
