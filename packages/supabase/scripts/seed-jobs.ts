...
function parseJobDescription(
  ...
  // Clean HTML tags
  const cleanText = description.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ")
    .trim();
...
function extractBulletPoints(text: string): string[] {
  ...
  if (bulletMatches && bulletMatches.length > 0) {
    points.push(
      ...bulletMatches.map((m) => m.replace(/^[•\-*]\s*/, "").trim()).filter(
        (p) => p.length > 10,
      ),
    );
  }
...
function extractCompensation(text: string): {
  ...
  if (match) {
    ...
    const max = match[2]
      ? Number.parseInt(match[2]) * (match[2].includes("k") ? 1000 : 1)
      : null;
    const period = match[3]
      ? (match[3].includes("hour") ? "hourly" : "yearly")
      : "yearly";
 ...
 */
function parseCompanyFromTitle(
  title: string,
  ...
  for (const feed of feeds || []) {
    ...
    try {
      ...
      for (const item of items.slice(0, limit)) {
        ...
        parsedData.company_name = company !== "Unknown Company"
          ? company
          ...
          .upsert(
            {
              ...
              requirements: parsedData.requirements.length > 0
                ...
                : null,
              benefits: parsedData.benefits.length > 0
                ? parsedData.benefits
                : null,
