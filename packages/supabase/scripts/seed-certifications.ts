...
async function seedCertifications() {
  ...
  try {
    ...
    if (deleteError) {
      console.error('⚠️  Warning: Could not clear existing certifications:', deleteError.message)
    } else {
    ...
    for (const [topSlug, topNode] of Object.entries(certData) as [string, CertificationNode][]) {
      sortOrder++
      ...
      for (const [childKey, childValue] of Object.entries(topNode)) {
        ...
        for (const [grandchildKey, grandchildValue] of Object.entries(childNode)) {
          // Skip metadata fields
    ...
    console.log(`   - Level 0 (top): ${certificationsToInsert.filter((c) => c.depth === 0).length}`)
    console.log(
      `   - Level 1 (categories): ${certificationsToInsert.filter((c) => c.depth === 1).length}`
    )
    console.log(
      `   - Level 2 (certifications): ${certificationsToInsert.filter((c) => c.depth === 2).length}`
    )

    // Insert certifications in order (depth 0, then 1, then 2)
    // This ensures parent IDs exist before children are inserted
    for (let depth = 0; depth <= 2; depth++) {
      const certsAtDepth = certificationsToInsert.filter((c) => c.depth === depth)
      ...
      for (const cert of certsAtDepth) {
        ...
        if (cert.parent_id) {
          ...
          if (!parentUuid) {
            console.error(`❌ Could not find parent UUID for ${cert.parent_id}`)
            continue
          }
        }
