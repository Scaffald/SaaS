...
async function checkData() {
  ...
  for (const job of jobs || []) {
    ...
    console.log(`   Compensation: ${job.compensation_min ? `$${job.compensation_min}` : 'N/A'}`)
    console.log(`   Website: ${job.company_website || 'N/A'}`)

    console.log('\n📝 Description:')
    console.log(job.description ? job.description.substring(0, 300) + '...' : 'N/A')
    ...
    console.log('\n🔍 Raw Data Sample:')
    if (job.raw_data) {
      console.log(`   Title: ${job.raw_data.title?.substring(0, 100) || 'N/A'}`)
      console.log(`   Description: ${job.raw_data.description?.substring(0, 200) || 'N/A'}...`)
    }
