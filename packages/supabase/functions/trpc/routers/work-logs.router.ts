...
// Deno requires file extension in Deno runtime
import { buildWorkLogCsv, buildWorkLogPdf } from '../../_shared/work-log-export.ts'
// Deno requires file extension in Deno runtime
...
const buildPoint = (latitude: number, longitude: number) => `POINT(${longitude} ${latitude})`
...
const LIST_SORT_FIELDS = ['log_date', 'created_at', 'updated_at', 'total_hours'] as const
...
const listWorkLogsInputSchema = z.object({
  ...
  dateFrom: z
    ...
    .optional(),
  dateTo: z.string().regex(DATE_ONLY_REGEX, 'Invalid date format. Expected YYYY-MM-DD.').optional(),
...
const WORK_LOG_STATUSES = ['draft', 'pending_verification', 'verified', 'disputed'] as const
...
const fetchWorkLogRelationshipCounts = async (
  ...
  const [photoResult, collaboratorResult, conversationResult] = await Promise.all([
    supabase
      .schema('core')
  ...
  for (const row of photoResult.data ?? []) {
    const workLogId = typeof row?.work_log_id === 'string' ? row.work_log_id : null
...
const toProjectMetadata = (project: Record<string, unknown>): ProjectMetadata => {
...
const getOrganizationManagerIds = async (supabase: DbClient, organizationId: string) => {
  const { data, error } = await supabase
    .schema('core')
  ...
  return data
    .filter(
      (assignment: { roles?: { scope?: string; name?: string } | null; user_id: string | null }) =>
...
const parseTimeEntries = (raw: unknown): WorkLogExportTimeEntry[] => {
  ...
  for (const candidate of raw) {
    ...
    const breakMinutes = Number.isFinite(rawBreak) && rawBreak > 0 ? Math.floor(rawBreak) : 0
...
function resolveProjectDisplayName(project: Record<string, unknown> | null | undefined): string {
  const name = resolveStringField(project, ['name', 'project_name', 'title', 'display_name']) ?? ''
...
const buildWorkLogExportSnapshot = async (
  ...
  if (organizationId) {
    ...
    organizationRecord = (organizationData as Record<string, unknown> | null) ?? null
  }
  ...
  for (const row of collaboratorRows ?? []) {
    ...
    const displayName = await getUserDisplayName(supabase, row.collaborator_user_id)
  ...
  if (skillIds.length > 0) {
    ...
    const enrichedSkills = await enrichUserSkills(supabase, userSkillRows ?? [])
...
const entriesOverlap = (a: { start: string; end: string }, b: { start: string; end: string }) => {
...
const fetchWorkLog = async (supabase: DbClient, workLogId: string): Promise<WorkLogRow> => {
  const { data, error } = await supabase
    .schema('core')
    .from('work_logs')
...
const resolveCsiSkillDetails = async (
  ...
  return new Map(
    (data ?? []).map(
      (row: { id: string; name: string; code_key: string | null; code_display: string | null }) => [
        row.id,
        {
          name: row.name,
...
const getSuggestedSkillsForWorkLog = async (
  ...
  for (const skillId of missingCsiSkillIds) {
    ...
    const label = detail.code_display ? `${detail.code_display} · ${detail.name}` : detail.name
...
const getUserDisplayName = async (supabase: DbClient, userId: string): Promise<string> => {
...
const getProjectVerificationRequirement = async (supabase: DbClient, projectId: string) => {
...
export const workLogsRouter = t.router({
  getProjectOptions: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { data: memberships, error: membershipsError } = await supabase
        ...
        .select('user_id, organization_id, organization_name, is_admin, is_owner')
        .eq('user_id', user.id)
      ...
      const projects = (projectRows ?? [])
        ...
        .map((project: Record<string, unknown>) => {
          ...
          const status = resolveStringField(project, ['status', 'project_status', 'state']) ?? null

          const startsAt =
            resolveStringField(project, ['start_date', 'starts_at', 'project_start']) ?? null

          const endsAt = resolveStringField(project, ['end_date', 'ends_at', 'project_end']) ?? null
        ...
        .filter(
          (project: { organizationId: string; isArchived: boolean; name: string; id: string }) => {
  ...
  getProjectRollup: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const workLogIds = workLogs
        .map((row: { id: string | number }) => (typeof row.id === 'string' ? row.id : null))
        .filter((value: string | null): value is string => Boolean(value))

      const projectMap = await fetchProjectMetadata(supabase, [input.projectId])
      ...
      const relationshipCounts = await fetchWorkLogRelationshipCounts(supabase, workLogIds)
      ...
      for (const log of workLogs) {
        ...
        const collaborators = relationshipCounts.collaboratorsByWorkLog.get(logId) ?? []
      ...
      const recentActivity = workLogs
        ...
        .sort((a: { updatedAt: string | null }, b: { updatedAt: string | null }) => {
  ...
  publicProfileFeed: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const logIds = workLogs
        ...
        .filter((value: string | number): value is string => typeof value === 'string')
      ...
      const projectMap = new Map<string, { name: string | null; organizationName: string | null }>()
      ...
      if (projectIds.length > 0) {
        ...
        if (projectError) {
          console.warn('[workLogs.publicProfileFeed] Unable to load project metadata', {
          ...
          for (const project of projectRows ?? []) {
            ...
            const organizationName =
              typeof project.organizations?.name === 'string' ? project.organizations.name : null
      ...
      if (logIds.length > 0) {
        const { data: photoRows, error: photoError } = await supabaseAdmin
          .schema('core')
          .from('work_log_photos')
          .select('id, work_log_id, file_path, thumbnail_path, caption, display_order')
          ...
          const signedPhotos = await Promise.all(
            (photoRows ?? []).map(
              async (photo: {
                ...
                const thumbnailPath =
                  typeof photo.thumbnail_path === 'string' ? photo.thumbnail_path : null
                ...
                try {
                  if (filePath) {
                    const { data: signed } = await supabaseAdmin.storage
                      ...
                      .createSignedUrl(filePath, PUBLIC_WORK_LOG_PHOTO_TTL_SECONDS)
                    signedUrl = signed?.signedUrl ?? null
                  }
                } catch (error) {
                  console.warn('[workLogs.publicProfileFeed] Failed to sign photo URL', {
                    filePath,
                    error,
                  })
                }
                ...
                try {
                  if (thumbnailPath) {
                    const { data: signedThumb } = await supabaseAdmin.storage
                      .from(WORK_LOG_PHOTO_BUCKET)
                      .createSignedUrl(thumbnailPath, PUBLIC_WORK_LOG_PHOTO_TTL_SECONDS)
                    thumbnailSignedUrl = signedThumb?.signedUrl ?? null
                  }
                } catch (error) {
                  console.warn('[workLogs.publicProfileFeed] Failed to sign thumbnail URL', {
                    thumbnailPath,
                    error,
                  })
                }
                ...
                return {
                  id: photo.id,
                  workLogId: typeof photo.work_log_id === 'string' ? photo.work_log_id : null,
      ...
      const result = workLogs.map(
        (log: { id: string | number; project_id?: string | null; [key: string]: unknown }) => {
          const logId = String(log.id)
          const project =
            typeof log.project_id === 'string' ? (projectMap.get(log.project_id) ?? null) : null
  ...
  projectAnalytics: officeProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      await assertUserIsOrganizationManager(supabase, user.id, organizationSettings)
      ...
      const workerMetadata = new Map<string, { displayName: string; username: string | null }>()
      ...
      for (const log of workLogs) {
        ...
        if (workerId) {
          ...
          aggregate.commentCount += relationshipCounts.commentCount.get(logId) ?? 0
          ...
          if (
            ...
            (!aggregate.latestActivity ||
              Number(new Date(activityTimestamp)) > Number(new Date(aggregate.latestActivity)))
          ) {
  ...
  list: protectedProcedure.input(listWorkLogsInputSchema).query(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const projectIds = workLogs
      .map((row: { project_id?: string | null }) =>
    ...
    const items = workLogs.map(
      (row: {
        ...
        return {
          ...
          entryType: typeof row.entry_type === 'string' ? row.entry_type : 'daily',
          totalHours,
          ...
          descriptionPreview:
            typeof row.work_description === 'string' ? shorten(row.work_description, 220) : null,
        }
      }
    )
  ...
  getOverview: protectedProcedure.input(ownerOverviewInputSchema).query(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const projectSummaries = Array.from(projectGroups.entries())
      .map(([projectId, logs]) => {
        ...
        for (const log of logs) {
          ...
          projectCommentCount += relationshipCounts.commentCount.get(logId) ?? 0
          ...
          if (
            activityTimestamp &&
            (!latestActivity ||
              Number(new Date(activityTimestamp)) > Number(new Date(latestActivity)))
          ) {
  ...
  create: protectedProcedure.input(createWorkLogSchema).mutation(async ({ ctx, input }) => {
    ...
    if (gpsCapture) {
      ...
      insertPayload.gps_location = buildPoint(gpsCapture.latitude, gpsCapture.longitude)
      insertPayload.gps_accuracy_meters = normalizeAccuracyMeters(gpsCapture.accuracyMeters)
      insertPayload.gps_captured_at = gpsCapture.capturedAt ?? null
      insertPayload.device_type = gpsCapture.deviceType ?? null
      insertPayload.location_permission_status = gpsCapture.permissionStatus ?? null
    }
  ...
  update: protectedProcedure.input(updateWorkLogSchema).mutation(async ({ ctx, input }) => {
    ...
    const { workLog: _workLog, role } = await getWorkLogAccess(supabase, input.workLogId, user.id)
    ...
    const assignField = <Key extends keyof typeof input.payload>(key: Key, columnName: string) => {
      const newVal = input.payload[key]
      if (typeof newVal !== 'undefined') {
        updates[columnName] = newVal
        oldValue[columnName] = (_workLog as Record<string, unknown>)[columnName]
    ...
    if (input.payload.gpsCapture) {
      ...
      updates.gps_accuracy_meters = normalizeAccuracyMeters(capture.accuracyMeters)
      ...
      oldValue.location_permission_status = _workLog.location_permission_status ?? null
      newValue.location_permission_status = updates.location_permission_status ?? null
    }
  ...
  submit: protectedProcedure.input(submitWorkLogSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const suggestedSkills = await getSuggestedSkillsForWorkLog(supabase, data, user.id)
  ...
  verify: officeProcedure.input(verifyWorkLogSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const actorName = await getUserDisplayName(supabase, user.id)
    await createSystemMessage(supabase, data, user.id, `Work log verified by ${actorName}`)

    return data
  }),

  dispute: officeProcedure.input(disputeWorkLogSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
  ...
  addCollaborator: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { workLog, role } = await getWorkLogAccess(supabase, input.workLogId, user.id)
  ...
  addComment: protectedProcedure.input(addWorkLogCommentSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const { workLog } = await getWorkLogAccess(supabase, input.workLogId, user.id)
    ...
    if (!input.isSystemMessage) {
      await notifyConversationParticipants(supabase, workLog, user.id, shorten(input.message, 140))
    }
  ...
  getSuggestedSkills: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const suggestions = await getSuggestedSkillsForWorkLog(supabase, workLog, user.id)
  ...
  addSkillToProfile: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: existingSkill, error: existingSkillError } = await supabase
        ...
        .eq(input.taxonomy === 'csi' ? 'csi_skill_id' : 'onet_occupation_id', skillIdentifier)
        .maybeSingle()
  ...
  uploadPhoto: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.gpsCapture) {
        ...
        insertPayload.gps_accuracy_meters = normalizeAccuracyMeters(input.gpsCapture.accuracyMeters)
        insertPayload.gps_captured_at = input.gpsCapture.capturedAt ?? null
        insertPayload.device_type = input.gpsCapture.deviceType ?? null
        insertPayload.location_permission_status = input.gpsCapture.permissionStatus ?? null
      }
  ...
  updatePhotoVisibility: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { workLog, role } = await getWorkLogAccess(supabase, photo.work_log_id, user.id)
  ...
  deletePhoto: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: photoRecord, error: photoError } = await supabase
        .schema('core')
        .from('work_log_photos')
        .select('id, work_log_id, file_path, thumbnail_path, medium_path, file_size_bytes')
        .eq('id', input.photoId)
        .single()
      ...
      if (photoBytes > 0) {
        ...
        if (usage) {
          const nextBytes = Math.max(0, (usage.work_log_photos_bytes ?? 0) - photoBytes)
  ...
  moveToProject: protectedProcedure.input(moveWorkLogSchema).mutation(async ({ ctx, input }) => {
    const { supabase, supabaseAdmin, user } = ctx
    ...
    if (workLog.project_id === input.targetProjectId && !workLog.pending_move_to_project_id) {
      return workLog
    }
    ...
    if (sourceProject.organization_id !== targetProject.organization_id) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Work logs can only be moved between projects within the same organization.',
      })
    }
    ...
    if (requireApproval) {
      ...
      if (managerRecipients.size > 0) {
        await notifyMoveEvent(notificationClient, [...managerRecipients], {
          ...
          message: 'A work log move is awaiting your approval before it can be completed.',
          metadata: {
  ...
  approveMoveRequest: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const actorName = await getUserDisplayName(supabase, user.id)
      await createSystemMessage(supabase, data, user.id, `Work log move approved by ${actorName}.`)

      const requesterId = workLog.pending_move_requested_by ?? workLog.user_id ?? null
      ...
      if (requesterId) {
        await notifyMoveEvent(notificationClient, [requesterId], {
          ...
          message: 'Your work log move request has been approved and is now complete.',
          metadata: {
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (managerRecipients.size > 0) {
        await notifyMoveEvent(notificationClient, [...managerRecipients], {
          ...
          message: 'A pending work log move request was cancelled by the worker.',
          metadata: {
  ...
  exportWorkLog: protectedProcedure.input(exportWorkLogSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
    ...
    const baseNameParts = ['work-log', workLog.log_date ?? null, workLog.id.slice(0, 8)].filter(
      Boolean
    )
    ...
    const storagePath = `${user.id}/${workLog.id}/${fileBaseName}-${timestampSuffix}.${extension}`
    ...
    const expiresAtIso =
      (signedUrlData as { signedUrl: string; expiresAt?: string }).expiresAt ??
  ...
  checkTimeOverlap: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.workLogId) {
        const { role } = await getWorkLogAccess(supabase, input.workLogId, user.id)
      ...
      const relevantLogs = (workLogs ?? []).filter(
        (log: { id: string | number }) => log.id !== input.workLogId
      )
