...
const buildWorkLogExportSnapshot = async (
  ...
  return {
    workLog,
    ownerName,
    ownerEmail: typeof ownerRecord?.email === 'string' ? ownerRecord.email : null,
...
export const workLogsRouter = t.router({
  ...
  update: protectedProcedure.input(updateWorkLogSchema).mutation(async ({ ctx, input }) => {
    ...
    if (input.payload.gpsCapture) {
      ...
      oldValue.location_permission_status = workLog.location_permission_status ?? null
