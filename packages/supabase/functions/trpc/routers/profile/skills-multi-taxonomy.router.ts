...
export const skillsMultiTaxonomyRouter = t.router({
  ...
  getUserSkills: protectedProcedure.query(async ({ ctx }) => {
    ...
    const validSkills = enrichedSkills.filter((skill) =>
      skill.skill_details !== null
    );
   ...
   */
  addSkill: protectedProcedure.input(addSkillInputSchema).mutation(
    async ({ ctx, input }) => {
      ...
      const ensureSkillAdded = async (
        ...
        const { error } = await userScopedClient.schema("core").from(
          "user_skills",
        ).insert(skillData);
      ...
      if (input.taxonomy === "csi") {
        ...
        const { data: parentIds, error: parentError } = await supabase.rpc(
          "get_skill_parent_ids",
          {
   ...
   * Update a user skill
   */
  updateSkill: protectedProcedure.input(updateSkillInputSchema).mutation(
    async ({ ctx, input }) => {
   ...
   * Remove a user skill
   */
  removeSkill: protectedProcedure.input(removeSkillInputSchema).mutation(
    async ({ ctx, input }) => {
      const { user, userToken } = ctx;
