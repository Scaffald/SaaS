...
async function extractResumeText(fileBytes: Uint8Array, fileType: ResumeFileType): Promise<string> {
  switch (fileType) {
...
function sanitizeString(value: unknown): string | null {
  ...
  const withoutControl = trimmed.replace(
    new RegExp(`[${controlChars.replace(/[[\]\\]/g, '\\$&')}]+`, 'g'),
    ''
  )
...
`
        ...
        if (content) {
          ...
          console.warn('[profileImport] OpenAI response failed schema validation')
        }
      }
    } catch (error) {
...
async function clearImportMetadata(supabase: Context['supabase'], userId: string): Promise<void> {
...
export const profileImportRouter = t.router({
  parseResume: protectedProcedure.input(resumeParseInputSchema).mutation(async ({ ctx, input }) => {
    const { user } = ctx
    if (!user) {
    ...
    const { payload, metadata } = await generateStructuredPayload(extractedText, {
      useOpenAi: true,
    })
