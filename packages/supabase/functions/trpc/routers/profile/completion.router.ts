...
function getSectionStatuses(
  ...
  const experienceComplete = Array.isArray(experienceData) &&
    experienceData.some((entry) => {
      if (!entry || typeof entry !== "object") return false;
      const record = entry as Record<string, unknown>;
      return `${record.job_title ?? ""}`.trim().length > 0 &&
  ...
  const preferencesMissing = preferencesComplete
    ? []
    : ["work_location_or_address"];

  const educationLevel = `${profile.education_level ?? ""}`.trim().length > 0;
  const educationEntriesExist = Array.isArray(educationData) &&
...
function calculateSummary(
...
): CompletionStatusPayload["summary"] {
  const completedWeight = sections.reduce(
    (total, section) =>
      section.completed ? total + section.weight : total,
    0,
  );
  ...
  const nextMilestone = COMPLETION_MILESTONES.find((threshold) =>
    threshold > currentPercentage
  ) ?? null;
...
function computeCompletionStatus({
  ...
  const completionHistory = parseCompletionHistory(
    preferencesData?.completion_history,
  );
  const nudgeHistory = parseNudgeHistory(preferencesData?.nudge_history);

  const { milestones, updatedMilestones, milestonesUpdated } =
    calculateMilestones(
  ...
  const shouldPersistHistory = milestonesUpdated ||
    completionHistory.lastPercentage !== completionPercentage;
...
async function evaluateCompletion({
  ...
  if (persistHistory && computation.shouldPersistHistory) {
    ...
    if (persistError) {
      console.error(
        "[profileCompletion] Failed to persist completion history",
        persistError,
      );
    }
  }
...
function generatePersonalizedBenefits(
  ...
  const baseMessages: Record<ProfileWizardStepId, {
    ...
    general: {
      ...
      general: {
        ...
        description:
          "A concise headline makes it easier for the right opportunities to find you.",
        relatedSection: "general",
        userType: "general",
        opportunityCount: 5,
      },
    ...
    education: {
      ...
      general: {
        ...
        description:
          "Education entries help the matching engine factor in certification pathways.",
        relatedSection: "education",
        userType: "general",
        opportunityCount: 4,
      },
...
export const profileCompletionRouter = t.router({
  ...
  dismissNudge: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const timestamp = new Date().toISOString();
        const { preferencesData } = await fetchCompletionData(
          supabase,
          user.id,
        );
        const existingHistory = parseNudgeHistory(
          preferencesData?.nudge_history,
        );
  ...
  getPersonalizedBenefits: protectedProcedure.query(async ({ ctx }) => {
    ...
    try {
      ...
      const normalizedUserTypes = computation.userTypes.map((type) =>
        type.toLowerCase()
      ) as Array<"worker" | "employer" | "customer" | "general">;

      if (normalizedUserTypes.length === 0) {
        normalizedUserTypes.push("general");
      }

      const benefits = generatePersonalizedBenefits(
        normalizedUserTypes,
