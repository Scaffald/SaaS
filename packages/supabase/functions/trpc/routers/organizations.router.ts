...
async function ensureOrganizationAccess(
  ...
  const isAdmin = superAdmin || isOwner ||
...
const locationUpsertSchema = z.object({
  ...
  name: z.string().min(1).max(255),
  locationType: z
    .enum(["headquarters", "branch", "job_site", "remote", "other"])
    .default("other"),
  address: z
    .object({
...
export const organizationsRouter = t.router({
  ...
  markInvitationViewed: t.procedure
    .input(viewInvitationSchema)
    .mutation(async ({ ctx, input }) => {
      const { data: invite, error } = await ctx.supabase
        .schema("core")
        .from("invites")
  ...
  acceptInvitation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        !["pending", "sent", "viewed"].includes(invite.status)
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
    ...
    .query(async ({ ctx, input }) => {
      ...
      (data ?? []).forEach((assignment) => {
        ...
        if (
          ...
          !entry.roles.includes(assignment.role.name)
        ) {
      ...
      if (input.roleNames?.length) {
        const roleSet = new Set(
          input.roleNames.map((role) => role.toLowerCase()),
        );
  ...
  createDocumentUploadSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: signedUrl, error: signedError } = await ctx.supabase
        .storage
  ...
  createDocumentDownloadUrl: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.versionId) {
        ...
        versionQuery = versionQuery.order("version_number", {
          ascending: false,
        }).limit(1);
      }
  ...
  shareDocument: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        input.shareType === "organization_member" &&
        !input.targetUserId
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
  ...
  getStorageUsageSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const warnings = policy.warningThresholds.filter((level) =>
        percentUsed >= level
      );
