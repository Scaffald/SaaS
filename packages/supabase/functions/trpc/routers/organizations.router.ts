...
export const organizationsRouter = t.router({
  ...
  updateSettings: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const payload = {
        ...
        default_currency: input.defaultCurrency ?? existing?.default_currency ?? 'USD',
        ...
        notification_preferences:
          input.notificationPreferences ?? existing?.notification_preferences ?? {},
        security_preferences: input.securityPreferences ?? existing?.security_preferences ?? {},
        privacy_preferences: input.privacyPreferences ?? existing?.privacy_preferences ?? {},
        enforce_mfa: input.enforceMfa ?? existing?.enforce_mfa ?? false,
        session_timeout_minutes:
          input.sessionTimeoutMinutes ?? existing?.session_timeout_minutes ?? 60,
  ...
  getStorageUsageSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const warnings = policy.warningThresholds.filter((level) => percentUsed >= level)
