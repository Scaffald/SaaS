...
async function ensureOrganizationAccess(
  ...
  const assignments = await loadUserRoleAssignments(ctx.supabaseAdmin, ctx.user.id)
  const superAdmin = isSuperAdmin(assignments)
  const orgAssignments = assignments.filter(
    (assignment) =>
      assignment.scope_org_id === organizationId && assignment.role?.scope === 'organization'
  )

  const isAdmin =
    superAdmin ||
    isOwner ||
    orgAssignments.some((assignment) => isOrganizationAdminRole(assignment.role?.name ?? null))
...
async function getStoragePolicy(ctx: Context, organizationId: string) {
  const [{ data: tier }, { data: override }] = await Promise.all([
    ctx.supabase
      ...
      .select('tier, max_storage_bytes, max_file_bytes, soft_warning_thresholds')
  ...
  const warningThresholds = tier?.soft_warning_thresholds ?? STORAGE_WARNING_LEVELS

  return {
    maxStorageBytes: override?.storage_bytes ?? tier?.max_storage_bytes ?? 10 * 1024 * 1024 * 1024,
    maxFileBytes: override?.file_bytes ?? tier?.max_file_bytes ?? 25 * 1024 * 1024,
    warningThresholds,
  }
}
...
async function incrementStorageUsage(
  ...
  const nextDocCount = existing.document_count + (options.incrementDocumentCount ? 1 : 0)
...
const shareDocumentSchema = z.object({
  ...
  shareType: z.enum(['organization_member', 'external']).default('organization_member'),
...
const locationUpsertSchema = z.object({
  ...
  name: z.string().min(1).max(255),
  locationType: z.enum(['headquarters', 'branch', 'job_site', 'remote', 'other']).default('other'),
  address: z
    .object({
...
export const organizationsRouter = t.router({
  ...
  createOrganizationRequest: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: existingOrganization, error: existingOrgError } = await ctx.supabase
        .schema('core')
        .from('organizations')
        .select('id')
   ...
   */
  listInvitations: protectedProcedure.input(listInvitationsSchema).query(async ({ ctx, input }) => {
  ...
  markInvitationViewed: t.procedure.input(viewInvitationSchema).mutation(async ({ ctx, input }) => {
    const { data: invite, error } = await ctx.supabase
      .schema('core')
      .from('invites')
  ...
  listMembers: protectedProcedure.input(listMembersSchema).query(async ({ ctx, input }) => {
    ...
    ;(data ?? []).forEach(
      (
        ...
        if (assignment.role?.name && !entry.roles.includes(assignment.role.name)) {
    ...
    if (input.roleNames?.length) {
      const roleSet = new Set(input.roleNames.map((role) => role.toLowerCase()))
      // biome-ignore lint/suspicious/noExplicitAny: Complex type inference from Supabase query
  ...
  createDocumentUploadSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const storagePath = `${input.organizationId}/${documentId}/${versionId}/${sanitizedFileName}`
      ...
      if (usage.storage_bytes + input.fileSize > policy.maxStorageBytes) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: "Uploading this document would exceed the organization's storage limit.",
        })
      }
  ...
  commitDocumentVersion: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (versionError || !version) {
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: `Failed to record document version: ${versionError?.message}`,
        })
      }
      ...
      await recordOrganizationAuditLog(ctx, input.organizationId, {
        ...
        description: `Uploaded version ${version.version_number} (${input.mimeType})`,
        metadata: {
  ...
  listDocuments: protectedProcedure.input(listDocumentsSchema).query(async ({ ctx, input }) => {
    await ensureOrganizationAccess(ctx, input.organizationId)
  ...
  getDocument: protectedProcedure.input(documentIdentifierSchema).query(async ({ ctx, input }) => {
    await ensureOrganizationAccess(ctx, input.organizationId)
  ...
  shareDocument: protectedProcedure.input(shareDocumentSchema).mutation(async ({ ctx, input }) => {
    await ensureOrganizationAccess(ctx, input.organizationId, {
      requireAdmin: true,
    })
  ...
  upsertFolder: protectedProcedure.input(folderUpsertSchema).mutation(async ({ ctx, input }) => {
    await ensureOrganizationAccess(ctx, input.organizationId, {
      requireAdmin: true,
    })
  ...
  archiveLocation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await recordOrganizationAuditLog(ctx, input.organizationId, {
        ...
        description: input.isActive ? 'Reactivated location' : 'Archived location',
      })
  ...
  updateSettings: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const payload = {
        ...
        default_currency: input.defaultCurrency ?? existingSettings?.default_currency ?? 'USD',
        ...
        notification_preferences:
          input.notificationPreferences ?? existingSettings?.notification_preferences ?? {},
        security_preferences:
          input.securityPreferences ?? existingSettings?.security_preferences ?? {},
        privacy_preferences:
          input.privacyPreferences ?? existingSettings?.privacy_preferences ?? {},
        enforce_mfa: input.enforceMfa ?? existingSettings?.enforce_mfa ?? false,
        session_timeout_minutes:
          input.sessionTimeoutMinutes ?? existingSettings?.session_timeout_minutes ?? 60,
  ...
  listAuditLog: protectedProcedure.input(auditLogListSchema).query(async ({ ctx, input }) => {
    await ensureOrganizationAccess(ctx, input.organizationId, {
      requireAdmin: true,
    })
  ...
  getStorageUsageSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const warnings = policy.warningThresholds.filter((level: number) => percentUsed >= level)
