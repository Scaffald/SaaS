...
async function upsertCertifications(
  ...
  const { error } = await supabase.schema('core').from('user_certifications').insert(payload)
        ...
        const skillArray = z
          .array(
            parsedSkillSchema.extend({
              csiMatch: z.object({ id: z.string(), code: z.string().nullable() }).optional(),
              onetMatch: z.object({ id: z.string(), title: z.string().nullable() }).optional(),
            })
          )
          .parse(input.data)
        await upsertSkills(
