...
export function normalizeOpenAiResumePayload(
  ...
  if ("employment" in record) {
    ...
    if (
      ...
      typeof employment === "string" && employment.trim().startsWith("{")
    ) {
      try {
...
async function downloadResumeFile(
  ...
  const { data, error } = await supabase.storage.from(RESUME_BUCKET_ID)
    .download(filePath);
...
async function matchSkillTaxonomies(
  ...
  return {
    ...
    csiMatch: csiTop
      ? {
        id: String(csiTop.id ?? ""),
        code: typeof csiTop.code_display === "string"
          ? csiTop.code_display
          : null,
    ...
    onetMatch: onetTop
      ? {
        id: typeof onetTop.onetsoc_code === "string"
          ? onetTop.onetsoc_code
          : "",
        title: typeof onetTop.title === "string" ? onetTop.title : null,
        description: typeof onetTop.description === "string"
          ? onetTop.description
          : null,
      }
      : undefined,
...
async function upsertProfileGeneral(
  ...
  if (data.first_name !== undefined || data.last_name !== undefined) {
    const displayName = [data.first_name ?? "", data.last_name ?? ""].join(" ")
      .trim();
    if (displayName) {
...
async function upsertExperienceEntries(
  ...
  const { error } = await supabase.schema("core").from("user_experience")
    .insert(payload);
...
async function upsertEducationEntries(
  ...
  const { error } = await supabase.schema("core").from("user_education").insert(
    payload,
  );
...
async function upsertSkills(
  ...
  const { error } = await supabase.schema("core").from("user_skills").insert(
    inserts,
  );
...
export const resumeRouter = t.router({
  ...
  parse: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const resume = await getResumeUploadForUser(
          supabase,
          input.resumeId,
          user.id,
        );
        ...
        const skillMatches = parsed.skills
          ? await Promise.all(
            parsed.skills.map(async (skill) =>
              await matchSkillTaxonomies(supabase, skill.name)
            ),
          )
          : [];
  ...
  saveSection: protectedProcedure.input(saveSectionInputSchema).mutation(
    async ({ ctx, input }) => {
      ...
      switch (input.section) {
        ...
        case "experience": {
          const experienceArray = z.array(parsedExperienceSchema).parse(
            input.data,
          );
          await upsertExperienceEntries(
            supabase,
            user.id,
            experienceArray,
            mergeStrategy,
        ...
        case "education": {
          const educationArray = z.array(parsedEducationSchema).parse(
            input.data,
          );
          await upsertEducationEntries(
            supabase,
            user.id,
            educationArray,
            mergeStrategy,
  ...
  hasUploaded: protectedProcedure.output(hasUploadedOutputSchema).query(
    async ({ ctx }) => {
      const { supabase, user } = ctx;
