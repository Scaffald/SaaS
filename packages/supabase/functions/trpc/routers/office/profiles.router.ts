...
export const officeProfilesRouter = t.router({
  getGhostProfiles: officeProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const completionScores = userIds.length > 0
      ...
      const items = paginatedRows.map((row) => {
        ...
        const lastReminder = uiPreferences.ghost_profile_last_reminder_at ??
        ...
        return {
          ...
          completionScore: scoreMap.get(row.user_id) ??
            uiPreferences.ghost_profile_completion_score ?? 0,
