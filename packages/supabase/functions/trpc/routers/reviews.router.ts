...
const reviewAnalyticsRowSchema = z.object({
  ...
  review_category_ratings: z.array(reviewCategoryRatingSchema).nullable().optional(),
  review_soft_skill_votes: z.array(reviewSoftSkillVoteSchema).nullable().optional(),
})
...
const updateCategoryRatingSchema = z.object({
  reviewId: z.string().uuid(),
  category: z.enum(['skills', 'reliability', 'collaboration', 'professionalism', 'technical']),
...
export const reviewsRouter = t.router({
  ...
  getSoftSkills: publicProcedure
    .input(
      z
        .object({
          category: z
            .enum(['reliability', 'collaboration', 'professionalism', 'technical'])
            .optional(),
        })
        .optional()
    )
    .query(async ({ ctx, input }) => {
      ...
      if (input?.category) {
        return data.filter((skill: { category: string }) => skill.category === input.category)
      }
  ...
  getSoftSkillsByCategory: publicProcedure.query(async ({ ctx }) => {
    ...
    const grouped = data.reduce(
      (acc: Record<string, unknown[]>, skill: { category: string; [key: string]: unknown }) => {
  ...
  createDraft: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (error) {
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: `Failed to create review draft: ${error.message || JSON.stringify(error)}`,
          cause: error,
        })
      }
   ...
   */
  updateStep: protectedProcedure.input(updateReviewStepSchema).mutation(async ({ ctx, input }) => {
    // Verify user owns this review
    const { data: review } = await ctx.supabase
      .schema('core')
      .from('reviews')
  ...
  updateCategoryRating: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { error } = await ctx.supabase.schema('core').from('review_category_ratings').upsert(
        {
          review_id: input.reviewId,
   ...
   */
  getBySubject: publicProcedure.input(getReviewsBySubjectSchema).query(async ({ ctx, input }) => {
    try {
      ...
      console.log('[reviews.getBySubject] Query successful', {
        ...
        sampleReview: data?.[0]
          ? {
              ...
              categoryRatingsLength: Array.isArray(data[0].review_category_ratings)
      ...
      const mappedReviews = (data || []).map(
        (review: {
          ...
          body: string | null
          created_at: string
          review_category_ratings?: Array<{ category: string; rating: number }> | null
        }) => {
          try {
            ...
            if (review.review_category_ratings) {
              if (Array.isArray(review.review_category_ratings)) {
                ...
                console.warn('[reviews.getBySubject] Unexpected category ratings format', {
                  reviewId: review.id,
  ...
  getReviewAnalytics: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const parsedReviewsResult = z.array(reviewAnalyticsRowSchema).safeParse(reviews ?? [])
      ...
      const strengthTags = new Map<string, { name: string; count: number; category: string }>()
      const improvementTags = new Map<string, { name: string; count: number; category: string }>()
      for (const review of parsedReviews) {
      ...
      for (const review of parsedReviews) {
        ...
        const avgCategoryRating =
          reviewCategories.length > 0
            ? reviewCategories.reduce((sum, cr) => sum + cr.rating, 0) / reviewCategories.length
            : 0
      ...
      return {
        overall: {
          ...
          recommendCount,
          notRecommendCount,
          recommendPercentage: totalReviews > 0 ? (recommendCount / totalReviews) * 100 : 0,
        },
        ...
        categories: Array.from(categoryRatings.entries()).map(([category, data]) => ({
          category,
          averageRating: data.total / data.count,
          frequency: data.count,
        })),
        ...
        timeline: Array.from(timelineData.values()).sort((a, b) => a.month.localeCompare(b.month)),
      }
    }),
