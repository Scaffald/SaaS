...
const typeOverridesSchema = z.record(
  z.string(),
  z.object({
...
const preferencesUpsertSchema = z.object({
  ...
  digestFrequency: z.enum(["immediate", "digest_daily", "digest_weekly", "mute"]).default("immediate"),
...
function buildListQuery(ctx: { supabase: any; user: { id: string } }, input: z.infer<typeof listInputSchema>) {
...
export const notificationsRouter = t.router({
  ...
  markAsUnread: protectedProcedure.input(z.object({ id: z.string().uuid() })).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx;
    ...
    digestQueue: officeProcedure
      ...
      .query(async ({ ctx, input }) => {
        const { data, error } = await ctx.supabase
          ...
          .select("id, user_id, type, bucket, count, channels, examples, last_event_at, processed_at, updated_at")
