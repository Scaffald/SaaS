    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const { data: existing, error: fetchError } = await supabase
          .schema('core')
          .from('preferences')
          .select('riasec_scores, current_occupation_code, target_occupation_codes')
          .eq('user_id', user.id)
          .single()
        ...
        if (input.current_occupation_code !== undefined) {
          updateData.current_occupation_code = input.current_occupation_code || null
        ...
        if (input.target_occupation_codes !== undefined) {
          ...
          updateData.target_occupation_codes = existing.target_occupation_codes || []
        }
        ...
        const { error } = await supabase.schema('core').from('preferences').upsert(updateData)
