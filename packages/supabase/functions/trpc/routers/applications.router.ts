...
export const applicationsRouter = router({
   ...
   * - Set is_complete: true
   */
  submit: publicProcedure.input(applicationSubmitSchema).mutation(async ({ ctx, input }) => {
    ...
    const attachmentsCount = Object.keys(application.attachment_metadata || {}).length
   ...
   */
  getUploadUrl: publicProcedure.input(fileUploadSchema).mutation(async ({ ctx, input }) => {
    const { supabase, user } = ctx
