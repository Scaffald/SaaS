...
export class GoogleDriveStorageBackend implements IStorageBackend {
  ...
  private async loadAccessToken(): Promise<string | null> {
    ...
    if (error || !data) {
      console.warn(
        "[GoogleDriveBackend] No Google Drive token found for user:",
        this.userId,
      );
      return null;
    }
  ...
  private async refreshToken(refreshToken: string): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          "[GoogleDriveBackend] Token refresh failed:",
          await response.text(),
        );
        return null;
      }
  ...
  async upload(
    ...
    try {
      ...
      const filePart =
        `${delimiter}Content-Type: ${options.contentType}\r\nContent-Transfer-Encoding: base64\r\n\r\n`;
  ...
  async getSignedUrl(
    ...
    try {
      ...
      throw {
        code: "URL_GENERATION_FAILED",
        message: `Google Drive link generation failed: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
        originalError: error,
      };
