...
const BASE_TEAM_ROLES: ReadonlyArray<{
...
}> = [
  {
    ...
    name: "Team Admin",
    description:
      "Full access to manage the team, members, roles, and invitations.",
  },
  {
    key: "lead",
    name: "Team Lead",
    description:
      "Manage day-to-day team operations and collaborate on hiring activities.",
  },
  {
    key: "recruiter",
    name: "Recruiter",
    description:
      "Manage applications, communication, and scheduling with candidates.",
  },
  {
    key: "reviewer",
    name: "Reviewer",
  ...
  },
  {
    ...
    name: "Member",
    description:
      "Collaborate on evaluations with read access to shared resources.",
  },
];
...
async function callEdgeFunction(path: string, payload: unknown) {
  ...
  if (!baseUrl || !serviceKey) {
    console.warn(
      `[teams] Skipping ${path} call: missing Supabase function configuration.`,
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[teams] ${path} responded with status ${response.status}`,
        data,
      );
      return null;
    }
...
function transformTeam(record: Record<string, unknown>) {
  ...
  return {
    ...
    analyticsMetadata: (record.analytics_metadata as Record<string, unknown>) ??
      {},
    analyticsLastRefreshedAt: (record.analytics_last_refreshed_at as string) ??
      null,
    analyticsRefreshIntervalMinutes: Number(
      record.analytics_refresh_interval_minutes ?? 60,
    ),
    ...
    team: team
      ? {
        ...
        organizationName: organization
          ? ((organization.name as string) ?? null)
          : null,
      }
      : null,
  };
...
async function fetchTeamOrThrow(
  ...
  const { data, error } = await supabaseAdmin
    ...
    .from("teams")
    .select(
      "id, name, organization_id, default_role_id, is_archived, analytics_last_refreshed_at",
    )
    .eq("id", teamId)
    .single();
    ...
    if (permissionInserts.length > 0) {
      ...
      if (permissionInsertError) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            `Failed to seed team role permissions: ${permissionInsertError.message}`,
        });
      }
    }
  }
...
async function resolveRoleId(options: {
  ...
  if (options.roleKey) {
    const roles = await ensureOrganizationRoles(
...
async function ensureOrganizationTeamPermission({
  ...
  if (!result.allowed) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message:
        "You do not have permission to manage teams for this organization.",
    });
  }
}
  ...
  return t.router({
    ...
    list: procedure.input(z.object({ teamId: teamIdSchema })).query(
      async ({ ctx, input }) => {
        ...
        return {
          members: (data ?? []).map((record: Record<string, unknown>) =>
    ...
    update: procedure.input(teamMemberUpdateSchema).mutation(
      async ({ ctx, input }) => {
        ...
        if (existingError || !existingMember) {
          throw new TRPCError({
            code: existingError?.code === "PGRST116"
              ? "NOT_FOUND"
        ...
        if (statusChanged) {
          ...
          if (newStatus === "removed") {
            ...
            newStatus === "active" && existingMember.status === "removed"
          ) {
            await recordTeamAuditLog({
              supabaseAdmin,
              teamId,
    ...
    remove: procedure.input(teamMemberRemoveSchema).mutation(
      async ({ ctx, input }) => {
        ...
        if (memberError || !existingMember) {
          throw new TRPCError({
            code: memberError?.code === "PGRST116"
              ? "NOT_FOUND"
              : "INTERNAL_SERVER_ERROR",
            message: memberError
    ...
    transferOwnership: procedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const metadata =
          (targetMember.metadata as Record<string, unknown> | null) ?? {};
        ...
        if (input.notify) {
          await publishTeamNotification({
            id:
              `team-ownership:${input.teamId}:${input.memberId}:${Date.now()}`,
            type: "team.ownership_transferred",
            severity: "info",
            title: "Team ownership updated",
            message:
              "You have been granted ownership permissions for this team.",
    ...
    selfRemove: procedure.input(teamMemberSelfRemovalSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, dbAdmin, user } = ctx;
      ...
      async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              `Invitation cannot be resent while in status "${invitation.status}".`,
          });
        }
        ...
        const rawToken = generateInvitationToken();
        const tokenHash = await hashInvitationToken(rawToken);
        const currentExpiry = invitation.expires_at
          ? new Date(invitation.expires_at as string)
          : null;
        const suggestedExpiry = addDays(
          new Date(),
          TEAM_INVITATION_TTL_DEFAULT,
        );
        const nextExpiry = currentExpiry && currentExpiry > suggestedExpiry
          ? currentExpiry
          : suggestedExpiry;
    ...
    respond: procedure.input(teamInvitationUserRespondSchema).mutation(
      async ({ ctx, input }) => {
        ...
        if (error || !invitation) {
          throw new TRPCError({
            code: error?.code === "PGRST116"
              ...
              : "INTERNAL_SERVER_ERROR",
            message: error
              ? `Invitation not found: ${error.message}`
              : "Invitation not found",
          });
        }
        ...
        const invitationEmail = invitation.email
          ...
          : null;
        const matchesInvitedUser = invitation.invited_user_id === user.id ||
          (invitationEmail && normalizedUserEmail &&
            invitationEmail === normalizedUserEmail);
...
function buildAnalyticsRouter(procedure: AuthenticatedProcedure) {
  return t.router({
    overview: procedure.input(teamAnalyticsOverviewInputSchema).query(
      async ({ ctx, input }) => {
        ...
        const metrics = (data ?? []).map((
          ...
          timeToFirstReview: {
            averageSeconds: record.avg_time_to_first_review_seconds as
              | number
              | null,
          ...
          workloadPressureScore: record.workload_pressure_score as
            | number
            | null,
        ...
        return {
          metrics,
          refreshedAt: (team.analytics_last_refreshed_at as string | null) ??
            null,
        };
      },
    ),
    ...
    activity: procedure.input(teamAnalyticsActivityFeedSchema).query(
      async ({ ctx, input }) => {
        const { supabaseAdmin } = ctx;
        ...
        return {
          events,
          nextCursor: events.length === input.pageSize
          ...
          (
            ...
            return {
              ...
              mentions: Array.isArray(payload.mentions)
                ? (payload.mentions as string[]).map((mention) =>
                  String(mention)
                )
                : [],
              occurredAt: record.occurred_at as string,
    ...
    postComment: procedure.input(teamActivityCommentSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, user } = ctx;
...
function buildJobAssignmentsRouter(procedure: AuthenticatedProcedure) {
  return t.router({
    list: procedure.input(teamJobAssignmentListSchema).query(
      async ({ ctx, input }) => {
        const { supabaseAdmin } = ctx;
    ...
    assign: procedure.input(teamJobAssignmentCreateSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, dbAdmin, user } = ctx;
        ...
        if (error || !data) {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: error
              ? `Failed to assign team: ${error.message}`
    ...
    update: procedure.input(teamJobAssignmentUpdateSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, dbAdmin, user } = ctx;
    ...
    remove: procedure.input(teamJobAssignmentDeleteSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, dbAdmin, user } = ctx;
  ...
  return t.router({
    list: procedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const { supabaseServiceKey, supabaseUrl } = await import(
          "../context.ts"
        );
        ...
        const { data: ownedOrganizations, error: ownedError } =
          await supabaseAdmin
            .schema("core")
            .from("organizations")
            .select("id")
        ...
        if (ownedError) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message:
              `Failed to load owned organizations: ${ownedError.message}`,
          });
        }
        ...
        for (const membership of teamMemberships ?? []) {
          ...
          if (organizationId) {
            accessibleOrganizationIds.add(organizationId);
            const set = memberTeamsByOrganization.get(organizationId) ??
              new Set<string>();
            if (teamId) {
              set.add(teamId);
            }
        ...
        if (input.organizationId) {
          if (superAdmin || adminOrganizationIds.has(input.organizationId)) {
            ...
            const memberTeams = memberTeamsByOrganization.get(
              input.organizationId,
            );
          ...
          if (memberTeamIds.size > 0) {
            const nonAdminTeamIds = Array.from(memberTeamIds).filter(
              (teamId) => {
    ...
    create: procedure.input(teamCreateSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, user } = ctx;
        ...
        const insertPayload = {
          ...
          analytics_refresh_interval_minutes:
            input.analyticsRefreshIntervalMinutes ?? 60,
    ...
    update: procedure.input(teamUpdateSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, user } = ctx;
        ...
        if (input.description !== undefined) {updates.description =
        ...
        if (input.autoAssignJobs !== undefined) {updates.auto_assign_jobs =
            input.autoAssignJobs;}
        ...
        if (input.analyticsRefreshIntervalMinutes !== undefined) {
          updates.analytics_refresh_interval_minutes =
            input.analyticsRefreshIntervalMinutes;
        }
    ...
    archive: procedure.input(teamArchiveSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, user } = ctx;
        ...
        const metadata =
          (teamRecord.metadata as Record<string, unknown> | null) ?? {};
        if (input.reason) {
    ...
    respondToInvitation: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const responderId = input.responderId ??
          (invitation.invited_user_id as string | null);
        if (!responderId) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "A responder user ID is required to process the invitation",
          });
        }
...
function buildApplicationAssignmentsRouter(procedure: AuthenticatedProcedure) {
  return t.router({
    assign: procedure.input(teamApplicationAssignmentSchema).mutation(
      async ({ ctx, input }) => {
        const { supabaseAdmin, user } = ctx;
        ...
        if (applicationError || !applicationRecord) {
          throw new TRPCError({
            code: applicationError?.code === "PGRST116"
              ? "NOT_FOUND"
              : "INTERNAL_SERVER_ERROR",
        ...
        if (jobTeamsError) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message:
              `Failed to load job-team assignments: ${jobTeamsError.message}`,
          });
        }
