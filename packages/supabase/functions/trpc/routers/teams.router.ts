...
function buildMembersRouter(procedure: AuthenticatedProcedure) {
  return t.router({
    ...
    list: procedure.input(z.object({ teamId: teamIdSchema })).query(async ({ ctx, input }) => {
      const { supabaseAdmin } = ctx
...
function buildTeamsRouter(procedure: AuthenticatedProcedure) {
  ...
  return t.router({
      ...
      .query(async ({ ctx, input }) => {
        ...
        const fetchTeamsByOrgIds = async (organizationIds: string[]) => {
          ...
          for (const record of data ?? []) {
            teamsMap.set(record.id as string, transformTeam(record as Record<string, any>))
          }
        }
    ...
    byId: procedure.input(z.object({ teamId: teamIdSchema })).query(async ({ ctx, input }) => {
      const { supabaseAdmin } = ctx
...
function buildApplicationAssignmentsRouter(procedure: AuthenticatedProcedure) {
  return t.router({
    assign: procedure.input(teamApplicationAssignmentSchema).mutation(async ({ ctx, input }) => {
      ...
      const teamIds = new Set((jobTeams ?? []).map((record) => record.team_id as string))
