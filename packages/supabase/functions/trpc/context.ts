...
const supabaseUrl =
  resolvedSupabaseUrl.length > 0 ? resolvedSupabaseUrl : DEFAULT_LOCAL_SUPABASE_URL
...
export const createTRPCContext = async (opts: { req: Request }) => {
  ...
  if (authorizationHeader) {
    ...
    if (!authorizationHeader.startsWith('Bearer ')) {
      console.error('Invalid authorization header format:', authorizationHeader.substring(0, 20))
      ...
      if (!token || token.length === 0) {
        ...
        if (isAnonKey) {
          console.log('Anon key detected - treating as public/unauthenticated request')
        } else {
