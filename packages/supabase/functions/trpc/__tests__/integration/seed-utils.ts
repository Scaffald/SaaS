...
async function createTeamRecord(
...
): Promise<{ id: string; slug: string; name: string; defaultRoleId: string; defaultRoleKey: string }> {
  const {
    client,
    organizationId,
    createdBy,
  ...
  if (error || !data) {
    throw new Error(
      `Failed to create team fixture: ${error?.message ?? "No record returned"}`,
    );
  }
...
async function upsertTeamMemberRecord(params: TeamMemberParams): Promise<void> {
  ...
  if (error) {
    throw new Error(
      `Failed to upsert team member fixture: ${error.message}`,
    );
  }
}
...
export async function setupTeamManagementFixture(
  ...
  const organization = await createOrganizationRecord({
    ...
    name:
      ...
      `Integration Org ${crypto.randomUUID().slice(0, 8)}`,
    visibility: options.organizationVisibility ?? "private",
  });

  const organizationTeamRoles = await ensureOrganizationTeamRoles(
    adminClient,
    organization.id,
  );

  const normalizedDefaultKey = normalizeTeamRoleKey(
    options.defaultTeamRoleKey ?? "member",
  );
  ...
  if (!ownerTeamRole) {
    throw new Error(
      'Organization team role "admin" is required for team management fixtures.',
    );
  }
  ...
  if (!memberRole) {
    throw new Error(
      'Organization team role "member" is required for team management fixtures.',
    );
  }
  ...
  const team = await createTeamRecord({
    ...
    name:
      options.teamName ?? `Integration Team ${crypto.randomUUID().slice(0, 6)}`,
    defaultRole,
