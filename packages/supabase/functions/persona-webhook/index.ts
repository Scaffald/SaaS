...
import {
  addMonths,
  mergeMetadata,
  patchPersonaMetadata,
} from "../_shared/id-verification-utils.ts";
...
function extractInquiryId(event: PersonaEvent): string | null {
  const relationshipId =
    event.data.relationships?.inquiry?.data && typeof event.data.relationships.inquiry.data === "object"
...
function determineTransition(event: PersonaEvent, status: string | null): "activate" | "expire" | "revoke" | null {
...
async function handleEvent(event: PersonaEvent): Promise<Response> {
  ...
  if (!inquiryId) {
    return new Response(
      JSON.stringify({ ok: true, ignored: "missing_inquiry" }),
  ...
  if (!verification) {
    ...
    return new Response(
      JSON.stringify({ ok: true, ignored: "unknown_inquiry" }),
      { status: 202, headers: { "Content-Type": "application/json", ...corsHeaders } },
    );
  }
  ...
  if (transition === "activate") {
    ...
    updates.badge_expires_at = event.data.attributes?.expired_at
      ?? verification.badge_expires_at
      ?? nowIso;
  } else if (transition === "revoke") {
    updates.badge_status = "revoked";
    updates.revoked_at = nowIso;
    updates.revocation_reason =
      event.data.attributes?.status_reason ??
      `persona:${event.data.type}`;
  } else {
    // Still update metadata/persona status even if no badge transition.
  }

  const mergedMetadata = mergeMetadata(verification.metadata, updates.metadata as Record<string, unknown>);
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: (error as Error).message ?? "internal_error" }),
      { status, headers: { "Content-Type": "application/json", ...corsHeaders } },
    );
  }
});
