...
serve(async (req) => {
  ...
  if (fetchError) {
    console.error('[feedback-to-braingrid] Failed to load feedback', {
      feedbackId,
      error: fetchError.message,
    })
    return new Response(JSON.stringify({ error: 'Failed to load feedback submission' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
  ...
  if (!feedback) {
    return new Response(JSON.stringify({ error: 'Feedback submission not found' }), {
      status: 404,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Network error communicating with Braingrid' }), {
      status: 502,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
