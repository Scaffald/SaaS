...
function markReminderSent(
  ...
  const expiration = isRecord(reminders.expiration)
...
serve(async (req) => {
  ...
  if (error) {
    ...
    return new Response(
      JSON.stringify({ error: "failed_to_fetch_checks" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      },
  ...
  for (const row of rows) {
    ...
    if (Number.isNaN(expiresAt.valueOf())) {
      summary.errors.push(
        `invalid_expires_at:${row.id}:${row.expires_at}`,
      );
      continue;
    }
    ...
    for (const window of REMINDER_WINDOWS) {
      ...
      try {
        ...
        const updatedMetadata = markReminderSent(
          row.metadata,
          window.key,
        ...
        console.error(
          "[notify-background-check-expiration] reminder error",
          error,
        );
