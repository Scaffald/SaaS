...
serve(async (req) => {
  ...
  for (const recipientId of event.recipients) {
    try {
      ...
      const { resolved: preferences } = await getUserPreferences(supabase, recipientId)
      ...
      const nextAttempt = event.sendAfter ? new Date(event.sendAfter) : null
      ...
      if (digestChannels.length > 0) {
        const bucket = getDigestBucket(routingPlan.effectiveFrequency, now, timezone)
