...
function isWithinQuietHours(now: Date, quietHours: QuietHours | null, timezone: string): boolean {
...
export function calculateNextAttempt(attempts: number): Date | null {
...
export async function upsertDigestQueue(
  ...
  if (!data) {
    ...
    if (insertError) {
      throw new Error(`Failed to insert digest queue entry: ${insertError.message}`)
    }
  ...
  if (updateError) {
    throw new Error(`Failed to update digest queue entry: ${updateError.message}`)
  }
}
...
export async function getDeviceTokens(
  ...
  const devices = (data ?? []) as Array<Database['core']['Tables']['notification_devices']['Row']>
...
export function mergeChannelSets(plan: RoutingPlan): NotificationChannel[] {
  return ensureChannelArray([...plan.immediate, ...plan.digest])
}
