...
function defaultDigestTitle(frequency: NotificationFrequency, count: number): string {
...
export async function processDigestQueue(
  ...
  for (const entry of entries) {
    ...
    const { resolved: preferences } = await getUserPreferences(supabase, entry.user_id)
    ...
    const permittedChannels = availableChannels.filter((channel) => {
      if (channel === 'email')
        return Boolean(contacts.email) && (preferences.channelEnabled.email ?? true)
      if (channel === 'sms')
        return Boolean(contacts.phone) && (preferences.channelEnabled.sms ?? false)
    ...
    const notification = await insertNotification(supabase, {
      ...
      routed_channels: ensureChannelArray([...permittedChannels, 'in_app']),
    })
