...
function buildEmailPayload(
  ...
  const htmlBody = typeof metadata.html === 'string' ? metadata.html : `<p>${messageFallback}</p>`

  const textBody = typeof metadata.text === 'string' ? metadata.text : messageFallback
  ...
  if (templateId) {
    ...
    payload.subject = typeof metadata.subject === 'string' ? metadata.subject : notificationTitle
    payload.content = [
...
export const emailAdapter: ChannelAdapter = {
  async send({ delivery, notification }) {
    ...
    const payload = buildEmailPayload(metadata, notification.title, bodyPayload, messageFallback)
