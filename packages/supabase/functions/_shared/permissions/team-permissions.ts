...
export function isSuperAdmin(assignments: TeamPermissionAssignment[]): boolean {
  return assignments.some(
    (assignment) => assignment.role?.scope === 'platform' && assignment.role?.name === 'super_admin'
  )
}
...
export async function checkTeamPermission({
  ...
  const resolvedOrganizationId = organizationId ?? teamRecord?.organization_id ?? null
  ...
  const hasOrganizationAdminRole = assignments.some(
    (assignment) =>
