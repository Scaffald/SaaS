...
export async function notifyBackgroundCheckExpirationReminder(
  ...
  if (Number.isNaN(expiresDate.valueOf())) {
    console.warn(
      "[background-checks] skip expiration reminder: invalid expires_at",
      { checkId, expiresAt },
    );
    return;
  }
  ...
  const subject =
    windowDays === 30
      ? "Background Check Expires in 30 Days"
      : "Background Check Expires in 7 Days";
  const dedupeBase = `bgcheck:${checkId}:expiration:${windowDays}`;
  const reminderStatusTag =
    windowDays === 30 ? "expiration_30_day" : "expiration_7_day";
  ...
  try {
    ...
    if (workerNotification && workerEmail) {
      try {
        ...
        console.error(
          "[background-checks] failed to queue worker expiration reminder email",
          error,
        );
      }
    }
...
function composeEmailContent(
  ...
  const subject =
    audience === "worker"
      ? template.workerTitle
      : template.requesterTitle ?? template.workerTitle;
  const baseMessage =
    audience === "worker"
  ...
  if (ADVERSE_NOTICE_STATUSES.has(status) && audience === "worker") {
    ...
    lines.push(
      `Summary of Rights under the FCRA: ${SUMMARY_OF_RIGHTS_URL}`,
    );
    lines.push(
  ...
  const needsSummaryOfRights =
    ADVERSE_NOTICE_STATUSES.has(status) && audience === "worker";
...
export async function notifyBackgroundCheckStatusChange(
  ...
  if (WORKER_EMAIL_STATUSES.has(status)) {
    try {
      ...
      console.error(
        "[background-checks] failed to load worker contact details",
        error,
      );
    }
  }
  ...
  const workerChannels = buildNotificationChannels(Boolean(workerEmail));
  const workerSummaryUrl = getString(
    workerEmailContent?.templateData.summaryOfRightsUrl,
  );
    ...
    try {
      ...
      if (notification && workerEmail && workerEmailContent) {
        try {
          ...
          console.error(
            "[background-checks] failed to queue worker email notification",
            error,
          );
        }
      }
  ...
  if (requesterId && requesterId !== workerId) {
    ...
    if (REQUESTER_EMAIL_STATUSES.has(status)) {
      try {
        ...
        console.error(
          "[background-checks] failed to load requester contact details",
          error,
        );
      }
    }
    ...
    const requesterChannels = buildNotificationChannels(
      Boolean(requesterEmail),
    );
    const requesterSummaryUrl = getString(
      requesterEmailContent?.templateData.summaryOfRightsUrl,
    );
      ...
      try {
        ...
        if (notification && requesterEmail && requesterEmailContent) {
          try {
            ...
            console.error(
              "[background-checks] failed to queue requester email notification",
              error,
            );
          }
        }
...
export async function notifyBackgroundCheckInvitation(
  ...
  try {
    ...
    console.error(
      "[background-checks] failed to load invitee contact details",
      error,
    );
  }

  const workerChannels = buildNotificationChannels(Boolean(workerEmail));
  const workerEmailMessage = getString(
    workerEmailContent?.templateData.message,
  );
  ...
  try {
    ...
    if (workerNotification && workerEmail && workerEmailContent) {
      try {
        ...
        console.error(
          "[background-checks] failed to queue invitation email",
          error,
        );
      }
    }
  ...
  if (invitedById && invitedById !== workerId) {
    ...
    const requesterEmailContent = composeEmailContent(
      status,
      requesterCtx,
      "requester",
    );
    ...
    try {
      ...
      console.error(
        "[background-checks] failed to load inviter contact details",
        error,
      );
    }

    const inviterChannels = buildNotificationChannels(Boolean(inviterEmail));
    const inviterEmailMessage = getString(
      requesterEmailContent?.templateData.message,
    );
    ...
    try {
      ...
      if (inviterNotification && inviterEmail && requesterEmailContent) {
        try {
          ...
          console.error(
            "[background-checks] failed to queue invitation confirmation email",
            error,
          );
        }
      }
