...
export const buildWorkLogCsv = (snapshot: WorkLogExportSnapshot): string => {
  const rows: Array<[string, string]> = [
    ...
    ['Tasks Completed', snapshot.tasks.length ? snapshot.tasks.join('; ') : 'None'],
    ['Skills Used', snapshot.skills.length ? snapshot.skills.join('; ') : 'None'],
    [
      'Collaborators',
...
export const buildWorkLogPdf = async (snapshot: WorkLogExportSnapshot): Promise<Uint8Array> => {
  ...
  drawLines(
    snapshot.timeEntries.length
      ? snapshot.timeEntries.flatMap((entry, index) =>
          wrapText(
            `${index + 1}. ${entry.start} - ${entry.end}  •  ${formatHours(
              entry.durationHours
