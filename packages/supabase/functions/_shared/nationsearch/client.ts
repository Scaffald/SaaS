...
async function verifyCertificateFingerprint(baseUrl: string): Promise<void> {
  ...
  try {
    ...
    const denoGlobal = (
      globalThis as {
        Deno?: { connectTls?: (options: { hostname: string; port: number }) => Promise<unknown> }
      }
    ).Deno
    if (typeof denoGlobal?.connectTls !== 'function') {
      console.warn(
        '[nationsearch] TLS validation unavailable in this runtime; skipping pin verification'
      )
      return
    }
    ...
    try {
      ...
      const certificate =
        typeof tlsConn.getCertificate === 'function' ? tlsConn.getCertificate() : null

      if (!certificate?.rawDER) {
        console.warn(
          '[nationsearch] TLS certificate information unavailable; skipping pin verification'
        )
        return
      }
...
async function requestWithRetry(ctx: RequestContext): Promise<unknown> {
  ...
  const bodyString = body ? (typeof body === 'string' ? body : JSON.stringify(body)) : undefined
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message =
        payload?.message ?? `NationSearch request failed with status ${response.status}`
    ...
    if (attempt < MAX_RETRIES && shouldRetry(error)) {
      const backoffMs = timeoutMs * 2 ** attempt
...
function getBaseConfig(): Required<
  Pick<NationSearchClientOptions, 'apiKey' | 'apiSecret' | 'baseUrl'>
> {
