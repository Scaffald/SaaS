...
export function createServiceClient() {
  ...
  if (!supabaseUrl || !serviceKey) {
    console.error('[feedback-sync] Missing Supabase environment configuration', {
...
export function buildBraingridPayload(feedback: FeedbackRow, projectId?: string) {
  const titleSnippet = feedback.feedback_text.replace(/\s+/g, ' ').trim().slice(0, 50)
  ...
  return {
    ...
    description: [
      ...
      feedback.screenshot_path ? `Screenshot: ${feedback.screenshot_path}` : null,
    ]
