...
const eventToStatusMap: Record<string, { status?: string; event: "accepted" | "delivered" | "opened" | "clicked" | "failed"; severity?: "error" | "info" } > = {
...
serve(async (req) => {
  ...
  for (const event of payload) {
    ...
    if (!messageId || !eventName) {
      handled.push({ messageId: messageId ?? "unknown", event: eventName ?? "unknown", error: "Missing message id or event" });
      continue;
    }
    ...
    if (mapping.status && isValidDeliveryStatus(mapping.status)) {
      ...
      updates.last_error = mapping.severity === "error"
