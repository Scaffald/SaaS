...
const handler = async (req: Request) => {
  ...
  try {
    ...
    if (!response.ok) {
      return new Response(
        JSON.stringify({ error: `Feed request failed with status ${response.status}` }),
        {
          status: 502,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    return new Response(JSON.stringify({ articles }), {
      ...
      headers: {
        ...
        'Cache-Control': `s-maxage=${CACHE_TTL_SECONDS}, stale-while-revalidate=${STALE_WHILE_REVALIDATE_SECONDS}`,
    ...
    return new Response(JSON.stringify({ error: 'Unable to reach RSS source' }), {
      status: 502,
      headers: { 'Content-Type': 'application/json' },
    })
  }
