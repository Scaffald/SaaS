...
async function updateComponentStatuses(
  ...
  for (const update of updates) {
    ...
    const merged = {
      ...
      ...update,
      completed_at: update.completed_at ?? components[index]?.completed_at ??
        null,
...
async function handleEvent(
  ...
  if (fetchError) {
    console.error(
      "[background-check-webhook] failed to load background check",
      fetchError,
    );
  ...
  if (!check) {
    console.warn(
      "[background-check-webhook] ignoring webhook for unknown check",
      {
        providerCheckId,
        eventId: event.id,
      },
    ...
    return new Response(
      JSON.stringify({ ok: true, ignored: "unknown_check" }),
      {
        status: 202,
  ...
  const candidateStatus = mapProviderStatus(event.data.status ?? null) ??
    (eventStatusDefaults[event.type]
      ? mapProviderStatus(eventStatusDefaults[event.type])
      : null);
  ...
  if (
    ...
    updates.component_statuses = await updateComponentStatuses(
      typedCheck.component_statuses,
      event,
    );
    shouldRecordHistory = true;
  }
  ...
  if (event.data.estimated_completion_date !== undefined) {
    updates.estimated_completion_date = event.data.estimated_completion_date ??
      null;
    shouldRecordHistory = true;
  }

  const previousReference =
    typedCheck.provider_reference &&
      typeof typedCheck.provider_reference === "object"
      ? (typedCheck.provider_reference as Record<string, unknown>)
      : {};
  const previousNationSearch =
    previousReference?.nationsearch &&
      typeof previousReference.nationsearch === "object"
  ...
  if (shouldRecordHistory) {
    const historyEntry: Record<string, unknown> = {
      status: (updates.status as BackgroundCheckStatus | undefined) ??
        typedCheck.status,
      occurred_at: occurredAt,
      actor: "provider",
      event: event.type,
      provider_status: event.data.status ?? eventStatusDefaults[event.type] ??
        null,
    };
  ...
  if (updateError) {
    console.error(
      "[background-check-webhook] failed to update check",
      updateError,
    );
  ...
  const refreshed =
    (updated ?? { ...typedCheck, ...updates }) as BackgroundCheckRecord;
  ...
  if (refreshed.status !== typedCheck.status) {
    await notifyBackgroundCheckStatusChange({
      ...
      packageName: refreshed.package?.display_name ?? refreshed.package?.slug ??
        null,
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: (error as Error).message ?? "invalid_request" }),
      {
        status,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      },
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return new Response(
        JSON.stringify({ error: "invalid_payload", issues: error.issues }),
        {
          status: 400,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        },
  ...
  const idempotencyKey = event.idempotency_key ??
    req.headers.get(IDEMPOTENCY_HEADER) ?? `${event.type}:${event.id}`;
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: (error as Error).message ?? "unhandled_error" }),
      {
        status,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      },
