...
import { createServiceClient, updateFeedbackStatus } from '../_shared/feedback-sync.ts'
...
serve(async (req) => {
  ...
  const limit =
    parseResult.success && parseResult.data?.limit ? parseResult.data.limit : DEFAULT_BATCH_SIZE
  const statuses =
    parseResult.success && parseResult.data?.statuses
