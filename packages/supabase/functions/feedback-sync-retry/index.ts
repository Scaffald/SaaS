...
const DEFAULT_BATCH_SIZE = Number(
  Deno.env.get("FEEDBACK_SYNC_RETRY_BATCH") ?? "10",
);
...
serve(async (req) => {
  ...
  const parseResult = body ? requestSchema.safeParse(body) : { success: true, data: {} };
  ...
  const limit = parseResult.success && parseResult.data?.limit
    ? parseResult.data.limit
    : DEFAULT_BATCH_SIZE;
  const statuses = parseResult.success && parseResult.data?.statuses
    ? parseResult.data.statuses
    : ["pending", "failed"];
  ...
  if (error) {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to load feedback submissions" }),
  ...
  for (const candidate of pending) {
    const { id } = candidate;
    try {
      const { data: syncResult, error: invokeError } = await supabase.functions
        .invoke("feedback-to-braingrid", {
          body: {
            feedbackId: id,
            trigger: "retry",
          },
