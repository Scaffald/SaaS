...
import {
  notifyIdVerificationExpirationReminder,
} from "../_shared/id-verification-notifications.ts";
import {
...
async function processReminderWindow(
  ...
  for (const row of rows) {
    ...
    if (updateError) {
      console.error(
        "[notify-id-verification-expiration] failed to persist reminder marker",
        { id: row.id, window, error: updateError },
      );
    } else {
      processed += 1;
    }
...
async function expireOverdueBadges(
  ...
  for (const row of rows) {
    ...
    if (updateError) {
      console.error(
        "[notify-id-verification-expiration] failed to mark badge expired",
