...
async function runTests() {
  ...
  try {
    ...
    if (!testKey.startsWith('sk_test_')) {
      throw new Error(`Expected test key to start with sk_test_, got ${testKey}`)
    }

    if (!liveKey.startsWith('sk_live_')) {
      throw new Error(`Expected live key to start with sk_live_, got ${liveKey}`)
    }
    ...
    if (testKeyParts.length !== 3) {
      throw new Error(`Expected test key to have 3 parts, got ${testKeyParts.length}`)
    }

    if (liveKeyParts.length !== 3) {
      throw new Error(`Expected live key to have 3 parts, got ${liveKeyParts.length}`)
    }
  ...
  try {
    ...
    if (validateApiKeyFormat(invalidKeyWrongEnv)) {
      throw new Error('Key with wrong environment should have failed validation')
    }
  ...
  try {
    ...
    if (RATE_LIMITS.free.requestsPerMinute !== 100) {
      throw new Error(`Free tier should be 100 req/min, got ${RATE_LIMITS.free.requestsPerMinute}`)
    }

    if (RATE_LIMITS.pro.requestsPerMinute !== 1000) {
      throw new Error(`Pro tier should be 1000 req/min, got ${RATE_LIMITS.pro.requestsPerMinute}`)
    }

    if (RATE_LIMITS.enterprise.requestsPerMinute !== 10000) {
      throw new Error(`Enterprise tier should be 10000 req/min, got ${RATE_LIMITS.enterprise.requestsPerMinute}`)
    }
    ...
    console.log(`✅ Enterprise tier: ${RATE_LIMITS.enterprise.requestsPerMinute} req/min`)
    testsPassed++
  } catch (error) {
    console.log(`❌ ${error.message}`)
    testsFailed++
