...
const searchSkillsRoute = createRoute({
  ...
  description: 'Search-first skill tagging: type a term, get results with auto-selected parent hierarchy',
  request: {
    query: z.object({
      q: z.string().min(1),
...
app.openapi(getTreeRoute, async (c) => {
  ...
  while (changed) {
    ...
    for (const skill of skills) {
      if (skill.parent_id && treeIds.has(skill.parent_id) && !treeIds.has(skill.id)) {
  ...
  const filtered = skills.filter((s: Record<string, unknown>) => treeIds.has(s.id as string))
...
const getAncestorsRoute = createRoute({
  ...
  description: 'Get the full parent chain for a skill (for auto-parent-tagging)',
  request: {
    params: z.object({ skillId: z.string().uuid() }),
  },
  responses: {
