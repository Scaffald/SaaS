...
app.openapi(listCommunitiesRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to fetch communities', message: error.message }, 500)
  }
...
app.openapi(joinCommunityRoute, async (c) => {
  ...
  if (body.license_number) verification_data.license_number = body.license_number
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to join community', message: error.message }, 500)
  }
...
app.openapi(leaveCommunityRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to leave community', message: error.message }, 500)
  }
