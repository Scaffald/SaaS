...
app.openapi(listCommentsRoute, async (c) => {
  ...
  const authorIds = [...new Set((comments || []).map((c: Record<string, unknown>) => c.author_id))]
  ...
  if (user) {
    ...
    if (commentIds.length > 0) {
      ...
      upvotedIds = new Set((upvotes || []).map((u: Record<string, unknown>) => u.target_id as string))
    }
  }
...
app.openapi(createCommentRoute, async (c) => {
  ...
  if (post && post.author_id !== user.id) {
    ...
    ).catch((err: unknown) => console.error('Failed to send comment notification:', err))
  }
...
const pinCommentRoute = createRoute({
  ...
  tags: ['Community Comments'],
  summary: 'Pin comment',
  description: 'Pin a comment on your post (1 pinned per post, post author only)',
  request: {
    params: z.object({ commentId: z.string().uuid() }),
  },
  responses: {
...
app.openapi(pinCommentRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to pin comment', message: error.message }, 500)
  }
