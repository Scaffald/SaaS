...
const submitLicenseRoute = createRoute({
  ...
  description: 'Submit license information for community verification (self-attestation)',
  request: {
...
app.openapi(submitLicenseRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to submit license', message: error.message }, 500)
  }

  return c.json({
    success: true,
    message: 'License submitted for review. You will be notified when verified.',
  })
})
...
app.openapi(myStatusesRoute, async (c) => {
  ...
  const communityIds = memberships.map((m: Record<string, unknown>) => m.community_id)
  ...
  const statuses = memberships.map((m: Record<string, unknown>) => ({
    ...
    verified_at: (m.verification_data as Record<string, unknown> | null)?.verified_at || null,
