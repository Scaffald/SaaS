...
app.get('/apps/:client_id', async (c) => {
  ...
  const { data, error } = await supabase
    .schema('core')
    .from('oauth_apps')
    .select('id, display_name, description, logo_url, homepage_url, privacy_policy_url, terms_of_service_url')
...
app.post('/apps', zValidator('json', registerAppSchema), async (c) => {
  ...
  const secretBuffer = await crypto.subtle.digest('SHA-256', encoder.encode(clientSecret))
  const secretHash = Array.from(new Uint8Array(secretBuffer)).map(b => b.toString(16).padStart(2, '0')).join('')
  ...
  if (error || !data) {
    return c.json({ error: 'Failed to register app', message: error?.message }, 500)
  }
...
app.post('/consent/grant', zValidator('json', grantConsentSchema), async (c) => {
  const user = c.get('user')
  const supabase = c.get('supabase')
  ...
  const encoder = new TextEncoder()
  const codeBuffer = await crypto.subtle.digest('SHA-256', encoder.encode(authCode))
...
app.get('/consent', async (c) => {
  ...
  const { data, error } = await supabase
    ...
    .select('id, granted_scopes, granted_at, expires_at, oauth_app:oauth_apps(id, display_name, description, logo_url, homepage_url)')
    .eq('user_id', user.id)
    .is('revoked_at', null)
    .order('granted_at', { ascending: false })

  if (error) {
    return c.json({ error: 'Failed to fetch consents', message: error.message }, 500)
  }
...
app.delete('/consent/:consent_id', async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to revoke consent', message: error.message }, 500)
  }
...
const listAppsQuerySchema = z.object({
  status: z.enum(['all', 'pending', 'active', 'trusted', 'suspended', 'revoked']).optional(),
  search: z.string().optional(),
})

app.get('/admin/apps', requireRole('office', 'platform'), zValidator('query', listAppsQuerySchema), async (c) => {
  ...
  let query = supabase
    .schema('core')
    .from('oauth_apps')
    .select('id, client_id, display_name, description, status, created_at, approved_at, owner_email')
  ...
  if (error) {
    return c.json({ error: 'Failed to fetch apps', message: error.message }, 500)
  }
...
app.get('/admin/scopes', requireRole('office', 'platform'), async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to fetch scopes', message: error.message }, 500)
  }
...
app.post('/admin/apps/:app_id/approve', requireRole('office', 'platform'), zValidator('json', approveAppSchema), async (c) => {
  const supabase = c.get('supabaseAdmin')
  const { app_id } = c.req.param()
  ...
  if (error) {
    return c.json({ error: 'Failed to approve app', message: error.message }, 500)
  }
...
app.post('/admin/apps/:app_id/reject', requireRole('office', 'platform'), zValidator('json', rejectAppSchema), async (c) => {
  const supabase = c.get('supabaseAdmin')
  const { app_id } = c.req.param()
  ...
  if (error) {
    return c.json({ error: 'Failed to reject app', message: error.message }, 500)
  }
...
app.post('/admin/apps/:app_id/suspend', requireRole('office', 'platform'), async (c) => {
  const supabase = c.get('supabaseAdmin')
  const { app_id } = c.req.param()
  ...
  await supabase
    .schema('core')
    .from('oauth_tokens')
    .update({ revoked_at: new Date().toISOString(), revoked_reason: 'app_suspended' })
  ...
  if (error) {
    return c.json({ error: 'Failed to suspend app', message: error.message }, 500)
  }
