 ...
 */
app.post('/authorize', authMiddleware, zValidator('json', authorizeSchema), async (c) => {
  ...
  if (appError || !app) {
    return c.json(oauthError('unauthorized_client', 'Invalid client_id'), 400)
  }

  // Check app status
  if (app.status !== 'active' && app.status !== 'trusted') {
    return c.json(oauthError('unauthorized_client', 'App is suspended or revoked'), 403)
  }

  // Verify redirect_uri matches registered URIs
  if (!app.redirect_uris.includes(input.redirect_uri)) {
    return c.json(oauthError('invalid_request', 'redirect_uri mismatch'), 400)
  }
  ...
  if (invalidScopes.length > 0) {
    return c.json(
      oauthError('invalid_scope', `Scopes not allowed: ${invalidScopes.join(', ')}`),
      400
    )
  }

  // Validate user permissions using database function
  const { data: authorizedScopes, error: scopeError } = await supabase.rpc('validate_oauth_scope', {
    p_user_id: user.id,
    p_requested_scopes: requestedScopes,
  })

  if (scopeError || !authorizedScopes || authorizedScopes.length === 0) {
    return c.json(oauthError('invalid_scope', 'User lacks required permissions'), 403)
  }
  ...
  if (codeError) {
    ...
    return c.json(oauthError('server_error', 'Failed to generate authorization code'), 500)
  }
...
app.post('/token', zValidator('form', tokenSchema), async (c) => {
  ...
  if (appError || !app) {
    return c.json(oauthError('invalid_client', 'Invalid client credentials'), 401)
  }
  ...
  if (input.grant_type === 'authorization_code') {
    ...
    const pkceValid = await verifyPKCE(input.code_verifier, authCode.code_challenge)
    if (!pkceValid) {
  ...
  if (input.grant_type === 'refresh_token') {
    ...
    if (tokenError || !refreshToken) {
      return c.json(oauthError('invalid_grant', 'Invalid or revoked refresh token'), 400)
    }
  ...
  if (input.grant_type === 'client_credentials') {
    ...
    const requestedScopes = input.scope ? input.scope.split(' ') : app.allowed_scopes
  ...
  return c.json(oauthError('unsupported_grant_type', 'Invalid grant_type'), 400)
})
