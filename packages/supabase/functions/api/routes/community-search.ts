...
app.openapi(searchProfessionalsRoute, async (c) => {
  ...
  const scoreMap = new Map(
    (scores || []).map((s: Record<string, unknown>) => [s.user_id, s.score]),
  );
  ...
  const skillNameMap = new Map(
    (skillNames || []).map((s: Record<string, unknown>) => [s.id, s.name]),
  );
  ...
  const results = users.map((user: Record<string, unknown>) => {
    ...
    const ratingAvg = stats.ratingCount > 0
    ...
    return {
      ...
      _rank: score * 0.4 + (ratingAvg || 0) * 10 + stats.count * 2 +
        (verifiedMap.has(user.id) ? 20 : 0),
    };
  });

  // Sort by ranking score
  results.sort((a: Record<string, unknown>, b: Record<string, unknown>) =>
    (b._rank as number) - (a._rank as number)
  );

  // Paginate
  const total = results.length;
  const paginated = results.slice(offset, offset + limit).map((
    { _rank, ...rest }: Record<string, unknown>,
  ) => rest);
            ...
            messages: [
              {
                role: "system",
                content:
                  `Extract structured search filters from a natural language query about finding trade professionals. Return JSON: {"skills": ["skill names"], "state": "2-letter state or null", "city": "city name or null", "keywords": ["other keywords"]}`,
              },
