...
function getCacheKey(
  ...
  if (
    north !== undefined && south !== undefined && east !== undefined &&
    west !== undefined
  ) {
...
app.get(
  ...
  async (c) => {
    ...
    const timeoutPromise = new Promise<
      { workers: number; jobs: number; employers: number }
    >(
...
app.get(
  ...
  async (c) => {
    ...
    const [workersResult, jobsResult, orgsResult] = await Promise.all([
      supabase.schema("core").from("v_profile_search").select(
      ...
      supabase.schema("core").from("jobs").select("id, address, location").eq(
        "status",
        "open",
      ).limit(100),
    ...
    for (const job of (jobsResult.data || [])) {
      ...
      if (address?.latitude && address?.longitude) {
        checkNearest(
          address.latitude,
          address.longitude,
          (job as { location?: string }).location || "Job location",
        );
      }
    }
    ...
    for (const org of (orgsResult.data || [])) {
      if (org.latitude && org.longitude) {
        checkNearest(
          org.latitude,
          org.longitude,
          org.name || "Organization location",
        );
      }
    }
    ...
    const nr = nearestResult as {
      coordinates: { lat: number; lng: number };
      label: string;
      distance: number;
    };
    const nearestBounds = {
      north: nr.coordinates.lat + 0.1,
      south: nr.coordinates.lat - 0.1,
      east: nr.coordinates.lng + 0.1,
      west: nr.coordinates.lng - 0.1,
    };
    ...
    const [workerCount, jobCount, orgCount] = await Promise.all([
      supabase.schema("core").from("v_profile_search").select("*", {
        ...
        .gte("longitude", nearestBounds.west).lte(
          "longitude",
          nearestBounds.east,
        )
        .gte("latitude", nearestBounds.south).lte(
          "latitude",
          nearestBounds.north,
        ),
      supabase.schema("core").from("jobs").select("*", {
        count: "exact",
        head: true,
      }).eq("status", "open"),
