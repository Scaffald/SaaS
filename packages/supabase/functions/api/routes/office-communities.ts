...
const verificationQueueRoute = createRoute({
  ...
  description:
    "List all pending community verification requests (office role required)",
  request: {
    query: z.object({
...
app.openapi(verificationQueueRoute, async (c) => {
  ...
  const communityIds = [
    ...new Set(memberships.map((m: Record<string, unknown>) => m.community_id)),
  ];
  const userIds = [
...
const approveVerificationRoute = createRoute({
  ...
  description:
    "Approve a community verification request (office role required)",
  request: {
