...
app.get("/has-uploaded", async (c) => {
  ...
  if (error && error.code !== "PGRST116") {
    return c.json({
      error: "Failed to check resume uploads",
...
app.post("/upload", async (c) => {
  ...
  if (!fileData || !fileName || !fileSize || !mimeType) {
    return c.json({
      error: "Missing required fields: fileData, fileName, fileSize, mimeType",
    }, 400);
  }
  ...
  try {
    ...
    return c.json({
      error: err instanceof Error ? err.message : "Failed to decode file",
    }, 400);
  }
  ...
  try {
    ...
    if (!resumeText || resumeText.trim().length === 0) {
      await supabase
        ...
        .eq("id", resumeId);
      return c.json({
        error: "Unable to process resume. Please try again or contact support.",
      }, 400);
    }
    ...
    for (const section of sections) {
      const hasData = (section === "employment" && parsed.employment) ||
        (section !== "employment" && Array.isArray(parsed[section]) &&
...
app.get("/wizard-state", async (c) => {
  ...
  if (!resumeId) {
    return c.json({ error: "resumeId query param is required" }, 400);
  }
...
app.post("/wizard-state/save-section", async (c) => {
  ...
  if (!section || !ws?.resumeId) {
    return c.json(
      { error: "section and wizardState.resumeId are required" },
      400,
    );
  }

  const mergeMode: "replace" | "append" | "keepExisting" =
  ...
  try {
    switch (section as ResumeSection) {
      case "general": {
        ...
        if (d?.avatar_path !== undefined) {
          userUpdate.avatar_path = d.avatar_path;
        }
        if (d?.first_name !== undefined || d?.last_name !== undefined) {
          const displayName = [d?.first_name ?? "", d?.last_name ?? ""].join(
            " ",
          ).trim();
          if (displayName) userUpdate.display_name = displayName;
        }
        if (d?.about !== undefined || d?.avatar_path !== undefined) {
          await supabase.schema("core").from("users").update(userUpdate).eq(
            "id",
            user.id,
          );
        }
        ...
        if (d?.first_name !== undefined) {
          privateUpdate.first_name = d.first_name;
        }
      ...
      case "skills": {
        ...
        const inserts = skills
          ...
          .map((s: any) => ({
            ...
            onet_occupation_id: s.csiMatch?.id
              ? null
              : (s.onetMatch?.id ?? null),
            proficiency_level: 3,
          }));
