...
import { z } from 'zod'
import { requireAuth, requireRole, addSupabaseAdminForUser } from '../middleware/auth.ts'
...
function toJson(c: { json: (body: unknown, status?: number) => Response }, body: unknown, status = 200) {
  return c.json(body, status)
}

function toError(c: { json: (body: unknown, status?: number) => Response }, message: string, status: number) {
...
app.post('/request', requireAuth, addSupabaseAdminForUser, zValidator('json', requestSchema), async (c) => {
  const user = c.get('user')
  const supabase = c.get('supabase')
  const supabaseAdmin = c.get('supabaseAdmin')

  if (!user?.id) return toError(c, 'Unauthorized', 401)
  if (!supabase || !supabaseAdmin) return toError(c, 'Admin client not available', 500)

  const input = c.req.valid('json')
  try {
    await ensureCanRequestForWorker(supabaseAdmin, user.id, input.workerUserId)
  } catch (e) {
  ...
  if (pricingError) return toError(c, `Failed to load pricing: ${pricingError.message}`, 500)

  const pricingList = (pricingRows ?? []) as Array<{ id: string; price_cents: number }>
  const selectedPricing = input.pricingId
    ? pricingList.find((r) => r.id === input.pricingId)
    : pricingList[0]

  if (!selectedPricing) return toError(c, 'ID verification pricing is not configured.', 400)
  if (selectedPricing.price_cents <= 0) return toError(c, 'Pricing for ID verification must be greater than zero.', 400)
  ...
  try {
    ...
    if (!intent.client_secret) return toError(c, 'Stripe client secret was not returned.', 500)
...
app.post('/confirm', requireAuth, addSupabaseAdminForUser, zValidator('json', confirmSchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  ...
  try {
    ...
    if (intent.status !== 'succeeded') {
      return toError(c, `Payment intent is not succeeded (current status: ${intent.status}).`, 400)
    }

    const metadata = (intent.metadata ?? {}) as Record<string, string>
    const workerUserId = metadata.worker_user_id ?? user.id
    if (!workerUserId) return toError(c, 'Unable to determine worker for verification.', 400)

    await ensureCanRequestForWorker(supabaseAdmin, user.id, workerUserId)

    const initiatedByUserId = metadata.initiated_by_user_id?.length ? metadata.initiated_by_user_id : user.id
    ...
    if (existing) {
      return toJson(c, {
        data: { id: existing.id, badgeStatus: existing.badge_status, badgeExpiresAt: null },
      })
    }
    ...
    if (insertError || !record) {
      return toError(c, insertError ? `Failed to create id verification: ${insertError.message}` : 'Failed to create record', 500)
    }
...
app.get('/status', requireAuth, addSupabaseAdminForUser, zValidator('query', statusQuerySchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  ...
  if (error) return toError(c, `Failed to load verification: ${error.message}`, 500)
  if (!data) return toError(c, 'Verification not found', 404)

  try {
    await ensureCanRequestForWorker(supabaseAdmin, user.id, data.worker_user_id ?? '')
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Forbidden'
    return toError(c, msg, 403)
  }
...
app.get('/current', requireAuth, addSupabaseAdminForUser, zValidator('query', currentQuerySchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  ...
  if (error) return toError(c, `Failed to load verification badge: ${error.message}`, 500)
...
// GET /list - office
app.get('/list', requireRole('office', 'platform'), zValidator('query', listQuerySchema), async (c) => {
  const supabaseAdmin = c.get('supabaseAdmin')
  if (!supabaseAdmin) return toError(c, 'Admin client not available', 500)
  ...
  if (error) return toError(c, `Failed to load ID verifications: ${error.message}`, 500)
  ...
  const deriveStatus = (row: RowRecord): DerivedStatus => {
    const base =
      row.badge_status === 'revoked' ? 'revoked' : row.badge_status === 'expired' ? 'expired' : 'active'
    if (
      base === 'active' &&
  ...
  const normalized = rows.map((row) => {
    ...
    const source: 'worker' | 'organization' | 'platform' =
      row.initiated_by_org_id ? 'organization'
      : row.initiated_by_user_id && row.worker_user_id && row.initiated_by_user_id !== row.worker_user_id
        ? 'platform'
        : 'worker'
    ...
    return {
      ...
      workerAvatarUrl: (row.worker as { avatar_path?: string })?.avatar_path ?? null,
  ...
  const statusFiltered =
    input.status === 'all' ? normalized : normalized.filter((i) => i.badgeStatus === input.status)
...
// POST /revoke - office
app.post('/revoke', requireRole('office', 'platform'), zValidator('json', revokeSchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  ...
  if (error) return toError(c, `Failed to revoke verification: ${error.message}`, 500)
  if (!data) return toError(c, 'Verification not found', 404)
