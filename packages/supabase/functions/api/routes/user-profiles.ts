    ...
    data?.some(
      (
        ...
        return role?.scope === "platform" &&
          ["office", "super_admin"].includes(role?.name ?? "");
      },
    ),
  );
}
...
const getPreviewRoute = createRoute({
  ...
  description:
    "Returns optimized lightweight data for quick loading in map context with top skills",
  request: {
    params: z.object({
...
app.openapi(getPreviewRoute, async (c) => {
  ...
  const { data: skills } = await supabase
    ...
    .from("user_skills")
    .select(
      "proficiency_level, skill_taxonomy, csi_skill_id, onet_occupation_id",
    )
    .eq("user_id", userId)
...
app.openapi(getUserProfileRoute, async (c) => {
  ...
  if (yearsError) {
    console.warn(
      "[userProfile.getUserProfile] Unable to calculate years of experience",
      {
        userId,
  ...
  return c.json(
    {
      ...profile,
      calculatedYearsOfExperience: calculatedYears ??
...
const getUserSkillsRoute = createRoute({
  ...
  description:
    "Includes skill details from CSI/ONET taxonomies with enrichment data",
  request: {
    params: z.object({
      userId: z.string().uuid().openapi({
        description: "User ID",
...
const getUserContactInfoRoute = createRoute({
  ...
  description:
    "Access is gated by success fee payment. Requires organizationId and applicationId query params.",
  request: {
    params: z.object({
      userId: z.string().uuid().openapi({
        description: "User ID",
    ...
    return c.json(
      {
        error: "Forbidden",
        message:
          "Success fee payment is required to access worker contact information.",
      },
      403,
    );
  }
