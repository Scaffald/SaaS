...
function mapOrganization(rec: Record<string, unknown> | null) {
  if (!rec) return null
  return { id: (rec as { id?: string | null }).id ?? null, name: (rec as { name?: string | null }).name ?? null }
}
...
function mapPackage(rec: Record<string, unknown> | null) {
  ...
  return {
    ...
    display_name: (rec as { display_name?: string | null }).display_name ?? null,
    slug: (rec as { slug?: string | null }).slug ?? null,
  }
}

// GET /checks
app.get('/checks', zValidator('query', z.object({
  ...
  if (error) return c.json({ error: 'Failed to fetch checks', message: error.message }, 500)
  ...
  const items = (data ?? []).map((row: Record<string, unknown>) => {
    ...
    return {
      ...
      requester: requester ? { id: requester.id ?? null, display_name: requester.display_name ?? null, email: requester.email ?? null } : null,
    }
  })
...
// GET /checks/:id
app.get('/checks/:id', zValidator('param', z.object({ id: z.string().uuid() })), async (c) => {
  const supabaseAdmin = c.get('supabaseAdmin')
  if (!supabaseAdmin) return c.json({ error: 'Admin client not available' }, 500)
  ...
  if (checkError) return c.json({ error: 'Failed to fetch check', message: checkError.message }, 500)
  if (!checkRecord) return c.json({ error: 'Check not found', message: 'Background check not found' }, 404)
  ...
  const { data: disputes } = await supabaseAdmin
    ...
    .select('id, status, dispute_reason, dispute_details, supporting_documents, created_at, resolved_at, resolution, resolution_notes')
    .eq('background_check_id', id)
  ...
  const check = {
    ...rec,
    metadata: rec.metadata && typeof rec.metadata === 'object' && !Array.isArray(rec.metadata) ? rec.metadata : null,
    worker: mapWorker(rec.user as Record<string, unknown> | null),
    organization: mapOrganization(rec.organization as Record<string, unknown> | null),
  }
...
const updateStatusSchema = z.object({
  status: z.enum([
    'pending', 'in_progress', 'under_review', 'completed_clear', 'completed_consider', 'completed_not_clear',
...
app.patch(
  ...
  async (c) => {
    ...
    const { data: existing, error: fetchError } = await supabaseAdmin
      .schema('core')
      .from('background_checks')
      .select('status_history, user_id, requested_by_user_id, package:background_check_packages(display_name, slug)')
      .eq('id', id)
      .maybeSingle()

    if (fetchError) return c.json({ error: 'Failed to load check', message: fetchError.message }, 500)
    if (!existing) return c.json({ error: 'Check not found', message: 'Background check not found' }, 404)

    const history = Array.isArray((existing as Record<string, unknown>).status_history)
    ...
    const { data: updated, error: updateError } = await supabaseAdmin
      ...
      .eq('id', id)
      .select('id, status, updated_at, summary, findings, component_statuses, status_history, expires_at, user_id, requested_by_user_id, package:background_check_packages(display_name, slug)')
      .single()

    if (updateError || !updated) return c.json({ error: 'Failed to update status', message: updateError?.message }, 500)
...
app.patch(
  ...
  async (c) => {
    ...
    const updatedMetadata = { ...existingMetadata, privacy: { ...privacy, ...updatedPrivacy } }
    ...
    if (updateError) return c.json({ error: 'Failed to update privacy', message: updateError.message }, 500)
...
app.post(
  ...
  async (c) => {
    ...
    if (fetchError) return c.json({ error: 'Failed to load document', message: fetchError.message }, 500)
    if (!document) return c.json({ error: 'Document not found', message: 'Document not found' }, 404)

    const doc = document as { file_path: string; file_name?: string | null; background_check_id: string }
    const { data: signedUrlData, error: signedUrlError } = await supabaseAdmin.storage
      .from(BACKGROUND_CHECK_BUCKET_ID)
      .createSignedUrl(doc.file_path, SIGNED_DOWNLOAD_URL_TTL_SECONDS, { download: doc.file_name ?? undefined })

    if (signedUrlError || !signedUrlData?.signedUrl) {
      return c.json({ error: 'Failed to generate download URL', message: signedUrlError?.message }, 500)
    }
    ...
    return c.json({
      data: {
        ...
        expires_at: new Date(Date.now() + SIGNED_DOWNLOAD_URL_TTL_SECONDS * 1000).toISOString(),
      },
    })
  }
)
...
// GET /disputes
app.get('/disputes', zValidator('query', z.object({
  ...
  const { data, error } = await query
  if (error) return c.json({ error: 'Failed to fetch disputes', message: error.message }, 500)
    ...
    return {
      ...
      background_check: bc
        ? {
            ...
            organization: mapOrganization(bc.organization as Record<string, unknown> | null),
          }
        : null,
    }
  })
...
app.patch(
  ...
  async (c) => {
    ...
    if (error || !data) return c.json({ error: 'Failed to resolve dispute', message: error?.message }, 500)
    return c.json({ data })
  }
)
...
app.get('/metrics', async (c) => {
  ...
  const [checksRes, disputesRes] = await Promise.all([
    supabaseAdmin
      .schema('core')
      .from('background_checks')
      .select('status, created_at, completed_at, package:background_check_packages(id, display_name, slug)'),
    supabaseAdmin.schema('core').from('background_check_disputes').select('status'),
  ])
  ...
  const averageCompletionDays = completedCount > 0 ? +(durationSumDays / completedCount).toFixed(1) : null
  ...
  return c.json({
    data: {
      ...
      packageDistribution: Object.entries(packageTotals).map(([label, count]) => ({ label, count })),
    },
  })
})

// GET /access-log
app.get('/access-log', zValidator('query', z.object({
  ...
  if (error) return c.json({ error: 'Failed to fetch access log', message: error.message }, 500)
  ...
  const items = (data ?? []).map((row: Record<string, unknown>) => {
    ...
    const actorName =
      actor?.display_name ??
      actor?.username ??
      (actor?.id ? `User ${(actor.id as string).slice(0, 8)}` : 'Administrator')
    return {
      id: row.id,
      background_check_id: row.background_check_id,
      ...
      background_check: bc
        ...
        : null,
      actor: actor ? { id: actor.id ?? null, name: actorName, email: actor.email ?? null } : null,
    }
  })
  return c.json({ data: items })
})
