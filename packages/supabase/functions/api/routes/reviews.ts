...
app.openapi(
  createRoute({
    ...
    request: {
      query: z.object({
        category: z.enum(['reliability', 'collaboration', 'professionalism', 'technical']).optional(),
      }),
    },
    responses: {
      200: {
  ...
  async (c) => {
    ...
    let query = supabase.schema('core').from('soft_skills').select('*').eq('is_active', true)
    ...
    if (error) {
      return c.json({ error: 'Failed to fetch soft skills', message: error.message }, 500)
    }
...
app.openapi(
  ...
  async (c) => {
    ...
    if (error) {
      return c.json({ error: 'Failed to create draft', message: error.message }, 500)
    }
...
app.openapi(
  ...
  async (c) => {
    ...
    let query = supabase.schema('core').from('reviews').select('*').eq('subject_id', subject_id).eq('subject_type', subject_type)
    ...
    if (error) {
      return c.json({ error: 'Failed to fetch reviews', message: error.message }, 500)
    }
...
app.openapi(
  ...
  async (c) => {
    ...
    if (error) {
      return c.json({ error: 'Failed to submit review', message: error.message }, 500)
    }
