...
import { z } from 'zod'
import { requireAuth, addSupabaseAdminForUser } from '../middleware/auth.ts'
...
async function ensureOrganizationAccess(supabaseAdmin: any, userId: string, organizationId: string): Promise<void> {
  ...
  if (assignmentError) throw new Error(`Failed to verify organization access: ${assignmentError.message}`)
  if (!assignment) {
    const err = new Error('You do not have access to this organization') as Error & { status: number }
    err.status = 403
    throw err
  }
}

function determinePaymentSchedule(totalFeeCents: number, jobDurationDays: number, hireStartDate: string) {
  ...
  return { paymentSchedule, upfrontPercentage, finalPercentage, upfrontAmountCents, finalAmountCents, finalPaymentDueDate }
}
...
app.get('/status', requireAuth, addSupabaseAdminForUser, zValidator('query', statusQuerySchema), async (c) => {
  ...
  const { organizationId, applicationId, workerUserId } = c.req.valid('query')
  ...
  if (error && error.code !== 'PGRST116') {
    return c.json({ error: `Failed to load success fee: ${error.message}` }, 500)
  }
 ...
 */
app.post('/', requireAuth, addSupabaseAdminForUser, zValidator('json', createSchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  if (!user || !supabaseAdmin) return c.json({ error: 'Unauthorized' }, 401)
  ...
  if (!input.applicationId) {
    return c.json({ error: 'applicationId is required to create a success fee.' }, 400)
  }

  try {
    await ensureOrganizationAccess(supabaseAdmin, user.id, input.organizationId)
  } catch (e) {
    const err = e as Error & { status?: number }
    return c.json({ error: err.message }, (err.status ?? 403) as 403 | 404)
  }
  ...
  const schedule = determinePaymentSchedule(totalFeeCents, input.jobDurationDays, input.hireStartDate)
  ...
  if (!latest) {
    ...
    if (insertError || !inserted) {
      return c.json(
        { error: insertError ? `Failed to create success fee: ${insertError.message}` : 'Failed to create success fee' },
        500,
      )
    }
  ...
  try {
    ...
    if (successFee.upfront_payment_intent_id) {
      ...
      if (updateError) {
        return c.json({ error: `Failed to link payment intent: ${updateError.message}` }, 500)
      }
    ...
    return c.json({ error: err.message || 'Failed to create payment intent' }, 500)
  }
})
 ...
 */
app.post('/:id/confirm-upfront', requireAuth, addSupabaseAdminForUser, zValidator('json', confirmBodySchema), async (c) => {
  const user = c.get('user')
  const supabaseAdmin = c.get('supabaseAdmin')
  if (!user || !supabaseAdmin) return c.json({ error: 'Unauthorized' }, 401)
  ...
  if (feeError) return c.json({ error: `Failed to load success fee: ${feeError.message}` }, 500)
  if (!feeData) return c.json({ error: 'Success fee not found' }, 404)

  const successFee = feeData as SuccessFeeRecord

  try {
    await ensureOrganizationAccess(supabaseAdmin, user.id, successFee.organization_id)
  } catch (e) {
    const err = e as Error & { status?: number }
    return c.json({ error: err.message }, (err.status ?? 403) as 403 | 404)
  }
  ...
  if (!successFee.upfront_payment_intent_id) {
    return c.json({ error: 'Success fee does not have an upfront payment intent.' }, 400)
  }

  if (successFee.upfront_payment_intent_id !== paymentIntentId) {
    return c.json({ error: 'Provided payment intent does not match the success fee.' }, 400)
  }
  ...
  try {
    ...
    if (updateError) {
      return c.json({ error: `Failed to update success fee: ${updateError.message}` }, 500)
    }
