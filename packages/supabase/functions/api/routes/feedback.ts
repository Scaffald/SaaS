...
async function resolveUserDisplayName(supabase: { schema: (s: string) => { from: (t: string) => { select: (c: string) => { eq: (k: string, v: string) => { maybeSingle: () => Promise<{ data: unknown }> } } } } }, userId: string): Promise<string | null> {
  ...
  const user = userRow as { display_name?: string | null; username?: string | null } | null
  const profile = profileRow as { first_name?: string | null; last_name?: string | null } | null
  const fallbackName = [profile?.first_name ?? '', profile?.last_name ?? '']
    .map((part) => (part ?? '').trim())
    .filter(Boolean)
    .join(' ')
  const displayName = user?.display_name?.trim() || fallbackName || user?.username?.trim()
...
app.get('/user-feedback', zValidator('query', historyQuerySchema), async (c) => {
  const supabase = c.get('supabase')
  const user = c.get('user')

  if (!user?.id) return c.json({ error: 'Unauthorized', message: 'You must be logged in to view feedback' }, 401)

  const input = c.req.valid('query')
  const limit = input.limit ?? DEFAULT_HISTORY_LIMIT
  const offset = input.offset ?? 0
  if (input.offset !== undefined && input.limit === undefined) {
    return c.json({ error: 'Bad Request', message: 'Limit is required when offset is provided' }, 400)
  }
  ...
  if (error) {
    console.error('[feedback.getUserFeedback] Query failed', { userId: user.id, message: error.message })
    return c.json({ error: 'Failed to load feedback history', message: error.message }, 500)
  }
...
app.post('/submit', zValidator('json', feedbackSubmitSchema), async (c) => {
  ...
  if (!user?.id) return c.json({ error: 'Unauthorized', message: 'You must be logged in to submit feedback' }, 401)

  const userEmail = (user as { email?: string }).email
  if (!userEmail) {
    return c.json({ error: 'Internal Server Error', message: 'Unable to resolve user email' }, 500)
  }

  const input = c.req.valid('json')

  if (input.screenshotPath && !input.screenshotPath.startsWith(`${user.id}/`)) {
    return c.json({ error: 'Forbidden', message: 'Screenshot path must belong to the authenticated user' }, 403)
  }
  ...
  if (error) {
    console.error('[feedback.submit] Failed to insert', { userId: user.id, message: error.message })
...
app.post('/upload-url', zValidator('json', uploadUrlSchema), async (c) => {
  ...
  if (!user?.id) return c.json({ error: 'Unauthorized', message: 'You must be logged in to upload screenshots' }, 401)
  ...
  if (error || !data) {
    return c.json({ error: 'Unable to create upload URL', message: error?.message }, 500)
  }
