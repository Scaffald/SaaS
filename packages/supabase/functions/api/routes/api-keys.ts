...
const CreateApiKeySchema = z.object({
  ...
  environment: z.enum(["test", "live"]).default("live").describe(
    "API key environment",
  ),
  scopes: z
    .array(
...
app.post("/", requireAuth, async (c) => {
  try {
      ...
      return c.json(
        {
          error: "Forbidden",
          message:
            "API keys cannot be created using another API key. Please authenticate as a user.",
        },
        403,
      );
    }
    ...
    const { data: createdKey, error: insertError } = await supabase
      ...
      .select(
        "id, name, key_prefix, scopes, rate_limit_tier, created_at, expires_at",
      )
      .single();
...
app.get("/:id/usage", requireAuth, async (c) => {
  try {
    ...
    const successRequests = usage.filter((u) =>
      u.status_code >= 200 && u.status_code < 300
    ).length;
    const errorRequests = usage.filter((u) => u.status_code >= 400).length;
    const avgResponseTime = usage.reduce((sum, u) =>
      sum + (u.response_time_ms || 0), 0) / (totalRequests || 1);
