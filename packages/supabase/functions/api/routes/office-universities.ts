...
app.get('/', requireRole('office', 'platform'), zValidator('query', listQuerySchema), async (c) => {
  const supabase = c.get('supabase')
  const { page, pageSize, search, country, sortBy, sortOrder } = c.req.valid('query')
  ...
  if (error) {
    return c.json({ error: 'Failed to fetch universities', message: error.message }, 500)
  }
...
app.get('/:id', requireRole('office', 'platform'), async (c) => {
  ...
  if (error) {
    return c.json({ error: 'University not found', message: error.message }, 404)
  }
...
app.get('/search', zValidator('query', searchQuerySchema), async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to search universities', message: error.message }, 500)
  }
...
app.post('/', requireRole('office', 'platform'), zValidator('json', createBodySchema), async (c) => {
  const supabase = c.get('supabase')
  ...
  if (existing) {
    return c.json({ error: 'A university with this slug already exists' }, 400)
  }
  ...
  if (error) {
    return c.json({ error: 'Failed to create university', message: error.message }, 500)
  }
...
app.patch('/:id', requireRole('office', 'platform'), zValidator('json', updateBodySchema), async (c) => {
  const supabase = c.get('supabase')
  const { id } = c.req.param()
  const input = c.req.valid('json')
  ...
  const updateData = {
    ...input,
    ...(input.alpha_two_code ? { alpha_two_code: input.alpha_two_code.toUpperCase() } : {}),
  ...
  if (error) {
    return c.json({ error: 'Failed to update university', message: error.message }, 500)
  }
...
app.delete('/:id', requireRole('office', 'platform'), async (c) => {
  ...
  if (count && count > 0) {
    return c.json({
      error: `Cannot delete university: ${count} user(s) have this in their education history`,
    }, 400)
  }

  const { error } = await supabase.schema('data').from('universities').delete().eq('id', id)

  if (error) {
    return c.json({ error: 'Failed to delete university', message: error.message }, 500)
  }
