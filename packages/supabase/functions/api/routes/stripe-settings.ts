...
app.get('/', async (c) => {
  ...
  if (error) {
    return c.json({ error: `Failed to load Stripe settings: ${error.message}` }, 500)
  }
...
app.put(
  ...
  async (c) => {
    ...
    if (error) {
      return c.json({ error: `Failed to update publishable key: ${error.message}` }, 500)
    }
...
app.put(
  ...
  async (c) => {
    ...
    if (error) {
      return c.json({ error: `Failed to update test mode: ${error.message}` }, 500)
    }
...
app.put(
  ...
  async (c) => {
    ...
    const user = c.get('user')
    if (!supabase || !supabaseAdmin) return c.json({ error: 'Unauthorized' }, 401)
    ...
    if (rotateError) {
      return c.json({ error: `Failed to store API secret: ${rotateError.message}` }, 500)
    }
    ...
    if (updateError) {
      return c.json({ error: `Failed to link API secret: ${updateError.message}` }, 500)
    }
    ...
    if (configureError) {
      return c.json({ error: `Stripe server configuration failed: ${configureError.message}` }, 500)
    }
...
app.put(
  ...
  async (c) => {
    ...
    if (rotateError) {
      return c.json({ error: `Failed to store webhook secret: ${rotateError.message}` }, 500)
    }
    ...
    if (updateError) {
      return c.json({ error: `Failed to link webhook secret: ${updateError.message}` }, 500)
    }
...
app.post('/test-connection', async (c) => {
  ...
  if (!row?.api_key_secret_id) {
    return c.json({ error: 'Add an API secret before testing the connection.' }, 400)
  }
