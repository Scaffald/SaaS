...
const updateCurrentStepSchema = z.object({
  step: z.enum(['luscher1', 'cooldown', 'ipip', 'luscher2', 'acute', 'completed']),
})

const saveLuscherTestSessionSchema = z.object({
  luscher1Choices: z.array(z.number()).length(8, 'Must select exactly 8 colors'),
  luscher2Choices: z.array(z.number()).length(8, 'Must select exactly 8 colors'),
...
app.openapi(getStatusRoute, async (c) => {
  ...
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to fetch assessment: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to get assessment status' }, 500)
  }
})
...
app.openapi(getIPIPStatusRoute, async (c) => {
  ...
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      return c.json(
        { error: 'Internal server error', message: `Failed to fetch status: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to get IPIP status' }, 500)
  }
})
...
app.openapi(getLuscherTest1StatusRoute, async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        isCompleted: !!(data?.luscher1_completed_at && data?.luscher1_choices?.length === 8),
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to get Luscher Test 1 status' }, 500)
  }
})
...
app.openapi(getLuscherTest2StatusRoute, async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        isCompleted: !!(data?.luscher2_completed_at && data?.luscher2_choices?.length === 8),
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to get Luscher Test 2 status' }, 500)
  }
})
...
app.openapi(getLuscherTestAvailabilityRoute, async (c) => {
  ...
  try {
    const { data, error } = await supabase
      .schema('core')
      .from('personality_assessments')
      .select('luscher1_completed_at, luscher2_completed_at, next_luscher_test_available_at')
      .eq('user_id', user.id)
      .single()

    if (error && error.code !== 'PGRST116') {
      return c.json(
        { error: 'Internal server error', message: `Failed to fetch availability: ${error.message}` },
        500
      )
    }

    const isCompleted = !!(data?.luscher1_completed_at && data?.luscher2_completed_at)
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to get test availability' }, 500)
  }
})
...
app.openapi(saveLuscher1Route, async (c) => {
  ...
  try {
    ...
    if (error) {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to save Luscher Test 1: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to save Luscher Test 1' }, 500)
  }
})
...
app.openapi(saveIPIPProgressRoute, async (c) => {
  ...
  try {
    ...
    if (error) {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to save IPIP progress: ${error.message}` },
        500
      )
    }
      ...
      try {
        ...
        if (!archetypeError && archetypeData) {
          ...
          if (!existingXP) {
            ...
            await supabase.schema('core').from('users').update({ frequency_xp: newXP }).eq('id', user.id)
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to save IPIP progress' }, 500)
  }
})
...
app.openapi(saveLuscher2Route, async (c) => {
  ...
  try {
    ...
    const luscher1Completed = assessment?.luscher1_choices?.length === 8
    const ipipAnswers =
      (assessment?.ipip_answers as Array<{
        id: string
        domain: string
        facet: number
        score: number
    ...
    const completionScore = (luscher1Completed ? 25 : 0) + (ipipCompleted ? 25 : 0) + 25
    ...
    if (error) {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to save Luscher Test 2: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to save Luscher Test 2' }, 500)
  }
})
...
app.openapi(updateCurrentStepRoute, async (c) => {
  ...
  try {
    ...
    if (error) {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to update step: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to update step' }, 500)
  }
})
...
const saveLuscherTestSessionRoute = createRoute({
  ...
  description: 'Save unified Luscher test session (both parts + diary + XP + cooldown)',
  request: {
    body: {
      content: {
        'application/json': {
...
app.openapi(saveLuscherTestSessionRoute, async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to save test session' }, 500)
  }
})
...
app.openapi(generateReportRoute, async (c) => {
  ...
  try {
    ...
    if (!openaiApiKey) {
      return c.json({ error: 'Internal server error', message: 'OpenAI API key not configured' }, 500)
    }
    ...
    if (!response.ok) {
      ...
      return c.json(
        { error: 'Internal server error', message: 'Failed to generate report from OpenAI' },
        500
      )
    }
    ...
    if (!report) {
      return c.json(
        { error: 'Internal server error', message: 'No report generated from OpenAI' },
        500
      )
    }
    ...
    if (error) {
      ...
      return c.json(
        { error: 'Internal server error', message: `Failed to save report: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to generate report' }, 500)
  }
})
...
app.openapi(generateShareTokenRoute, async (c) => {
  ...
  try {
    ...
    if (!assessment?.ipip_completed_at) {
      return c.json(
        { error: 'Bad request', message: 'IPIP assessment must be completed before sharing' },
        400
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to generate share token' }, 500)
  }
})
...
app.openapi(revokeShareTokenRoute, async (c) => {
  ...
  try {
    ...
    if (error) {
      return c.json(
        { error: 'Internal server error', message: `Failed to revoke token: ${error.message}` },
        500
      )
    }
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to revoke share token' }, 500)
  }
})
...
app.openapi(awardResultsViewXPRoute, async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Internal server error', message: 'Failed to award results view XP' }, 500)
  }
})
