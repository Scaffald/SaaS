...
app.get("/analytics", async (c) => {
  ...
  const utilizationPercent = totalLimitBytes > 0
    ? Number(((totalBytes / totalLimitBytes) * 100).toFixed(1))
    : null;
  ...
  const topUsers = topUsersSource.map((row) => {
    ...
    const total = toNumber(row.total_bytes);
    const limit = toNumber(row.storage_limit_bytes);
    const usagePercentOfLimit = limit > 0
      ? Number(((total / limit) * 100).toFixed(1))
      : null;
  ...
  if (otherBytes > 0) {
    breakdownTotals.push({ key: "Unattributed", bytes: otherBytes });
  }
