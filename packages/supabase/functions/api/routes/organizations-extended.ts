...
async function getOrgAccess(
  ...
  orgId: string,
  requireAdmin = false
): Promise<{ error?: string; status?: number; isOwner?: boolean; isAdmin?: boolean }> {
  ...
  const isAdmin = isOwner || (assignments ?? []).some((a: any) =>
    (a.scope_org_id === orgId && ['owner', 'admin'].includes(a.role?.name ?? '')) ||
    (['admin', 'super_admin'].includes(a.role?.name ?? '') && a.role?.scope === 'platform')
  )

  // biome-ignore lint/suspicious/noExplicitAny: Complex type inference
  const isMember = isAdmin || (assignments ?? []).some((a: any) => a.scope_org_id === orgId)
...
app.get('/:id/invitations', async (c) => {
  ...
  if (access.error) return c.json({ error: access.error }, access.status as 403 | 404)
  ...
  let query = supabase
    .schema('core')
    .from('invites')
    .select('id, invitee_email, role_name, status, message, personal_note, viewed_at, expires_at, created_at, resent_count')
  ...
  if (error) return c.json({ error: 'Failed to load invitations', message: error.message }, 500)
...
app.post('/:id/invitations/:inviteId/resend', async (c) => {
  ...
  if (fetchError || !invite) return c.json({ error: 'Invitation not found' }, 404)
  ...
  if (updateError || !updated) {
    return c.json({ error: 'Failed to resend invitation', message: updateError?.message }, 500)
  }
...
app.post('/:id/invitations/:inviteId/cancel', async (c) => {
  ...
  if (updateError || !updated) {
    return c.json({ error: 'Failed to cancel invitation', message: updateError?.message }, 500)
  }
...
app.post('/invitations/accept', async (c) => {
  ...
  const { data: invite, error } = await supabase
    .schema('core')
    .from('invites')
    .select('id, organization_id, invitee_email, status, expires_at, role_name, metadata')
    .eq('token', token)
    .single()
  ...
  if (invite.invitee_email.toLowerCase() !== user.email.toLowerCase()) {
    return c.json({ error: 'This invitation is linked to a different email address.' }, 403)
  }
  ...
  if (!role) return c.json({ error: `Role "${roleName}" is not available.` }, 400)
  ...
  const { error: assignmentError } = await adminClient
    .schema('core')
    .from('role_assignments')
    .insert({ role_id: role.id, user_id: user.id, scope_org_id: invite.organization_id })

  if (assignmentError && assignmentError.code !== '23505') {
    return c.json({ error: 'Failed to add member', message: assignmentError.message }, 500)
  }
...
app.get('/:id/member-activity', async (c) => {
  ...
  const lookbackDays = Math.min(90, Math.max(1, parseInt(c.req.query('lookbackDays') ?? '30', 10)))
  ...
  const activityMap = new Map<string, { userId: string; actions: number; lastActionAt: string }>()
  // biome-ignore lint/suspicious/noExplicitAny: Complex type inference
  for (const entry of (data ?? []) as any[]) {
    if (!entry.actor_user_id) continue
    if (!activityMap.has(entry.actor_user_id)) {
      activityMap.set(entry.actor_user_id, { userId: entry.actor_user_id, actions: 0, lastActionAt: entry.created_at })
    }
...
app.post('/:id/transfer-ownership', async (c) => {
  ...
  if (!newOwnerUserId) return c.json({ error: 'newOwnerUserId is required' }, 400)
  if (newOwnerUserId === user.id) return c.json({ error: 'You are already the owner.' }, 400)
...
app.post('/:id/documents/:documentId/commit-version', async (c) => {
  ...
  const body = await c.req.json()
  const { versionId, storagePath, fileName: _fileName, mimeType, fileSize, checksum, notes } = body

  if (!versionId || !storagePath || !mimeType || !fileSize) {
    return c.json({ error: 'versionId, storagePath, mimeType, fileSize are required' }, 400)
  }
  ...
  if (docError || !document) return c.json({ error: 'Document not found' }, 404)
  if (document.organization_id !== id) return c.json({ error: 'Document belongs to a different organization.' }, 403)
  ...
  if (versionError || !version) {
    return c.json({ error: 'Failed to record document version', message: versionError?.message }, 500)
  }
...
app.get('/:id/documents/:documentId/versions', async (c) => {
  ...
  const { data, error } = await supabase
    .schema('core')
    .from('organization_document_versions')
    .select('id, version_number, size_bytes, mime_type, checksum, created_at, uploaded_by')
    .eq('organization_id', id)
    .eq('document_id', documentId)
    .order('version_number', { ascending: false })

  if (error) return c.json({ error: 'Failed to load versions', message: error.message }, 500)
...
app.get('/:id/documents/:documentId/shares', async (c) => {
  ...
  const { data, error } = await supabase
    ...
    .select('id, share_type, permission, target_user_id, external_email, expires_at, revoked_at, created_at, metadata')
    .eq('organization_id', id)
    .eq('document_id', documentId)
    .is('revoked_at', null)

  if (error) return c.json({ error: 'Failed to load document shares', message: error.message }, 500)
...
app.post('/:id/documents/:documentId/shares', async (c) => {
  ...
  const body = await c.req.json()
  const { permission = 'view', shareType = 'organization_member', targetUserId, externalEmail, expiresAt } = body

  if (shareType === 'organization_member' && !targetUserId) {
    return c.json({ error: 'Target user is required for organization member shares.' }, 400)
  }
  ...
  if (error || !share) {
    return c.json({ error: 'Failed to share document', message: error?.message }, 500)
  }
...
app.get('/:id/folders', async (c) => {
  ...
  if (error) return c.json({ error: 'Failed to load folders', message: error.message }, 500)
...
app.post('/:id/folders', async (c) => {
  ...
  if (folderId) {
    const { data, error } = await supabase
      .schema('core')
      .from('organization_folders')
      .update({ name, description: description ?? null, parent_folder_id: parentFolderId ?? null, depth, updated_by: user.id })
      .eq('id', folderId)
      .select('*')
      .single()
  ...
  if (error || !data) {
    return c.json({ error: 'Failed to create folder', message: error?.message }, 500)
  }
...
app.delete('/:id/folders/:folderId', async (c) => {
  ...
  if (typeof docCount === 'number' && docCount > 0) {
    return c.json({ error: 'Move or delete documents before removing the folder.' }, 400)
  }
  ...
  if (error) return c.json({ error: 'Failed to delete folder', message: error.message }, 500)
...
app.get('/:id/documents/search', async (c) => {
  ...
  if (error) return c.json({ error: 'Failed to search documents', message: error.message }, 500)
...
app.get('/:id/locations', async (c) => {
  ...
  let query = supabase
    ...
    .select('id, name, location_type, address, latitude, longitude, timezone, phone, email, is_active, metadata, created_at')
    .eq('organization_id', id)
    .order('name', { ascending: true })
  ...
  if (error) return c.json({ error: 'Failed to load locations', message: error.message }, 500)
...
app.post('/:id/locations', async (c) => {
  ...
  const body = await c.req.json()
  const { locationId, name, locationType = 'other', address, latitude, longitude, timezone, phone, email, isActive } = body
...
app.get('/:id/audit-log', async (c) => {
  ...
  let query = supabase
    .schema('core')
    .from('organization_audit_log')
    .select('id, action_type, target_type, target_id, actor_user_id, description, metadata, created_at')
    .eq('organization_id', id)
    .order('created_at', { ascending: false })
    .limit(limit)
  ...
  if (error) return c.json({ error: 'Failed to load audit log', message: error.message }, 500)
...
app.post('/:id/audit-log/export', async (c) => {
  ...
  let query = supabase
    .schema('core')
    .from('organization_audit_log')
    .select('created_at, action_type, target_type, target_id, actor_user_id, description')
    .eq('organization_id', id)
    .order('created_at', { ascending: false })
    .limit(500)
  ...
  if (error) return c.json({ error: 'Failed to export audit log', message: error.message }, 500)
  ...
  const rows = (data ?? []).map(
    (row: { created_at: string; action_type?: string | null; target_type?: string | null; target_id?: string | null; actor_user_id?: string | null; description?: string | null }) => {
      const values = [
        row.created_at,
        row.action_type ?? '',
        row.target_type ?? '',
...
app.get('/:id/storage-usage', async (c) => {
  ...
  const [{ data: usage }, { data: tier }, { data: override }] = await Promise.all([
    supabase
      .schema('core')
      .from('organization_storage_usage')
  ...
  const maxStorageBytes = override?.storage_bytes ?? tier?.max_storage_bytes ?? 10 * 1024 * 1024 * 1024
  const maxFileBytes = override?.file_bytes ?? tier?.max_file_bytes ?? 25 * 1024 * 1024
  const warningThresholds = tier?.soft_warning_thresholds ?? [0.75, 0.9, 0.95, 0.99]

  const percentUsed = storageBytes / maxStorageBytes
  const warnings = warningThresholds.filter((level: number) => percentUsed >= level)
