...
const STEP_COMPLETION_CHECKS: StepCompletionChecks = {
  ...
  certifications: (data) =>
    ...
    data.certifications.some((cert) =>
      Boolean(cert?.name?.trim() && cert?.issuer?.trim())
    ),
  preferences: (data) =>
    Boolean(
  ...
  education: (data) =>
    Boolean(data?.degreeType?.trim() || data?.institutionName?.trim()),
};
...
async function loadStoredProgress(
  ...
  const stored = data?.wizard_progress as
    | Partial<ProfileWizardProgress>
    | null
    | undefined;
  if (!stored) {
    return {
  ...
  const lastCompletedStep = effectiveCompletedSteps.length > 0
