...
app.patch('/:id', zValidator('json', updateJobBodySchema), async (c) => {
  ...
  if (!superAdmin) {
    ...
    if (!orgAccess) {
      ...
      if (!teamAccess) {
        return c.json({ error: 'Forbidden', message: 'You do not have access to this job' }, 403)
      }
    }
  }
  ...
  if (input.remote_option !== undefined) updateData.remote_option = input.remote_option
  if (input.position_level !== undefined) updateData.position_level = input.position_level
  if (input.location !== undefined) updateData.location = input.location
  if (input.pay_range_min_cents !== undefined) updateData.pay_range_min_cents = input.pay_range_min_cents
  if (input.pay_range_max_cents !== undefined) updateData.pay_range_max_cents = input.pay_range_max_cents
  if (input.pay_range_type !== undefined) updateData.pay_range_type = input.pay_range_type
...
app.delete('/:id', async (c) => {
  ...
  if (!superAdmin) {
    ...
    const { data: ownedOrgs } = await supabaseAdmin
      .schema('core').from('organizations').select('id').eq('owner_user_id', user.id)
    for (const org of ownedOrgs ?? []) { if (org.id) organizationIds.add(org.id as string) }

    const { data: teamMemberships } = await supabaseAdmin
      .schema('core').from('team_members').select('teams!inner(organization_id)')
  ...
  await supabaseAdmin.schema('core').from('job_skills').delete().eq('job_id', id)
  await supabaseAdmin.schema('core').from('applications').delete().eq('job_id', id)

  const { error } = await supabaseAdmin.schema('core').from('jobs').delete().eq('id', id)
...
app.post('/:id/duplicate', async (c) => {
  ...
  const {
    ...
    posted_at: _posted_at, created_by_user_id: _created_by_user_id,
    ...jobData
  ...
  if (createError) {
    return c.json({ error: 'Internal Server Error', message: createError.message }, 500)
  }

  const { data: certifications } = await supabaseAdmin
    .schema('core').from('job_certifications').select('certification_id, is_required').eq('job_id', id)
  ...
  if (teamAssignments && teamAssignments.length > 0) {
    await supabaseAdmin.schema('core').from('job_team_assignments').insert(
      teamAssignments.map((a: { team_id: string; is_primary?: boolean | null; role_key?: string | null; organization_id: string }) => ({
        job_id: (newJob as { id: string }).id,
        team_id: a.team_id,
