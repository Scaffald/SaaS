...
const ratePostRoute = createRoute({
  ...
  summary: "Rate post",
  description:
    "Rate a critique or showcase post (5-star + optional sub-dimensions)",
  request: {
...
app.openapi(ratePostRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json(
      { error: "Failed to rate post", message: error.message },
      500,
    );
  }

  // Award reputation to rater
  const isDetailed = body.quality_rating && body.technique_rating &&
    body.creativity_rating;
  const raterDelta = isDetailed ? 10 : 3;
  const raterAction = isDetailed
...
app.openapi(listRatingsRoute, async (c) => {
  ...
  const raterIds = [
    ...new Set((ratings || []).map((r: Record<string, unknown>) => r.rater_id)),
  ];
...
app.openapi(ratingSummaryRoute, async (c) => {
  ...
  return c.json({
    data: {
      rating_avg: avg(
        all.map((r: Record<string, unknown>) => r.base_rating as number),
      ),
      rating_count: count,
      quality_avg: avg(
        all.filter((r: Record<string, unknown>) => r.quality_rating != null)
          .map((r: Record<string, unknown>) => r.quality_rating as number),
      ),
      technique_avg: avg(
        all.filter((r: Record<string, unknown>) => r.technique_rating != null)
          .map((r: Record<string, unknown>) => r.technique_rating as number),
      ),
      creativity_avg: avg(
        all.filter((r: Record<string, unknown>) => r.creativity_rating != null)
          .map((r: Record<string, unknown>) => r.creativity_rating as number),
      ),
      distribution,
    },
  });
