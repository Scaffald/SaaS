...
app.openapi(
  ...
  async (c) => {
    ...
    const { search, industry, location, limit = 20, offset = 0 } = c.req.valid('query')

    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401)
    }

    let query = supabase.schema('core').from('organizations').select('*', { count: 'exact' })
    ...
    if (error) {
      return c.json({ error: 'Failed to fetch employers', message: error.message }, 500)
    }
...
app.openapi(
  ...
  async (c) => {
    ...
    if (error) {
      return c.json({ error: 'Failed to claim employment', message: error.message }, 500)
    }
