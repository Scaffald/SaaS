...
app.openapi(getSummaryRoute, async (c) => {
  ...
  const sumByType = (data: typeof rows, type: string) =>
    data.filter((r) => r.event_type === type).reduce(
      (sum, r) => sum + (r.count || 0),
      0,
    );
  ...
  const sparklineByType = (data: typeof rows, type: string) =>
    buildSparkline(
      data.filter((r) => r.event_type === type).map((r) => ({
        date: r.date,
        count: r.count || 0,
      })),
      days,
  ...
  const profileViewsCurrent = sumByType(rows, "profile_view") +
    sumByType(rows, "profile_view_direct");
  const profileViewsPrev = sumByType(prevRows, "profile_view") +
    sumByType(prevRows, "profile_view_direct");
  const jobViewsCurrent = sumByType(rows, "job_view");
  const jobViewsPrev = sumByType(prevRows, "job_view");
  const appsCurrent = sumByType(rows, "application_start") +
    sumByType(rows, "application_complete");
  const appsPrev = sumByType(prevRows, "application_start") +
    sumByType(prevRows, "application_complete");
  const searchAppCurrent = sumVisibility(visRows);
  const searchAppPrev = sumVisibility(prevVisRows);

  const totalCurrent = profileViewsCurrent + jobViewsCurrent + appsCurrent +
    searchAppCurrent;
  ...
  const pvSparkline = buildSparkline(
    rows
      .filter((r) =>
        r.event_type === "profile_view" ||
        r.event_type === "profile_view_direct"
      )
      .reduce(
        (acc, r) => {
  ...
  return c.json({
    profileViews: {
      total: profileViewsCurrent,
      previous: profileViewsPrev,
    ...
    applications: {
      ...
      sparkline: buildSparkline(
        rows
          .filter((r) =>
            r.event_type === "application_start" ||
            r.event_type === "application_complete"
          )
          .reduce(
            (acc, r) => {
              const existing = acc.find((a) => a.date === r.date);
    ...
    totalEngagement: {
      total: totalCurrent,
      previous: totalPrev,
      sparkline: pvSparkline.map((v, i) =>
        v + (sparklineByType(rows, "job_view")[i] || 0)
      ),
    },
...
const getEngagementTimelineRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": { schema: engagementTimelineResponseSchema },
      },
    },
    401: {
      description: "Unauthorized",
...
app.openapi(getEngagementTimelineRoute, async (c) => {
  ...
  for (const row of rollups || []) {
    ...
    const entry = dateMap.get(dateKey) ??
      { profileViews: 0, jobViews: 0, applications: 0, searches: 0, total: 0 };
...
app.openapi(getVisitorsRoute, async (c) => {
  ...
  for (const row of industryData || []) {
    const name = (row as { industry?: { name?: string } }).industry?.name ||
      "Unknown";
...
const getVisibilityTimelineRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": { schema: visibilityTimelineResponseSchema },
      },
    },
    401: {
      description: "Unauthorized",
...
app.openapi(getVisibilityTimelineRoute, async (c) => {
  ...
  for (const row of rollups || []) {
    ...
    const entry = dateMap.get(dateKey) ??
      { searchResults: 0, recommendations: 0, feedAppearances: 0, clicks: 0 };
  ...
  const timeline = Array.from(dateMap.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([date, data]) => {
      const totalImpressions = data.searchResults + data.recommendations +
        data.feedAppearances;
      return {
        date,
        ...data,
        ctr: totalImpressions > 0
          ? Math.round((data.clicks / totalImpressions) * 10000) / 100
          : 0,
      };
    });
...
app.openapi(getSearchKeywordsRoute, async (c) => {
  ...
  for (const row of impressions || []) {
    ...
    const entry = queryMap.get(query) ??
      { impressions: 0, clicks: 0, totalPosition: 0 };
    entry.impressions++;
  ...
  const keywords = Array.from(queryMap.entries())
    .map(([query, data]) => ({
      ...
      ctr: data.impressions > 0
        ...
        : 0,
      avgPosition: data.impressions > 0
        ? Math.round((data.totalPosition / data.impressions) * 10) / 10
        : 0,
    }))
...
const trackImpressionRoute = createRoute({
  ...
  description:
    "Track when a user profile appears in search results (server-side)",
  request: {
    body: {
...
app.openapi(trackImpressionRoute, async (c) => {
  const supabase = c.get("supabase");
  const user = c.get("user");
  const { userId, impressionType, searchQuery, searchFilters, position } = c.req
    .valid("json");
