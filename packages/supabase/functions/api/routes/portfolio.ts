...
import { OpenAPIHono, createRoute as createOpenAPIRoute, z } from '@hono/zod-openapi'
...
app.openapi(listRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to fetch portfolio', message: error.message }, 500)
  }
...
app.openapi(createRoute, async (c) => {
  ...
  const displayOrder = body.displayOrder ?? ((maxItem?.display_order ?? -1) + 1)
  ...
  if (error) {
    return c.json({ error: 'Failed to create portfolio item', message: error.message }, 500)
  }
...
app.openapi(updateRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to update portfolio item', message: error.message }, 500)
  }
...
app.openapi(deleteRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to delete portfolio item', message: error.message }, 500)
  }
