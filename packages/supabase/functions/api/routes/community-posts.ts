...
const postSchema = z
  .object({
    ...
    status: z.enum(['draft', 'pending_moderation', 'published', 'flagged', 'removed']),
    title: z.string(),
...
const listQuerySchema = z.object({
  ...
  sort: z.enum(['recent', 'top_rated', 'most_upvoted']).optional().default('recent'),
})
...
async function enrichPosts(
  ...
  if (userId) {
    ...
    upvotedPostIds = new Set((upvotes || []).map((u: Record<string, unknown>) => u.target_id as string))
    ...
    bookmarkedPostIds = new Set((bookmarks || []).map((b: Record<string, unknown>) => b.post_id as string))
  }
...
app.openapi(getCommunityFeedRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to fetch feed', message: error.message }, 500)
  }

  const enriched = await enrichPosts(supabase, posts || [], user?.id)

  const lastPost = posts?.[posts.length - 1]
  const next_cursor = lastPost ? (lastPost as Record<string, unknown>).created_at : null
...
app.openapi(getPublishedFeedRoute, async (c) => {
  ...
  const lastPost = posts?.[posts.length - 1]
  const next_cursor = lastPost ? (lastPost as Record<string, unknown>).published_at : null
...
app.openapi(createPostRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to create post', message: error.message }, 500)
  }
...
app.openapi(updatePostRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to update post', message: error.message }, 500)
  }
...
app.openapi(submitPostRoute, async (c) => {
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to submit post', message: error.message }, 500)
  }
  ...
  if (isPublished) {
    ...
    const action = allTiersTagged ? 'post_published_full_taxonomy' : 'post_published'
...
app.openapi(publishPostRoute, async (c) => {
  ...
  if (existing.post_type === 'advice') {
    return c.json({ error: 'Advice posts cannot be published to profile' }, 400)
  }
  ...
  if (existing.post_type === 'critique' && existing.comment_count === 0 && existing.rating_count === 0) {
    return c.json({ error: 'Critique posts need feedback before publishing to profile' }, 400)
  }
  ...
  if (error) {
    ...
    return c.json({ error: 'Failed to publish post', message: error.message }, 500)
  }
  ...
  await supabase.rpc('update_scaffold_score', {
    p_user_id: user.id,
    p_delta: delta,
    p_action: allTiersTagged ? 'post_published_full_taxonomy' : 'post_published',
...
app.openapi(unpublishPostRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to unpublish', message: error.message }, 500)
  }
...
app.openapi(deletePostRoute, async (c) => {
  ...
  if (error) {
    return c.json({ error: 'Failed to delete post', message: error.message }, 500)
  }
