...
Deno.test("GET /v1/profiles/experience - returns list for authenticated user", async () => {
  ...
  if (!token) {
    throw new Error(
      "Test user test@example.com not found. Run pnpm supa db reset to seed 002a_seed-api-test-user.sql",
    );
  }
...
Deno.test("POST /v1/profiles/experience - saves experience entries", async () => {
  ...
  assert(
    Array.isArray(
      (response.body as { experience_entries?: unknown[] }).experience_entries,
    ),
  );
});
