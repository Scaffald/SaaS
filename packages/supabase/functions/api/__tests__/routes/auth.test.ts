...
Deno.test('POST /v1/auth/magic-link - sends magic link for new user (signup mode)', async () => {
  ...
  assertEquals(response.body.data.redirectTo, 'https://app.example.com/auth/callback')
...
Deno.test('GET /v1/auth/roles - returns user roles successfully', async () => {
  ...
  const user = await registerUserWithMagicLink(`test-roles-${Date.now()}@example.com`)
...
Deno.test('GET /v1/auth/roles - returns empty array for user with no roles', async () => {
  ...
  const user = await registerUserWithMagicLink(`test-noroles-${Date.now()}@example.com`)
  assertExists(user, 'User should be created')
...
Deno.test('GET /v1/auth/roles - handles database errors gracefully (500)', async () => {
  ...
  const user = await registerUserWithMagicLink(`test-db-error-${Date.now()}@example.com`)
  assertExists(user, 'User should be created')
...
Deno.test('GET /v1/auth/session - returns session info successfully', async () => {
  ...
  const user = await registerUserWithMagicLink(`test-session-${Date.now()}@example.com`)
  assertExists(user, 'User should be created')
...
Deno.test('GET /v1/auth/roles - filters out null role names', async () => {
  ...
  const user = await registerUserWithMagicLink(`test-null-roles-${Date.now()}@example.com`)
  assertExists(user, 'User should be created')
  ...
  assert(response.body.data.roles.every((role: unknown) => typeof role === 'string'))
