...
Deno.test("GET /v1/jobs - filters by status", async () => {
  ...
  assert(
    publishedResponse.body.data.every((job: { status: string }) =>
  ...
  assert(
    draftResponse.body.data.every((job: { status: string }) =>
      job.status === "draft"
    ),
  );
...
Deno.test("GET /v1/jobs - filters by location (partial match)", async () => {
  ...
  assertSuccessResponse(response);
  assert(
    response.body.data.every((job: { location: string }) =>
...
Deno.test("GET /v1/jobs - filters by remote_option", async () => {
  ...
  assertSuccessResponse(response);
  assert(
    response.body.data.every((job: { remote_option: string }) =>
...
Deno.test("GET /v1/jobs/:id/similar - returns similar jobs from same organization", async () => {
  ...
  assert(
    !response.body.data.some((j: { id: string }) => j.id === sourceJob.id),
  );
