...
import { createTestApiKey, cleanupCurrentTestData } from '../helpers/fixtures.ts'
...
Deno.test('POST /v1/api-keys - returns 403 if authenticated with API key', async () => {
  ...
  const user = await createTestUserWithOrg()
  const apiKey = await createTestApiKey({ organization_id: user.organization.id })
  ...
  assert(response.body.message?.includes('cannot be created using another API key'))
...
Deno.test('GET /v1/api-keys/:id - returns 403 if user not in same organization', async () => {
  ...
  const apiKey = await createTestApiKey({ organization_id: user1.organization.id })
...
Deno.test('GET /v1/api-keys/:id - allows access with API key from same org', async () => {
  ...
  const user = await createTestUserWithOrg()
  const apiKey1 = await createTestApiKey({ organization_id: user.organization.id })
  const apiKey2 = await createTestApiKey({ organization_id: user.organization.id })
...
Deno.test('PATCH /v1/api-keys/:id - returns 403 if not org admin', async () => {
  ...
  const admin = await createTestUserWithOrg({ email: 'admin@example.com', user_type: 'employer' })
  const user = await createTestUserWithOrg({ email: 'jobseeker@example.com', user_type: 'job_seeker' })
  ...
  const apiKey = await createTestApiKey({ organization_id: admin.organization.id })
...
Deno.test('DELETE /v1/api-keys/:id - returns 403 if not org admin', async () => {
  ...
  const admin = await createTestUserWithOrg({ email: 'admin@example.com', user_type: 'employer' })
  const user = await createTestUserWithOrg({ email: 'user@example.com', user_type: 'job_seeker' })
...
Deno.test('GET /v1/api-keys/:id/usage - returns 403 if not in same organization', async () => {
  ...
  const user1 = await createTestUserWithOrg({ email: 'user1-usage@example.com' })
  const user2 = await createTestUserWithOrg({ email: 'user2-usage@example.com' })
