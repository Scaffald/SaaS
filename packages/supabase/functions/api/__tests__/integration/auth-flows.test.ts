...
Deno.test('AUTH FLOW: Complete OAuth 2.0 authorization code flow with PKCE', async () => {
  ...
  if (authorizeResponse.body.consent_required) {
    ...
    const codeHash = codeHashArray.map((b) => b.toString(16).padStart(2, '0')).join('')
  ...
  assertExists(tokenResponse.body.refresh_token, 'Should receive refresh token')
  ...
  assertExists(refreshResponse.body.access_token, 'Should receive new access token')
  assertExists(refreshResponse.body.refresh_token, 'Should receive new refresh token')
...
Deno.test('AUTH FLOW: API key creation and usage', async () => {
  ...
  assert(listKeysResponse.body.data.some((k: { name: string }) => k.name === 'Integration Test Key'))
