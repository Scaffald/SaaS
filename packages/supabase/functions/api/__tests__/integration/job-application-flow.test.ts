...
Deno.test('JOB FLOW: Complete job discovery to application submission', async () => {
  ...
  const user = await registerUserWithMagicLink(`jobseeker-${timestamp}@example.com`)
  ...
  assert(browseResponse.body.data.length >= 1, 'Should find at least 1 remote job in SF')
  ...
  const similarJobsResponse = await client.get(`/v1/jobs/${targetJob.id}/similar`, {
    query: { limit: 5 },
  })
  ...
  const getApplicationResponse = await client.get(`/v1/applications/${applicationId}`)
  ...
  assertEquals(getApplicationResponse.body.data.job.title, 'Senior Software Engineer')

  // Step 7: Update application with additional info
  const updateResponse = await client.patch(`/v1/applications/${applicationId}`, {
    cover_letter: 'I am very excited about this opportunity and believe my skills align perfectly with your needs.',
  })
  ...
  const withdrawResponse = await client.post(`/v1/applications/${applicationId}/withdraw`)
  ...
  const attemptUpdateResponse = await client.patch(`/v1/applications/${applicationId}`, {
...
Deno.test('JOB FLOW: Duplicate application prevention and re-application', async () => {
  ...
  const user = await registerUserWithMagicLink(`duplicate-test-${timestamp}@example.com`)
  assertExists(user)
  ...
  assert(reapplication.body.data.id !== applicationId, 'Should create new application')
...
Deno.test('JOB FLOW: Application deadline enforcement', async () => {
  ...
  const { data: job } = await admin
    ...
    .insert({
      ...
      status: 'published',
      application_deadline: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), // Yesterday
    })
    .select()
    .single()

  const user = await registerUserWithMagicLink(`deadline-test-${timestamp}@example.com`)
  assertExists(user)
...
Deno.test('JOB FLOW: Progressive job search refinement', async () => {
  ...
  const user = await registerUserWithMagicLink(`search-test-${timestamp}@example.com`)
  const client = createTestClient({ authToken: user.token })
  ...
  assert(sfFullTimeCount < fullTimeCount, 'Adding location should refine results')
  ...
  assert(remoteCount < sfFullTimeCount, 'Adding remote option should further refine')
