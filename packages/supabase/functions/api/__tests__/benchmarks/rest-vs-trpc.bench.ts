...
Deno.bench({
  ...
  async fn() {
    ...
    const user = await registerUserWithMagicLink(
      `bench-jobs-${Date.now()}@example.com`,
    );
...
Deno.bench({
  ...
  async fn() {
    ...
    const user = await registerUserWithMagicLink(
      `bench-job-detail-${Date.now()}@example.com`,
    );
    const client = createTestClient({ authToken: user?.token });
...
Deno.bench({
  ...
  async fn() {
    ...
    const org = await createTestOrganization();
    const _job = await createTestJob({
      organization_id: org.id,
      status: "published",
    });

    const user = await registerUserWithMagicLink(
      `bench-app-create-${Date.now()}@example.com`,
    );
    const client = createTestClient({ authToken: user?.token });
...
Deno.bench({
  ...
  async fn() {
    ...
    const user = await registerUserWithMagicLink(
      `bench-concurrent-${Date.now()}@example.com`,
    );
    const client = createTestClient({ authToken: user?.token });
    ...
    const requestsPerSecond = (10 * BENCHMARK_CONFIG.concurrent) /
      (totalTime / 1000);
...
Deno.bench({
  ...
  async fn() {
    ...
    const user = await registerUserWithMagicLink(
      `bench-filter-${Date.now()}@example.com`,
    );
    const client = createTestClient({ authToken: user?.token });
...
export async function generateBenchmarkReport(): Promise<void> {
  ...
  await createTestJob({ organization_id: org.id, status: "published" });
  const user = await registerUserWithMagicLink(
    `bench-report-${Date.now()}@example.com`,
  );
  const client = createTestClient({ authToken: user?.token });
  ...
  if (listAvg > 200) {
    console.log(
      "⚠️  List endpoint P95 > 200ms - consider adding database indexes",
    );
  } else {
  ...
  if (detailAvg > 100) {
    console.log(
      "⚠️  Detail endpoint P95 > 100ms - consider query optimization",
    );
  } else {
