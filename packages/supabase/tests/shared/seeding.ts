...
export async function withSeedClient<T>(handler: (client: SeedClient) => Promise<T>): Promise<T> {
  const client = createSeedClient()
  return await handler(client)
}

export async function truncateTables(client: SeedClient, tables: string[]): Promise<void> {
