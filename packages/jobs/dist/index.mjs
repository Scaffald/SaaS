...
var CompensationFrequencySchema = z.enum(["hour", "day", "week", "month", "year", "total"]);
var CompensationSchema = z.object({
  currency: z.string().trim().length(3, "Use ISO-4217 currency codes").transform((value) => value.toUpperCase()),
...
var DateTimeSchema = z.preprocess((value) => {
...
var OrganizationIdentifierSchema = z2.object({
  externalId: z2.string().trim().min(1).optional(),
  source: z2.string().trim().min(1).optional(),
  slug: z2.string().trim().min(1).optional()
}).refine(
  (value) => Boolean(value.externalId || value.source || value.slug),
  "At least one organization identifier must be provided"
);
var OrganizationSizeSchema = z2.object({
  label: z2.string().trim().min(1).optional(),
  min: z2.number().int().nonnegative().optional(),
  max: z2.number().int().nonnegative().optional()
}).superRefine((value, ctx) => {
...
var SocialProfileSchema = z2.object({
  type: z2.enum([
    "website",
    "linkedin",
    "twitter",
  ...
  url: z2.string().url(),
  handle: z2.string().trim().min(1).optional()
});
var NormalizedOrganizationSchema = z2.object({
  id: z2.string().trim().min(1).optional(),
  identifier: OrganizationIdentifierSchema.optional(),
  name: z2.string().trim().min(1),
  legalName: z2.string().trim().min(1).optional(),
  description: z2.string().optional(),
  websiteUrl: z2.string().url().optional(),
  careersUrl: z2.string().url().optional(),
  logoUrl: z2.string().url().optional(),
  industries: z2.array(z2.string().trim().min(1)).optional(),
  size: OrganizationSizeSchema.optional(),
  headquarters: LocationSchema.optional(),
  locations: z2.array(LocationSchema).optional(),
  remoteFriendly: z2.boolean().optional(),
  foundedAt: DateTimeSchema.optional(),
  emails: z2.array(z2.string().email()).optional(),
  phoneNumbers: z2.array(z2.string().trim().min(1)).optional(),
  socialProfiles: z2.array(SocialProfileSchema).optional(),
...
var ExperienceLevelSchema = z3.enum([
  "intern",
  "entry",
  "mid",
  "senior",
...
var WorkplaceTypeSchema = z3.enum(["onsite", "remote", "hybrid", "flexible"]);
var JobStatusSchema = z3.enum(["open", "closed", "draft"]);
var JobIdentifierSchema = z3.object({
  externalId: z3.string().trim().min(1),
  source: z3.string().trim().min(1),
  url: z3.string().url().optional()
});
var NormalizedJobSchema = z3.object({
  id: z3.string().trim().min(1).optional(),
  identifier: JobIdentifierSchema,
  title: z3.string().trim().min(1),
  url: z3.string().url(),
  description: z3.string().trim().min(1).optional(),
  summary: z3.string().trim().min(1).optional(),
  language: z3.string().trim().min(1).optional(),
  applicationUrl: z3.string().url().optional(),
  ...
  locations: z3.array(LocationSchema).nonempty().optional(),
  primaryLocation: LocationSchema.optional(),
  remote: z3.boolean().optional(),
  tags: z3.array(z3.string().trim().min(1)).optional(),
  skills: z3.array(z3.string().trim().min(1)).optional(),
  benefits: z3.array(z3.string().trim().min(1)).optional(),
  keywords: z3.array(z3.string().trim().min(1)).optional(),
...
var buildJobSourceRecord = (job, provider, jobId, firstSeenIso, timestampIso) => {
  ...
  const payloadHash = createHash2("sha256").update(JSON.stringify(preparedJob)).digest("hex");
  return {
    job_id: jobId,
    provider,
