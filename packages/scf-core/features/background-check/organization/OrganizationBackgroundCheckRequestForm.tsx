...
type OrganizationSummary =
  RouterOutputs["office"]["getOrganizations"]["organizations"][number];
...
export function OrganizationBackgroundCheckRequestForm() {
  ...
  const costCents =
    selectedPackage?.retail_cost_cents ??
    selectedPackage?.platform_cost_cents ??
    0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await requestPaymentMutation.mutateAsync({
        ...
        paid_by: "employer",
        consent: {
          consent_signature: "organization_request",
    ...
    <ScrollView style={{ flex: 1 }}>
      <Stack flex={1} gap={16} paddingHorizontal={16} paddingVertical={24}>
        <Stack gap={4}>
          ...
          <Text style={{ color: colors.text[theme].secondary }}>
            Invite a worker to complete the required screening package on behalf
        ...
        <Stack gap={12}>
          ...
          <Stack gap={8}>
            ...
            <ResponsiveSelect
              ...
              placeholder={
                selectedPackage
                  ? `${selectedPackage.display_name} (${formatCurrency(
                      selectedPackage.retail_cost_cents
                    )})`
                  : "Select package"
              }
              ...
              options={packages.map((pkg) => ({
                value: pkg.id,
                label: `${pkg.display_name} · ${formatCurrency(
                  pkg.retail_cost_cents
                )}`,
              }))}
            />
          ...
          >
            ...
            <Text style={{ color: colors.text[theme].secondary }}>
              Charges are collected immediately via Stripe. Screenings are
              submitted after payment succeeds.
            </Text>
          </Stack>
          ...
          {paymentSession && (
            <Stack gap={8}>
              ...
              <PaymentIntentForm
                ...
                description={`Background check for ${
                  selectedWorker?.display_name ?? "selected worker"
                }`}
                submitLabel={
