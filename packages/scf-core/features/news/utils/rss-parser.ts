...
function normalizeAtomFormat(feed: AtomFeed): ParsedRSSFeed {
  ...
  const feedTitle =
    typeof feed.title === 'object' && feed.title !== null && '#text' in feed.title
      ? feed.title['#text']
  ...
  const feedSubtitle =
    typeof feed.subtitle === 'object' && feed.subtitle !== null && '#text' in feed.subtitle
      ? feed.subtitle['#text']
...
function parseDate(dateString: Date | string): Date | null {
  ...
  const dateStr =
    typeof dateString === 'object' && dateString !== null && '#text' in dateString
