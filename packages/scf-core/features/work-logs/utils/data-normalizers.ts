...
const toProjectSelectorOrganization = (
  ...
  return {
    ...
    name,
    isAdmin: Boolean(
      (input as { isAdmin?: unknown }).isAdmin ??
        (input as { is_admin?: unknown }).is_admin,
    ),
    isOwner: Boolean(
      (input as { isOwner?: unknown }).isOwner ??
        (input as { is_owner?: unknown }).is_owner,
    ),
  };
};
...
const toProjectSelectorProject = (
  ...
  const organizationId =
    typeof (input as { organizationId?: unknown }).organizationId === "string"
      ...
      : typeof (input as { organization_id?: unknown }).organization_id ===
          "string"
  ...
  const endsAt = typeof (input as { endsAt?: unknown }).endsAt === "string"
...
export const normalizeProjectOptions = (input: unknown): ProjectOptionsData => {
  ...
  const organizationsSource =
    (input as { organizations?: unknown }).organizations;
...
export const extractExplicitSkills = (
  input: unknown,
): ExplicitSkillRecord[] => {
  if (!input || typeof input !== "object") {
    return [];
  ...
  return explicitSkills.filter(
    (skill): skill is ExplicitSkillRecord =>
      Boolean(skill) && typeof skill === "object",
  );
};

export const mapExplicitSkillsToOptions = (
  input: unknown,
