...
const parseJson = <T>(rawValue: string | null): T | null => {
  ...
  try {
    ...
    console.warn('[work-logs/offline-storage] Failed to parse JSON value', error)
    return null
  }
}
...
export const loadOfflineWorkLogs = async (): Promise<OfflineWorkLog[]> => {
  try {
    ...
    console.warn('[work-logs/offline-storage] Unable to read offline work logs', error)
    return []
  }
}

export const saveOfflineWorkLogs = async (workLogs: OfflineWorkLog[]): Promise<void> => {
  try {
    await AsyncStorage.setItem(OFFLINE_WORK_LOGS_KEY, JSON.stringify(workLogs))
  } catch (error) {
    console.warn('[work-logs/offline-storage] Unable to persist offline work logs', error)
  }
}
...
export const clearOfflineWorkLogs = async (): Promise<void> => {
  try {
    ...
    console.warn('[work-logs/offline-storage] Unable to clear offline work logs', error)
  }
}
...
export const loadSyncSettings = async (): Promise<SyncSettings> => {
  try {
    ...
    console.warn('[work-logs/offline-storage] Unable to read sync settings', error)
...
export const saveSyncSettings = async (settings: SyncSettings): Promise<void> => {
  try {
    await AsyncStorage.setItem(WORK_LOG_SYNC_SETTINGS_KEY, JSON.stringify(settings))
  } catch (error) {
    console.warn('[work-logs/offline-storage] Unable to persist sync settings', error)
  }
}
