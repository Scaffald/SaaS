...
const buildUpdatePayloadFromCreate = (input: CreateWorkLogInput): UpdateWorkLogInput['payload'] => {
  return {
...
export interface UseWorkLogFormReturn {
  ...
  timeEntryFields: ReturnType<typeof useFieldArray<CreateWorkLogInput, 'timeEntries'>>['fields']
  addTimeEntry: () => void
...
export const useWorkLogForm = ({
  ...
  const { queueWorkLog, mutateOfflineWorkLog, markWorkLogForSync, offlineWorkLogs } =
    useOfflineWorkLogs()
  ...
  const createWorkLogMutation = useCreateWorkLogMutation({
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to save work log draft.'
      setAutoSaveStatus({
        state: 'error',
        message,
      })
  ...
  const updateWorkLogMutation = useUpdateWorkLogMutation({
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to update work log draft.'
      setAutoSaveStatus({
        state: 'error',
        message,
      })
  ...
  const performAutoSave = useCallback(
    async (payload: CreateWorkLogInput) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to auto-save work log.'
