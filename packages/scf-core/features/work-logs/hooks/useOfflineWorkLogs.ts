...
interface UseOfflineWorkLogsResult {
  ...
  isLoading: boolean;
  queueWorkLog: (
    options: QueueOfflineWorkLogOptions,
  ) => Promise<OfflineWorkLog>;
  ...
  const copyPhotoToCache = useCallback(
    async (
      ...
      if (!FileSystem || !directory || !isNativePlatform) {
        const guessedFileName = photo.fileName ??
      ...
      const fileName = photo.fileName ?? getFileNameFromUri(photo.uri) ??
