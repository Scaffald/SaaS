...
const createTeamMock = vi.hoisted(() => ({
  mutateAsync: vi.fn(),
  useMutation: vi.fn(),
}));
const updateTeamMock = vi.hoisted(() => ({
  mutateAsync: vi.fn(),
  useMutation: vi.fn(),
}));
...
vi.mock("lucide-react-native", async (importOriginal) => ({
  ...
  ClipboardCheck: () => (
    <span data-testid="clipboard-check-icon">ClipboardCheck</span>
  ),
...
vi.mock("@scaffald/ui", async () => {
  const actual = await vi.importActual<typeof import("@scaffald/ui")>(
    "@scaffald/ui"
  );
  ...
  SelectBase.Trigger = ({ children }: { children: ReactNode }) => (
    <div>{children}</div>
  );
  SelectBase.Value = ({ children }: { children?: ReactNode }) => (
    <span>{children}</span>
  );
  SelectBase.Content = ({ children }: { children: ReactNode }) => (
    <div>{children}</div>
  );
  SelectBase.ScrollUpButton = () => null;
  SelectBase.ScrollDownButton = () => null;
  SelectBase.Viewport = ({ children }: { children: ReactNode }) => (
    <div>{children}</div>
  );
  SelectBase.Group = ({ children }: { children: ReactNode }) => (
    <div>{children}</div>
  );
  SelectBase.Label = ({ children }: { children: ReactNode }) => (
    <div>{children}</div>
  );
  SelectBase.Item = ({
    value,
  ...
  SelectBase.ItemText = ({ children }: { children: ReactNode }) => (
    <span>{children}</span>
  );
  SelectBase.ItemIndicator = ({ children }: { children: ReactNode }) => (
    <span>{children}</span>
  );
...
describe("TeamForm", () => {
  ...
  it("calls cancel handler when cancel button pressed", async () => {
    ...
    render(
      <TeamForm mode="create" organizationId="org-1" onCancel={onCancel} />
    );
  ...
  it.skip("displays validation errors", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.queryByText("Loading team options…")
