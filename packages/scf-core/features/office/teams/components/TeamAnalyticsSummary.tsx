...
export function TeamAnalyticsSummary({ teamId }: TeamAnalyticsSummaryProps) {
  ...
  const membersActiveAvg = metrics.length
    ? Math.round(
        metrics.reduce(
          (sum: number, day: TeamDailyMetric) =>
            sum + Number(day.members?.active ?? 0),
          0
        ) / metrics.length
      )
    : undefined;
  ...
  const totalApplicationsReviewed = metrics.reduce(
    (sum: number, day: TeamDailyMetric) =>
      sum + Number(day.applications?.reviewed ?? 0),
    0
  );
  ...
  const trendDescription =
    metrics.length > 1 && latest?.applications?.reviewed !== undefined
      ? (() => {
          ...
          const delta =
            Number(latest.applications.reviewed ?? 0) - Number(previous);
    ...
    <Stack gap={12}>
      ...
      {analyticsQuery.isLoading ? (
        ...
        <Stack gap={8}>
          ...
          <Text style={{ color: colors.text[theme].secondary }}>
            Metrics will appear once the team starts reviewing invitations and
            applications.
          </Text>
        </Stack>
      ) : (
        <Stack gap={12}>
          <Row gap={12} wrap>
            ...
            <StatTile label="Workload pressure">
              {workloadPressureLatest !== undefined &&
              workloadPressureLatest !== null
