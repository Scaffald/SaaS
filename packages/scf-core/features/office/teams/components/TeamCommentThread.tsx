...
export function TeamCommentThread({
  ...
  const postCommentMutation = usePostTeamCommentMutation({
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["teams", teamId, "analytics", "comments"],
      });
      await queryClient.invalidateQueries({
  ...
  const mentionLabel = useMemo(() => {
    ...
    return (
      mentionOptions.find((item) => item.id === selectedMentionId)?.label ??
      null
    );
  ...
  return (
    <Card
      ...
      padding="md"
    >
      <Stack gap={16} style={{ paddingHorizontal: 12 }}>
        <Stack gap={8}>
          <Row gap={8} align="center">
            <MessageCircle
              size={18}
              accessibilityLabel="Team discussion icon"
            />
            ...
            >
              {mentionOptions.map((option) => (
                <Button
                  ...
                  size="sm"
                  variant={
                    selectedMentionId === option.id ? "outline" : undefined
                  }
                  onPress={() =>
