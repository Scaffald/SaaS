...
export function TeamAutomationSettings({
  ...
  const initialState = useMemo<FormState>(
    () => ({
      allowSelfJoin,
      autoAssignJobs,
      invitationExpirationDays: Math.min(
        Math.max(invitationExpirationDays || 7, 1),
        90
      ),
      workloadStrategy: WORKLOAD_STRATEGIES.some(
        (item) => item.value === workloadStrategy
      )
        ? workloadStrategy
        : "manual",
      workloadSettings: {
        maxActiveAssignments: Number(
          (workloadSettings as Record<string, unknown> | undefined)
            ?.maxActiveAssignments ?? 10
        ),
        maxPendingAssignments: Number(
          (workloadSettings as Record<string, unknown> | undefined)
  ...
  useEffect(() => {
    ...
    updateMutation.mutate({
      ...
      params: {
        ...
        workloadStrategy: (debouncedFormState.workloadStrategy === "round_robin"
          ? "auto_round_robin"
          : debouncedFormState.workloadStrategy === "load_balance"
          ? "auto_balanced"
          : "manual") as "manual" | "auto_round_robin" | "auto_balanced",
        workloadSettings: debouncedFormState.workloadSettings,
        analyticsRefreshIntervalMinutes:
          debouncedFormState.analyticsRefreshIntervalMinutes,
      },
    });
  ...
  const statusLabel = updateMutation.isPending
    ? "Saving changes…"
    : "Settings up to date";
    ...
    <Stack gap={16}>
      <Stack gap={8}>
        ...
        <Text style={{ color: colors.text[theme].secondary }}>
          Configure how the team auto-assigns work and balances workloads across
          members.
        </Text>
      </Stack>
            ...
            onChangeText={(text) =>
              handleRangeInput(
                ...
                text,
                {
                  min: 1,
                  max: 90,
                  fallback: DEFAULT_FORM_STATE.invitationExpirationDays,
                }
              )
            }
          />
          ...
          >
            ...
            <Stack gap={8}>
              ...
              <Input
                keyboardType="numeric"
                value={String(
                  formState.workloadSettings.maxActiveAssignments ?? 10
                )}
                disabled={!canEdit || updateMutation.isPending}
                onChangeText={(text) =>
                  handleRangeInput(
                    (value) =>
                      handleWorkloadSettingChange(
            ...
            <Stack gap={8}>
              ...
              <Input
                keyboardType="numeric"
                value={String(
                  formState.workloadSettings.maxPendingAssignments ?? 15
                )}
                disabled={!canEdit || updateMutation.isPending}
                onChangeText={(text) =>
                  handleRangeInput(
            ...
            <Text style={{ color: colors.text[theme].secondary }}>
              When a member reaches these limits, assignments roll to the next
              available teammate.
            </Text>
          </Stack>
        ) : null}
            ...
            onChangeText={(text) =>
              handleRangeInput(
                ...
                text,
                {
                  min: 5,
                  max: 1440,
                  fallback: DEFAULT_FORM_STATE.analyticsRefreshIntervalMinutes,
                }
              )
            }
          />
