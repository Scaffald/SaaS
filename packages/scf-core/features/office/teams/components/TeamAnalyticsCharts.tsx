...
export function TeamAnalyticsCharts({
  ...
  const workloads = (workloadQuery.data?.snapshots ??
    []) as TeamWorkloadSnapshot[];

  const applicationsTrend = useMemo((): Array<{
    value: number;
    label: string;
  }> => {
  ...
  const timeToFirstReviewTrend = useMemo((): Array<{
    value: number;
    label: string;
  }> => {
    if (metrics.length === 0) return [];
  ...
  const workloadBreakdown = useMemo((): Array<{
    text: string;
    value: number;
  }> => {
    ...
    return workloads
      .map((snapshot) => {
        const value =
          Number(snapshot.activeAssignments ?? 0) +
          Number(snapshot.pendingAssignments ?? 0);
        return {
  ...
  const applicationsSummary = useMemo(() => {
    ...
    const totalReviewed = applicationsTrend.reduce(
  ...
  const timeToFirstReviewSummary = useMemo(() => {
    ...
    const latestPoint =
      timeToFirstReviewTrend[timeToFirstReviewTrend.length - 1];

    return `Average first-review time is ${averageHours.toFixed(
      1
    )} hours across the last ${
      timeToFirstReviewTrend.length
    } days. The most recent reading on ${
      latestPoint.label
    } was ${latestPoint.value.toFixed(1)} hours.`;
  ...
  const workloadSummary = useMemo(() => {
    ...
    const totalAssignments = workloadBreakdown.reduce(
    ...
    return (
      ...
      >
        <Stack gap={8}>
          ...
          <Text style={{ color: colors.text[theme].secondary }}>
            We&apos;ll start charting metrics once your team begins reviewing
            applications and recording activity.
          </Text>
        </Stack>
      </Card>
    );
    ...
    <Stack gap={16}>
      ...
      >
        ...
        >
          <Stack
            ...
            style={
              isSmallScreen
                ? { minWidth: Math.max(width - 48, 320) }
                : undefined
            }
          >
            <BarChart
              data={applicationsTrend.map((p) => p.value)}
              height={220}
      ...
      >
        ...
        >
          ...
          >
            <LinearChart
              data={timeToFirstReviewTrend.map((p) => ({
                x: p.label,
                y: p.value,
              }))}
              height={220}
