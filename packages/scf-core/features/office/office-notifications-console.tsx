...
export function OfficeNotificationsConsole() {
  ...
  return (
    <Stack gap={24}>
      {/* A real heading, at the level the page's other sections use (#839):
          this was a bare `Text`, so the console had no place in the document
          outline and the Overview's H2 was followed by nothing until the
          storage dashboard's. */}
      <Stack gap={8}>
            ...
            {deliveries.map((delivery, index) => {
              ...
              return (
                <Stack
                  ...
                  style={{
                    backgroundColor:
                      index % 2 === 0 ? colors.bg[theme].default : colors.bg[theme].subtle,
                  }}
                  padding="sm"
                >
                  <Row gap={12} align="center">
                    <Stack flex={2} gap={4}>
                      <Row gap={8} align="center">
                        ...
                        <NotificationTag size="sm">{severity.toUpperCase()}</NotificationTag>
                      </Row>
                      <Text style={{ color: colors.text[theme].secondary }}>
                    ...
                    <Stack style={{ flex: 1 }}>
                      <NotificationTag size="md">{delivery.status}</NotificationTag>
                    </Stack>
                    <Text style={{ flex: 1, color: colors.text[theme].secondary }}>
                      {delivery.attempts}
                    </Text>
          ...
          >
            ...
            {digestItems.map((item, index) => (
              <Row
                ...
                align="flex-start"
              >
                <Text style={{ flex: 1, color: colors.text[theme].secondary }}>{item.user_id}</Text>
                <Text style={{ flex: 1, color: colors.text[theme].secondary }}>{item.type}</Text>
                <Text style={{ flex: 1, color: colors.text[theme].secondary }}>{item.bucket}</Text>
                <Text style={{ flex: 1, color: colors.text[theme].secondary }}>{item.count}</Text>
                <Text style={{ flex: 2, color: colors.text[theme].secondary }}>
