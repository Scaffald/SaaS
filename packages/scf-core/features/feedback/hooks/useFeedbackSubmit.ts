...
async function convertScreenshotToBase64(
  ...
  if (screenshot.kind === "web") {
    ...
    return {
      ...
      mimeType:
        (screenshot.file.type || "image/png") as FeedbackPendingScreenshot[
          "mimeType"
        ],
...
export function useFeedbackSubmit(): UseFeedbackSubmitResult {
  ...
  const ensureSubmission = useCallback(
    async (
      payload: FeedbackSubmitInput,
      screenshot?: FeedbackPendingScreenshot | null,
    ) => {
      let screenshotPath: string | undefined = payload.screenshotPath ??
        undefined;

      if (screenshot) {
        const fileSize = screenshot.size ??
          estimateBase64Size(screenshot.base64);
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      ...
      if (failures.length > 0) {
        console.warn(
          "[useFeedbackSubmit] Failed to process pending feedback",
          failures,
        );
      }
    } finally {
    ...
    async ({ formValues, context, screenshotSource }: SubmitOptions) => {
      ...
      try {
        ...
        const payload: FeedbackSubmitInput = {
          feedbackType: formValues
            .feedbackType as FeedbackSubmitInput["feedbackType"],
        ...
        console.warn(
          "[useFeedbackSubmit] Submission failed, queueing locally",
          message,
        );
        ...
        try {
          ...
          const pending: FeedbackPendingSubmission =
            feedbackPendingSubmissionSchema.parse({
              feedbackType: formValues
                .feedbackType as FeedbackPendingSubmission["feedbackType"],
              feedbackText: formValues.feedbackText,
              screenshotPath: undefined,
              pageUrl: context.pageUrl,
              pageTitle: context.pageTitle ?? undefined,
