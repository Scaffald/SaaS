...
export function useTransactionReceipt(
  ...
  options?: UseQueryOptions<TransactionReceipt, Error>
) {
  const client = useScaffaldJobsClient()
...
export function useDepositCreditsMutation(
  options?: UseMutationOptions<DepositCreditsResponse, Error, DepositCreditsParams>
) {
  const client = useScaffaldJobsClient()
...
export function useCreditLedger(
  params: { organizationId: string; limit?: number; offset?: number; transactionType?: string },
  ...
  return useQuery<CreditLedgerResponse, Error>({
    ...
    enabled: !!client && Boolean(params.organizationId) && (options?.enabled !== false),
    staleTime: 30 * 1000,
    ...options,
  })
}
