...
const createEventId = () =>
  `${Date.now()}-${Math.random().toString(36).slice(2)}` as const;
...
const enforceQueueSizeLimit = async (queue: QueuedEvent[]) => {
  ...
  console.warn(
    "[analytics] Offline analytics queue trimmed due to size limit",
    {
...
export const flushQueue = async () => {
  ...
  for (const item of queue) {
    const success = captureEvent(
      item.name,
      item.properties as AnalyticsEventProperties,
    );
    if (success) continue;
    ...
    if (attempts >= MAX_RETRY_ATTEMPTS) {
      console.warn(
        "[analytics] Dropping queued analytics event after max retries",
        {
          event: item.name,
        },
      );
