...
const normalizeLocale = (languageTag?: string | null): SupportedLocale => {
  ...
  const candidates = [
    normalizedTag,
    normalizedTag.split("-")[0] ?? normalizedTag,
  ];

  const match = candidates.find(
    (candidate): candidate is SupportedLocale =>
      Boolean(candidate) && isSupportedLocale(candidate),
  );
...
const logTranslationWarning = ({
  ...
  const parts = [
    `[i18n] ${
      issue === "object"
        ? "Translation resolved to object"
        : "Missing translation"
...
if (isDevEnvironment) {
  ...
  for (const [localeCode, localeData] of Object.entries(translations)) {
    ...
    if (duplicates.length > 0) {
      console.warn(
        `[i18n] Duplicate translation keys detected in locale "${localeCode}": ${
          duplicates.join(
            ", ",
          )
        }`,
