  ...
  for (const record of records) {
    ...
    while (true) {
      ...
      synthetic.push({
        ...
        parent_id: grandparentCode ? toUUID(generateCodeKey(grandparentCode)) : null,
        active: true,
        csi_code: parentCode,
...
async function upsertSkills(records: CSIRecord[]): Promise<void> {
  ...
  for (let i = 0; i < records.length; i += batchSize) {
    ...
    const { error } = await supabase.schema('data').from('masterformat').upsert(batchData, {
      onConflict: 'code_key',
    })
    ...
    console.log(`Processed batch ${i / batchSize + 1}: ${batch.length} records`)
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`Total records (including synthetic parents): ${allRecords.length}`)
  ...
  try {
    ...
    console.log(`  Depth 1 (Divisions): ${allRecords.filter((r) => r.csi_depth === 1).length}`)
    console.log(`  Depth 2 (Level 2): ${allRecords.filter((r) => r.csi_depth === 2).length}`)
    console.log(`  Depth 3 (Level 3): ${allRecords.filter((r) => r.csi_depth === 3).length}`)
    console.log(`  Depth 4 (Level 4): ${allRecords.filter((r) => r.csi_depth === 4).length}`)
  } catch (err) {
