 ...
 *
 * Run with: deno test --allow-all packages/scf-supabase/functions/trpc/__tests__/performance-monitoring.test.ts
 */
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'test', method: 'query', durationMs: 100, success: true })
    monitor.record({ path: 'test', method: 'query', durationMs: 200, success: true })
    monitor.record({ path: 'test', method: 'query', durationMs: 300, success: true })
...
Deno.test({
  ...
  fn() {
    ...
    for (let i = 1; i <= 100; i++) {
      monitor.record({ path: 'test', method: 'query', durationMs: i, success: true })
    }
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'fast', method: 'query', durationMs: 100, success: true })
    monitor.record({ path: 'slow', method: 'query', durationMs: 600, success: true })
    monitor.record({ path: 'also-slow', method: 'query', durationMs: 1000, success: true })
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'documents.upload', method: 'mutation', durationMs: 100, success: true })
    monitor.record({ path: 'documents.upload', method: 'mutation', durationMs: 200, success: true })
    monitor.record({ path: 'documents.upload', method: 'mutation', durationMs: 300, success: false })

    monitor.record({ path: 'documents.get', method: 'query', durationMs: 50, success: true })
    monitor.record({ path: 'documents.get', method: 'query', durationMs: 60, success: true })

    const snapshot = monitor.getSnapshot()
    assertEquals(snapshot.endpointMetrics.length, 2)

    const uploadMetrics = snapshot.endpointMetrics.find((m) => m.path === 'documents.upload')
    ...
    const getMetrics = snapshot.endpointMetrics.find((m) => m.path === 'documents.get')
...
Deno.test({
  ...
  async fn() {
    ...
    monitor.record({ path: 'old', method: 'query', durationMs: 50, success: true })

    await new Promise((resolve) => setTimeout(resolve, 150))

    monitor.record({ path: 'new', method: 'query', durationMs: 50, success: true })
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'slow1', method: 'query', durationMs: 200, success: true })
    monitor.record({ path: 'slow2', method: 'query', durationMs: 500, success: true })
    monitor.record({ path: 'slow3', method: 'query', durationMs: 300, success: true })
    monitor.record({ path: 'fast', method: 'query', durationMs: 50, success: true })
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'test1', method: 'query', durationMs: 100, success: true })
    monitor.record({ path: 'test2', method: 'mutation', durationMs: 200, success: false })
...
Deno.test({
  ...
  fn() {
    ...
    // Add 20 metrics
    for (let i = 0; i < 20; i++) {
      monitor.record({ path: `test-${i}`, method: 'query', durationMs: 50, success: true })
    }
...
Deno.test({
  ...
  fn() {
    ...
    monitor.record({ path: 'test', method: 'query', durationMs: 100, success: true })
    monitor.record({ path: 'test', method: 'query', durationMs: 50, success: true })
    monitor.record({ path: 'test', method: 'query', durationMs: 200, success: true })

    const snapshot = monitor.getSnapshot()
    const endpointMetrics = snapshot.endpointMetrics.find((m) => m.path === 'test')
...
Deno.test({
  ...
  fn() {
    const monitor1 = getPerformanceMonitor()
    monitor1.record({ path: 'test', method: 'query', durationMs: 100, success: true })
