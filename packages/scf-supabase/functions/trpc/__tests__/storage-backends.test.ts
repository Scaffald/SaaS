 ...
 *
 * Run with: deno test --allow-all packages/scf-supabase/functions/trpc/__tests__/storage-backends.test.ts
 */
...
class MockStorageBackend implements IStorageBackend {
  ...
  private files: Map<string, { data: Uint8Array; options: UploadOptions }> = new Map()
  ...
  async getSignedUrl(path: string, expirySeconds: number): Promise<SignedUrlResult> {
    ...
    return {
      url: `https://mock-storage.example.com/${path}?token=mock&expires=${expirySeconds}`,
...
Deno.test({
  name: 'IStorageBackend - upload should return UploadResult with required fields',
  async fn() {
...
Deno.test({
  ...
  async fn() {
    ...
    const result3 = await backend.upload(new Uint8Array(100).fill(66), 'file3.pdf', {
      contentType: 'application/pdf',
    })
...
Deno.test({
  ...
  async fn() {
    ...
    await assertRejects(
      async () => {
        await backend.upload(createTestFile(), 'test.pdf', { contentType: 'application/pdf' })
      },
      undefined,
...
Deno.test({
  ...
  async fn() {
    ...
    await backend.upload(createTestFile(), 'to-delete.pdf', { contentType: 'application/pdf' })
...
Deno.test({
  ...
  async fn() {
    ...
    for (const contentType of contentTypes) {
      const result = await backend.upload(createTestFile(), `file.${contentType.split('/')[1]}`, {
        contentType,
      })
      assertEquals(result.storageBackend, 'supabase')
    }
...
Deno.test({
  ...
  async fn() {
    ...
    for (const path of paths) {
      await backend.upload(createTestFile(), path, { contentType: 'application/pdf' })
    }
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      await backend.upload(createTestFile(), 'fail.pdf', { contentType: 'application/pdf' })
    } catch (error) {
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      await backend.upload(createTestFile(), 'temp-fail.pdf', { contentType: 'application/pdf' })
    } catch {
      // Expected
    }
