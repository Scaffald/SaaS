 ...
 *
 * Run with: deno test --allow-all packages/scf-supabase/functions/trpc/__tests__/document-cache.test.ts
 */
...
Deno.test({
  ...
  fn() {
    const cache1 = getDocumentCache()
    cache1.set(DocumentMetadataCache.keyById('test'), createTestDocument('test'))
