...
type ValidationIssue = 'missing-key' | 'object-instead-of-string' | 'empty-value'
...
const loadRoutesConfig = (): RouteNode => {
  ...
  try {
    ...
    console.error('Failed to evaluate routes.ts while validating translations.')
    throw error
  }

  const routesExport = module.exports as { ROUTES?: RouteNode }
  if (!routesExport.ROUTES) {
    throw new Error('Failed to load ROUTES from routes.ts for translation validation')
  }
...
const isRouteConfig = (node: RouteNode): node is RouteConfig =>
  typeof node === 'object' && node !== null && 'path' in node && 'titleKey' in node
...
const flattenRoutes = (node: RouteNode, parents: string[]): RouteEntry[] => {
  if (isRouteConfig(node)) {
    ...
    const location = `packages/core/constants/routes.ts -> ${pathSegments.join('.')}`
...
const getTranslationValue = (record: TranslationRecord, titleKey: string) => {
  ...
  for (const segment of segments) {
    if (value && typeof value === 'object' && segment in (value as TranslationRecord)) {
    ...
    for (const error of routeValidationErrors) {
      console.error(
        [
          ...
          error.actualValue ? `  Actual Value: ${JSON.stringify(error.actualValue, null, 2)}` : null,
        ]
          .filter(Boolean)
          .join('\n'),
      )
...
console.log('✅ Translation validation passed (no duplicates, no route titleKey issues).')
