...
const capturedEvents: Array<
  { event: string; properties: Record<string, unknown>; distinctId?: string }
> = [];
...
const mockPlatform = {
  OS: "ios" as const,
  select: vi.fn(
    <T>(selections: { ios?: T; android?: T; web?: T; default?: T }) => {
...
// Helper function removed - not currently used in tests
// Future implementation would query PostHog API for events when POSTHOG_ALL_ACCESS is set
...
describe("analytics integration", () => {
  ...
  describe("event capture", () => {
    test("captures events with correct properties", async () => {
      const { initAnalytics, captureEvent, isAnalyticsInitialized } =
        await import("../client");
    ...
    test("rejects invalid event properties", async () => {
      ...
      // Test invalid properties - missing required is_new_user field
      const result = captureEvent("user_signed_in", {
        provider: "email",
        // Missing is_new_user
      } as { provider: string });
  ...
  describe("environment tagging", () => {
    test("events tagged with correct environment", async () => {
      ...
      (mockConstants.expoConfig.extra.analytics as { posthog?: { env?: string } }).posthog = { env: "staging" };
  ...
  describe("event batching and flushing", () => {
    test("events are batched before sending", async () => {
      ...
      expect(mockPostHogInstance.capture.mock.calls.length)
