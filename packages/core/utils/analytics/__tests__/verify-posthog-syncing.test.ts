...
describe.skipIf(!shouldRunVerification)('PostHog Event Syncing Verification', () => {
  test('can connect to PostHog API', async () => {
    const response = await queryPostHogEventsAPI(undefined, undefined, 1)
    
    expect(response).toBeDefined()
    console.log('[posthog-verification] ✅ PostHog API connection successful')
  ...
  test('can query events from PostHog', async () => {
    ...
    if (data.results && Array.isArray(data.results)) {
      console.log(`[posthog-verification] ✅ Found ${data.results.length} events in PostHog`)
      ...
      if (data.results.length > 0) {
        ...
        console.log('[posthog-verification] ⚠️  No events found in last 7 days')
        console.log('[posthog-verification] This could mean:')
        console.log('[posthog-verification]   1. Analytics is not sending events')
        console.log('[posthog-verification]   2. Events are being filtered out')
        console.log('[posthog-verification]   3. Events are being sent to a different project')
      }
    } else {
      console.log('[posthog-verification] Unexpected API response format:', Object.keys(data))
    }
  })

  test('can find analytics events by name', async () => {
    const eventTypes = ['user_signed_in', 'user_signed_out', 'job_viewed', 'auth_magic_link_requested']
    ...
    for (const eventType of eventTypes) {
      try {
        ...
        if (data.results && Array.isArray(data.results) && data.results.length > 0) {
          foundAnyEvents = true
          console.log(`[posthog-verification] ✅ Found ${data.results.length} ${eventType} events`)
          
          // Verify event properties
          const event = data.results[0]
          if (event.properties) {
            console.log(`[posthog-verification] ${eventType} event properties:`, {
              env: event.properties.env,
              source: event.properties.source,
              expo_channel: event.properties.expo_channel,
              hasRequiredProps: !!event.properties.provider || !!event.properties.job_id || !!event.properties.email_domain,
            })
            
            // Verify environment property exists
            if (event.properties.env) {
              expect(['development', 'staging', 'production']).toContain(event.properties.env)
              console.log(`[posthog-verification] ✅ Event has correct env: ${event.properties.env}`)
            }
            ...
            if (event.properties.source) {
              ...
              console.log(`[posthog-verification] ✅ Event has source: ${event.properties.source}`)
            }
          }
        ...
        console.warn(`[posthog-verification] Error querying ${eventType}:`, error)
      }
    }
    
    if (foundAnyEvents) {
      console.log('[posthog-verification] ✅ SUCCESS: Events are syncing to PostHog!')
    } else {
      console.log('[posthog-verification] ⚠️  WARNING: No analytics events found in PostHog')
      console.log('[posthog-verification] This suggests events are not being sent or are being filtered')
    }
  })
  ...
  test('verifies event properties match schemas', async () => {
    ...
    if (data.results && Array.isArray(data.results) && data.results.length > 0) {
      console.log(`[posthog-verification] Found ${data.results.length} user_signed_in events to validate`)
      ...
      for (const posthogEvent of data.results) {
        if (posthogEvent.properties) {
          ...
          const coreProperties = {
            ...
            has_anonymous_history: posthogEvent.properties.has_anonymous_history,
          }
          
          const validation = validateEventProperties('user_signed_in', coreProperties)
          
          if (validation.success) {
            console.log('[posthog-verification] ✅ Event properties match schema')
          } else {
            console.warn('[posthog-verification] ⚠️  Event properties validation failed:', validation.error)
          }
        }
      }
    }
...
describe('PostHog Sync Verification Setup', () => {
  test('verification credentials check', () => {
    if (!shouldRunVerification) {
      ...
      console.warn('To enable, set POSTHOG_ALL_ACCESS and POSTHOG_PROJECT_ID in .env')
