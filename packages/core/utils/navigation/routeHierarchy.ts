...
const ROUTE_ENTRIES = Object.entries(ROUTES) as Array<
...
const isDescendantOf = (
  info: TraversalResult,
  ancestorKey: RouteKey,
): boolean => info.ancestors.includes(ancestorKey);
...
export const getChildRoutes = (
  ...
  return hierarchyEntry.node.children
    .map((child) => routeMap[child.key])
    .filter(Boolean);
};
...
export const shouldShowInQuickLinks = (
  ...
  if (currentInfo.depth < MIN_QUICK_LINK_DEPTH) {
    return (
      candidateInfo.depth > currentInfo.depth &&
      isDescendantOf(candidateInfo, currentInfo.key)
    );
  }

  const anchorKey =
    getAncestorKeyAtDepth(currentInfo, MIN_QUICK_LINK_DEPTH) ?? currentInfo.key;
