...
function findRouteByPath(
  ...
  if (isRouteConfig(routeNode)) {
    ...
    if (
      routeNode.path === normalizedPath ||
      matchesRoute(normalizedPath, routeNode)
    ) {
  ...
  for (const [, value] of Object.entries(routeNode)) {
    if (isRouteConfig(value)) {
      if (
        value.path === normalizedPath || matchesRoute(normalizedPath, value)
      ) {
...
export function filterRoutesByRole(
  ...
  if (isRouteConfig(routeNode)) {
    ...
    if (routeNode.roles && routeNode.roles.length > 0) {
      const hasRequiredRole = routeNode.roles.some((role) =>
