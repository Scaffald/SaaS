...
vi.mock('react-hook-form', () => {
  return {
    ...
    Controller: ({ control, name, render }: { control: { values: Record<string, unknown> }; name: string; render: (props: any) => ReactNode }) => {
      const store = control
      const field = {
        name,
...
vi.mock('@app/ui', () => ({
  ...
  AddressAutocomplete: ({ value, onChange, onAddressSelect }: { value?: string; onChange: (value: string) => void; onAddressSelect: (result: any) => void }) => (
    <div>
      <input
...
const selectState = { onChange: (value: string) => {} }
