...
const toProjectSelectorOrganization = (
  ...
  const name = typeof input.name === 'string' && input.name.trim().length > 0 ? input.name : 'Unknown Organization'
  return {
    id,
    name,
    isAdmin: Boolean((input as { isAdmin?: unknown }).isAdmin ?? (input as { is_admin?: unknown }).is_admin),
    isOwner: Boolean((input as { isOwner?: unknown }).isOwner ?? (input as { is_owner?: unknown }).is_owner),
  }
}
...
const toProjectSelectorProject = (
  ...
  const name = typeof input.name === 'string' && input.name.trim().length > 0 ? input.name : 'Untitled Project'
  ...
  return {
    ...
    isArchived: Boolean((input as { isArchived?: unknown }).isArchived ?? (input as { is_archived?: unknown }).is_archived),
    startsAt,
    endsAt,
  }
}
...
export const normalizeProjectOptions = (input: unknown): ProjectOptionsData => {
  ...
  const organizations = Array.isArray(organizationsSource)
    ? organizationsSource
        ...
        .filter((organization): organization is ProjectSelectorOrganization => Boolean(organization))
    : []
