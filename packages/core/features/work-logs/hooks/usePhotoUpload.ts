...
const compressWebImage = async (
  file: File,
): Promise<{ data: Uint8Array; mimeType: UploadWorkLogPhotoInput["contentType"]; size: number }> => {
...
const compressNativeImage = async (
  ...
  const manipulator: typeof import("expo-image-manipulator") =
...
export const usePhotoUpload = ({
  ...
  const { data: workLogData, isLoading: isLoadingPhotos } =
    api.workLogs.getById.useQuery(
      { workLogId: workLogId ?? "" },
      {
        enabled: isReady,
        staleTime: 30_000,
  ...
  const updateMetadataMutation =
    api.workLogs.updatePhotoMetadata.useMutation();
  ...
  const runUpload = useCallback(
    async ({ candidate }: UploadPhotoOptions) => {
      ...
      try {
        let payload: { data: Uint8Array; mimeType: UploadWorkLogPhotoInput["contentType"]; size: number };
  ...
  const updatePhoto = useCallback(
    async (photoId: string, updates: UpdatePhotoOptions) => {
      ...
      const payload = {
        ...
        photoType:
          typeof updates.photoType === "string"
            ? updates.photoType
            : undefined,
