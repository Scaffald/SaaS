...
import type { UploadWorkLogPhotoInput } from '../schemas.ts'
import { deleteWorkLogPhotoSchema, updateWorkLogPhotoSchema } from '../schemas.ts'
import type { ResolvedWorkLogPhoto, WorkLogPhoto } from '../types/photos.ts'
...
export interface UsePhotoUploadReturn {
  ...
  togglePhotoVisibility: (photoId: string, showOnProfile: boolean) => Promise<void>
...
const mergePhoto = (photo: WorkLogPhoto, cache: SignedUrlCache): ResolvedWorkLogPhoto => {
...
export const usePhotoUpload = ({
  ...
  const { data: workLogData, isLoading: isLoadingPhotos } = api.workLogs.getById.useQuery(
    { workLogId: workLogId ?? '' },
    {
      enabled: isReady,
      staleTime: 30_000,
  ...
  const updateVisibilityMutation = api.workLogs.updatePhotoVisibility.useMutation()
  ...
  const storageUsage = useMemo(() => {
    const used = photos.reduce((total, photo) => total + Number(photo.fileSizeBytes ?? 0), 0)
    return {
      usedBytes: used,
    ...
    async (currentPhotos: WorkLogPhoto[]) => {
      ...
      await Promise.all(
        currentPhotos.map(async (photo) => {
          ...
          if (cached && cached.expiresAt - now > 30_000 && cached.url && !cached.isRefreshing) {
            return
          }
  ...
  const runUpload = useCallback(
    async ({ candidate }: UploadPhotoOptions) => {
      ...
      try {
        ...
        if (storageError) {
          throw new Error(storageError.message ?? 'Failed to upload photo to storage.')
        }
  ...
  const uploadPhoto = useCallback(
    async (options: UploadPhotoOptions) => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Unable to upload photo. Please try again.'
  ...
  const updatePhoto = useCallback(
    async (photoId: string, updates: UpdatePhotoOptions) => {
      ...
      const payload = {
        ...
        workLogId,
        caption:
          typeof updates.caption === 'string' ? updates.caption : (updates.caption ?? undefined),
        photoType: typeof updates.photoType === 'string' ? updates.photoType : undefined,
