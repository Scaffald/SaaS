...
describe('timeEntriesSchema', () => {
  ...
  it('rejects overlapping entries', () => {
    ...
    expect(() => timeEntriesSchema.parse(overlapping)).toThrowError(/Time overlap detected/);
  ...
  it('rejects entries that exceed 24 hours total', () => {
    ...
    expect(() => timeEntriesSchema.parse(marathon)).toThrowError(/cannot exceed 24 hours/);
  });
});
