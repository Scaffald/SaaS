...
type VerificationBadgeRow =
  Database["core"]["Views"]["v_id_verification_latest"]["Row"];
...
type TalentProfilesQueryOptions = Pick<
  UseTalentProfilesOptions,
  "bounds" | "limit"
>;

export const buildTalentProfilesQuery = (
  options: TalentProfilesQueryOptions = {},
) => {
      ...
      if (badgeError) {
        ...
        badgeMap = new Map(
          badgeRows
            .filter(
              (
                row: { worker_user_id: string | null },
              ): row is VerificationBadgeRow & { worker_user_id: string } =>
                typeof row.worker_user_id === "string" &&
                row.worker_user_id.length > 0,
            )
            .map((
              row: { worker_user_id: string },
            ) => [row.worker_user_id as string, row]),
        );
      }
    }
