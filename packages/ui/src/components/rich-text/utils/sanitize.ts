...
export function sanitizeTipTapJSON(content: JSONContent): JSONContent {
  ...
  if (
    content.type &&
    !ALLOWED_NODES.includes(content.type as (typeof ALLOWED_NODES)[number])
  ) {
  ...
  if (content.content && Array.isArray(content.content)) {
    sanitized.content = content.content.map((child) =>
      sanitizeTipTapJSON(child)
    ).filter(Boolean); // Remove any nullified nodes
  }
...
export function extractPlainText(content: JSONContent): string {
  ...
  // Handle lists
  if (
    (content.type === "bulletList" || content.type === "orderedList") &&
    content.content
  ) {
    text += content.content.map((child) => extractPlainText(child)).join("");
    text += "\n";
