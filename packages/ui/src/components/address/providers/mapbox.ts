...
export class MapboxProvider extends BaseGeocodingProvider {
  ...
  private buildSearchUrl(query: string, options: SearchOptions): string {
    ...
    if (options.types?.length) {
      params.append(
        "types",
        this.mapTypesToMapbox(options.types, options.zoomLevel).join(","),
      );
    } else if (options.zoomLevel) {
      params.append(
        "types",
        this.getTypesForZoomLevel(options.zoomLevel).join(","),
      );
    }
  ...
  private normalizeMapboxResult(feature: MapboxFeature): AddressResult {
    ...
    const streetAddress = [components.streetNumber, components.route].filter(
      Boolean,
    ).join(" ");
    ...
    return {
      ...
      stateAbbreviation: components.stateAbbreviation ||
        getStateAbbreviation(components.administrativeAreaLevel1),
