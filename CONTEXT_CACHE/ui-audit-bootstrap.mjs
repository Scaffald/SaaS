...
if (!gi.split(/\r?\n/).some(line => line.trim() === 'CONTEXT_CACHE/')) {
  fail('CONTEXT_CACHE/ not found in .gitignore. Please add a line with "CONTEXT_CACHE/" and re-run.');
}
log('.gitignore contains CONTEXT_CACHE/');

// 4) Initialize SQLite db via `sqlite3` if available
function execP(cmd) { return new Promise((resolve, reject) => exec(cmd, (e, stdout, stderr) => e ? reject(new Error(stderr || e.message)) : resolve({ stdout, stderr }))); }
