/**
 * Helper function to build route-explore ticket description
 * Based on docs/testing/route-task-review.md template
 */
...
**Naming Convention**:
- ${userLevel === 'super-admin' ? 'Super-admin' : userlevel === 'admin' ? 'Admin' : 'Regular'} user: \`test-${testPrefix}###-{name}.spec.ts\` (e.g., \`${testFileName}\`)
...
- \`${userType === 'super-admin' ? 'signInAsSuperAdmin(page)' : userType === 'admin' ? 'signInAsAdmin(page)' : 'signInAsTestUser(page)'}\` - Sign in as ${userLevel} user (\`${userEmail}\`)
