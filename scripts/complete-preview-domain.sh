#!/bin/bash
# Complete the preview.scaffald.com domain setup
...
DISTRIBUTION_ID="${AWS_CLOUDFRONT_DISTRIBUTION_ID:-E3R40C1707V33O}"
CERT_ARN="arn:aws:acm:us-east-1:625030017471:certificate/a41f1cd3-ebb1-428b-923f-0de6e08870ad"
...
aws cloudfront get-distribution-config \
    --id "$DISTRIBUTION_ID" \
    --profile "$AWS_PROFILE" \
    --output json > /tmp/preview-dist-config.json

ETAG=$(jq -r '.ETag' /tmp/preview-dist-config.json)
DIST_CONFIG=$(jq '.DistributionConfig' /tmp/preview-dist-config.json)
...
echo "$UPDATED_CONFIG" > /tmp/preview-updated-dist-config.json

echo "Updating CloudFront distribution..."
aws cloudfront update-distribution \
    --id "$DISTRIBUTION_ID" \
    --if-match "$ETAG" \
    --distribution-config file:///tmp/preview-updated-dist-config.json \
...
echo "$CHANGE_BATCH" > /tmp/preview-dns-change.json
CHANGE_ID=$(aws route53 change-resource-record-sets \
    --hosted-zone-id "$HOSTED_ZONE_ID" \
    --change-batch file:///tmp/preview-dns-change.json \
    --profile "$AWS_PROFILE" \
    --query 'ChangeInfo.Id' \
    --output text)
